...
- **A cell that is already running is not an error.** Starting a cell that was up answered with a conflict the sheet presented as a failure, with the buttons disabled, while the cell was simply already there. That answer now closes the sheet with a notice saying the cell is already running. A stop on a cell that is already down was never an error, and it is not treated as one. (commit e682efc)
- **A slow start on a remote node is reported as in progress, not as a failure.** The federated proxy answered the same way for a node it cannot reach and for one that is answering slowly, so a remote node that took longer than the proxy budget looked dead. The waiting case now carries its own cause and reaches the operator as a start in progress to be checked, while a genuinely unreachable node stays an error. (commit e682efc)
- **Outcome notices stay visible.** The notices reporting a degraded or failed outcome were written to a surface that the following refresh cleared about a hundred milliseconds later, so they were never actually read. They now live on their own notice surface with a ten-second life, independent of the refresh, and the notices that already existed use it too. (commits 806e5f6, a2117e8)
- **One behaviour wherever the action is offered.** The start and stop flow is a single shared handler for the mobile roster, the desktop sidebar and Settings → Fleet, which used to duplicate the confirmation three times; the settings sheet no longer waits for a refresh before closing after a successful start. (commits 0f82958, f87f515)

## 0.9.44 — 2026-09-24

- **The documentation says what the runtime does.** The adapter list in the README and in the Fleet guide now names every engine a clean installation ships — Grok and VL are in the catalog and were missing from that list, so a new user had no way to learn they existed. The tmux version stops being a requirement nobody verifies: `doctor` checks that tmux is present, not which version it is, and the guide now says that instead of promising a check that does not run. The skills table also gains the one skill the package carries that the table did not mention. (commits 951ebc1, a58893b)
- **The activity hooks state their own limit.** Hooks are installed only on the CLI versions that have been verified against them; on any other version no hook is injected and the cell runs without them. That was the behaviour in the code and nowhere in the guides, so an operator on a newer client would have lost the hooks without a line saying why. (commit 951ebc1)
- **The Live lease tools are documented.** The MCP guide listed twenty of the twenty-three tools the bridge exposes: the three that keep a cell registered with the node's Live lease now have their place — the one that can answer "pending" while the supervisor has not tracked the cell yet, the one that renews a registration that is still live, and the one that resumes the same incarnation from a recently expired proof — with the difference between them stated rather than left to the reader. (commit 951ebc1)
- **`nexuscrew help` lists the runtime commands too.** `serve`, `mcp`, `fleet-boot` and `identity provision` were reachable and invisible: the help omitted them on purpose, which left an operator unable to discover the commands their own service manager was running. They now appear under their own heading, apart from the public ones, with their behaviour taken from the commands themselves, and a test asserts they stay there. (commit 951ebc1)
- **The skills describe the runtime they ship with.** The capability tables gain what was missing — the VL micro-device tools, the spoken-alert controls and the Live lease tools in the entry-point skill — the agent skill gains the helper-path recipe and the lease guide, CellForge gains the real engine cap and the built-in capability set, and the AI-desktop skill states the Docker host requirement and the boundary of what happens without Docker. The companion manifest's primary tool names follow the skills again. (commits f8130c2, ba648e2, 65df947, 7e86691, 3ddbb27)
- **Two internal design contracts no longer reach the public repository.** The statusline telemetry note and the identity verify-channel contract stay in the working repository, where they are useful to whoever reads the code, and are excluded from the public tree by the same list that already keeps planning documents out; a stale release note from an early version is gone. Nothing about the product changes — the published tree simply stops carrying work documents. (commits a58893b, 763fe06)

## 0.9.43 — 2026-09-24

- **On the phone, the first tap previews a cell and the second opens it.** A tap on a row now shows that cell's stream in the upper half of the screen, read-only and in the same font as the main terminal, and the preview's − and + move that one value; the second tap on the same row opens the cell, after a fresh re-read that still refuses a cell which died between the two taps. Rows have a fixed height and clip what is inside them, so a long name, both badges and the telemetry stay within their own row and a tap cannot land on the row below any more; a cell states **one** condition instead of showing two at once, and the node name rises once into the group header instead of repeating on every row. The dot only reports the state — looking live is the row's first tap, and the preview replaces it on this surface. A scroll is not a tap, and a gesture that is cancelled, started by a second finger, or dragged past its threshold neither selects nor opens, while Enter and Space keep working as the keyboard activation. The selection dies with its row: a cell that disappears, stops, or leaves the active filter closes the preview, and when it comes back it must be selected again rather than reopening on its own. (commits f0c0638, 20f6fea, 2dd71d0, 9678c34, 48c66ae, 1a22171, 73e218e)
- **The mobile top bar keeps the zoom controls and gathers the rest into one menu.** The keyboard, files, panel and renderer switches leave the bar and move into the ⋯ menu, which is the same actions menu the cell rows already use, in its two existing shells — the bottom sheet on the phone and the anchored popover on the desktop: no second menu to learn, and the panel entry appears only for a cell that publishes one. The centre of the bar now carries both the engine and the state, and its subtitle truncates instead of sliding under the buttons on a narrow screen. (commit f0c0638)
- **A federated question that cannot be answered now says which of the two reasons it is.** The read-only card showed one message even when the cause was not the grant but a missing feed of the owner's node — the card had arrived over push while the subscription was never activated — so the button that was offered could not have fixed anything. The state of that view is now kept next to the grants: without it the card says to enable reception, with a live view and a denied grant the message stays what it was. Permissions are unchanged; only the text is. (commit 3b06b91)
- **codex-vl 0.156.1 is accepted by the cell hooks guard.** The fork's binary after the upstream merge reports itself as 0.156.1, and the table of proven versions admits it next to 0.155.1, with a test that covers acceptance and refusal — trust stays per exact hash, never per prefix, so any other version is still turned away. (commit b544f4f)

## 0.9.42 — 2026-09-23

- **A cell that has finished stays "stopped" instead of turning unverified.** The expiry window applied to every state, so a cell whose turn had ended more than five minutes earlier fell back to "unverified": the UI showed an uncertain cell while it was simply stopped, with the right generation. Expiry exists for one case only — a turn interrupted with Ctrl-C emits no event and would stay "working" forever — and it no longer applies to "stopped", a state that stays true and that the process ending already invalidates. (commit 3594905)
- **A "stopped" state no longer outlives what should invalidate it.** Three cases the removed expiry had been hiding: an internal restart happens inside the same launch, so the generation does not change — the supervisor now declares the client's exit before every return, and that reads as "unverified", never "stopped", while the client is dead or backing off; an activity file that carries no generation is not bound to any launch and is discarded once a launch publishes one; and if a launch cannot publish its generation, no hooks are injected for it — a state nobody can invalidate is more honestly reported as unverified, with a log line saying why. (commit 700816d)
- **A client's exit wins over timestamp order, and a discarded write says so.** A stop event whose timestamp sat inside the reader's future tolerance stayed published after the client had died: the exit carried an older timestamp, the writer discarded it while still returning success, and the view kept saying "stopped" over a dead client. The exit is authoritative now — it is republished right after the state it invalidates, never into the future the reader rejects, and the reading becomes "unverified" — and any other discarded event returns a failure, so the caller can tell "your event is not in the store" from "done". An event arriving after the exit renews the state: the exit invalidates the past, not the future. (commit 5846c0e)
- **Turn state is published for codex cells too, and only where the hook is trusted.** Codex cells published no turn state at all, so the UI showed them as unverified: true, but it was information that could be had. They now use codex's own hook system, under one extra constraint — a hook passed on the command line is not trusted until its hash appears in the client's hook state, otherwise the client opens a review dialog and the cell stalls on a question nobody sees. The hash is computed the way the client expects it (of the canonical JSON of a normalized identity, not of the command text), the hook table travels inline, interruption is part of the event matrix, and the events, the writer and the format are the same as for Claude cells, with the user's own hooks untouched. Hooks are injected only on proven versions, and the gate runs the binary the cell actually resolves rather than the one on the process path, with a short time cap: where the trust key cannot be proven (Windows, Termux) nothing is injected and the cell stays unverified with a log line, which costs less than a cell stuck on a dialog. (commits 119fa86, 250d469, ed4fe2d, 91ad9f0)
- **A stale answer can no longer overwrite a fresh one in the desktop rail.** The desktop poll runs every four seconds while a federated read may take up to eight, so two rounds stayed in flight together and the older answer could land last, rolling the cell list back to a superseded outcome. A round already in flight now makes the tick skip, a superseded outcome is never applied, the guard is invalidated when the effect is cleaned up and re-checked before every write, and every request carries a time cap below the poll period — **including the body**, which used to be uncovered because the timer stopped when the headers arrived: a body that does not arrive within the cap is a failed read, not an empty answer. A failed local read is also declared to the roster instead of being presented as authoritative. (commits 8051fd6, b62d204, f00b233, fef0908)
- **The AI desktop switch governs the container and the button that opens it.** The panel button that opened a blank page was a switch with no wire: the panel existed even with its container stopped, and the proxy's 502 reached the frame as an empty page that reported itself as loaded. The container is now commanded without a shell (argument vector, fixed name, time cap, outcome always reported), an explicit key rules, and a switch nobody has ever touched loses nothing — the default is derived from the container only where it makes sense, never in the hot status path. Settings gain the switch with the true outcome of the command: the save is not rolled back, so the switch states what is wanted and the badge states whether the world got there. The page served when the panel does not answer announces itself to the window that hosts it, with the trust placed in the receiver — the frame that is actually on screen, its declared origin, the expected type — so the panel moves from "loaded" to "unreachable" with Retry, and a failed start shows the cause instead of being wiped by a reopen that promises the same outcome. "Start" appears only for the local node's desktop panel: a remote cell never starts another node's container. Other people's containers keep the behaviour they had. (commits 79ef96a, 86d148e)
- **One menu for a cell's actions, on every surface.** Assigning or removing the Live host, pinning, booting at startup and watching a cell live are now a single menu with two shells — an anchored popover on the desktop and a sheet from the bottom on the phone — and an action that does not apply (capability missing, cell off, no handler) **is absent**: not a disabled item, an item that is not there, so the menu never promises what it cannot do. (commit da211e2)
- **In the desktop sidebar the row keeps fewer buttons and says more state.** The star and the boot toggle leave the row: a cell keeps the actions menu and the power button, and its state reads at a glance — two lines instead of one clipped, activity in relative form, the new-file badge, a LIVE mark on the cell hosting the stream, the reorder handle only while the pointer or the focus is there. The line naming the Live host becomes a strip, and the outcome of the last command arrives there too. Hovering the status dot raises a small preview of that cell — the same body as the full peek, three sources, stream by default — and the pin turns it into the free window. (commit 1a7985e)
- **On the phone, the cell list and the cell selector follow the same design.** The mobile home adopts the same actions menu and the same strip, and reordering becomes a **mode**: the handles appear when it is switched on, not always. Tapping a row **opens** the cell, and a fresh re-check always comes first — if the read fails the row does not say "off", it says it could not verify, and a verified dead cell remains the only case that can be called no longer active. The dot opens the live stream **directly**: on a phone there is no hover cloud, and looking is not going. The cell selector moves its header to the top, gains the reorder mode, marks "you are here" on the cell being worked in, gets its own actions sheet, and its status line is readable at 12 px — on a switched-off row the dimming no longer drags the status text down with it. A remote cell's preview window reads the sessions of **its own node**, never a locally same-named one. (commits 52ae1af, 22b315c, 9cf0701, 28bb61b, 23f9573, 8a8fe3d, caa9469)
- **A remote row's new-file badge reads its own node's sessions.** The count was read from the **local** sessions: a same-named session on this node attributed its files to the cell of another node, and when no same-named session existed the badge disappeared even with real new files waiting. (commit d63677b)

## 0.9.41 — 2026-09-23

- **Cells show three honest states instead of a guess.** Managed Claude cells now feed a per-cell activity record from real lifecycle hooks, served through `/sessions`: working, stopped, or unverifiable. Where the terminal title proves nothing, the UI says "unverified" instead of inventing idle; expired data downgrades to unverified rather than a fake idle, and a process death invalidates the record instead of freezing it. (commits d319230, e389509, 4490e89, c66acce)
- **Claude cells start their work only with their tools connected.** The bootstrap prompt of every managed Claude engine no longer travels on the command line — where the first turn began before the MCP servers finished connecting and the cell answered without any tool, forever — but through the supervised delivery the Kimi engines already use: the cell composer must be ready (never a login, trust or permission dialog) and the MCP servers materialized for the cell must be connected, read per-server from the client's own connection logs for the current generation only (older generations never unlock a new one). If the connect budget runs out, the cell still starts but visibly degraded, with the pending and failed servers listed instead of a silent partial start. Delivery stays at-most-once per generation, cancellation stops the wait before a respawn, and the Kimi contract is unchanged. (commits 08f6da3, 9a3bbee, 08e7777, 68a6b02, 95cbb86, f73d130)
- **The MCP connection log format is a versioned adapter.** The readiness reader declares the client version it was measured on, reads every line tolerantly (variable schema, malformed rows ignored), and a future client change degrades to a visible pending instead of crashing or guessing. (commit 08f6da3)
- **Grid tiles stay stable while owners answer at different speeds.** Four verified presence states (present, absent, unknown/stale, owner unhealthy) replace the old binary: a tile is recreated only when presence genuinely flips or identity provably changes; an unreadable session list never proves a session ended; the peer cache is bound to the pairing token and generation instead of time, and stale data never enables actions. The roster declares degradation instead of inferring it from a missing field. (commits 3367509, 6199fc7, 914f616, 9d56035, 3b778e5)
- **Codex per-cell profiles are valid on their own, so saving a model works again.** The generated profile copies the minimal transport fields from the user config, so every MCP table in it passes the client's standalone validation that `/model` performs before writing: disabled servers carry only the command or url (never env, args or credentials), on-demand servers keep their full definition, and an unparsable user config now refuses the launch instead of silently granting the whole surface. (commits fd721fc, e628490, 35a4877)
- **Notices from other nodes arrive and can be reviewed calmly.** A federated notify backlog is imported as a consultable, silent list — deduplicated per owner and event, bounded, and never sourced from revoked views — instead of repeated toasts. Ask cards answered from a different node can reconcile uncertain outcomes against the asking node instead of blind-resending, and the ask relay now resolves producers over shared inbound peers. (commits 77d4d7e, d38ebf6, a9586b6, 1f7ef7a)

## 0.9.40 — 2026-09-21

- **Deck rail keeps its order while owners answer at different times.** A background owner refresh used to rebuild the list by appending that owner's decks, so with several owners answering at different moments each group jumped to the end on every answer and the rail visibly reshuffled. The merge now replaces each deck in place (same id, same index), appends only genuinely new decks, and still drops the ones the owner no longer lists.
- **A half-open terminal connection is now noticed by the client itself.** The client sends an application ping on a cadence and, if it sees neither data nor a pong within the silence budget, closes and reconnects instead of waiting for a close that a dead tunnel never sends. The bridge always answers a ping, even on a silent session, without touching the PTY.
- **The terminal stream resumes by sequence.** Output frames are numbered and the bridge keeps a bounded ring per session, so a client that comes back asks for just the missing stretch; when the gap is not covered, it asks for a repaint from the pane instead of being handed a partial delta. No duplicated output.
- **The hub keeps a federated attach alive while the client is away.** Wired into the server (one store per server, its grace and ring size from the config), on a client drop the upstream attach is retained for a grace window and its output is buffered, so a client returning with the same identity (same token fingerprint, same route, same session) resumes the same attach without opening a new one; a different identity, an unknown id or an expired grace opens a new attach. Bounded: at most eight retained attaches, the oldest closed first. If the upstream dies while the client is attached, the client is told at once with the same readable close instead of waiting for the liveness budget. The attach id is 128 random bits per session and node, kept in sessionStorage (never on disk) and carried only in the upgrade query — the proxy never unmasks frames — and the token is compared by fingerprint, never held in clear.
- **Keystrokes typed while the channel is down are queued, not lost.** Input is buffered up to a small cap and delivered in order when the link returns; beyond the cap it is refused explicitly rather than dropped silently. The reconnecting overlay states how much is queued.
- **The reconnecting overlay appears only after a second, and the view is preserved.** A shorter blip no longer makes the terminal flicker; after a repaint the reader keeps their scroll position instead of being pulled to the bottom, and a discreet boundary line marks the restore only when the overlay was actually shown.
- **The terminal parameters are configurable and they act.** Under `terminal` in the server config (per-key merge, exposed by the config API, with documented defaults): `retryBaseMs` and `retryMaxMs` (reconnect backoff), `pingMs` and `deadMs` (liveness and silence budget), `queuedInputBytes` (input queued during a reconnect), `overlayDelayMs` (when the reconnecting overlay appears), `captureLines` (how much the resync repaints from the pane), `outputRingBytes` (the resume ring on the owning node) and `hubGraceMs` (how long the hub retains a federated attach). A value that is missing, non-numeric or not positive falls back to its default, so a bad config degrades to the known behaviour instead of breaking the terminal.
...
- **A closure that reached only some of the peers is retried towards the others.** The retry queue used to take a closure only when *no* peer had received it, so as soon as one peer answered, an unreachable one dropped out of the queue for good — and its copy stayed open, because from that side there is no way to repair it. A closure now enters the queue with the peers that did not answer, a second delivery for the same closure merges the missing peers instead of being discarded, and a retry goes only to those, closing the entry when none is left.
- **An engine now says where its credential came from, and warns when two files disagree.** Every "compatible" source used to look the same, so a key taken from the shell file, from `ai.env` or from `secure/.env` was indistinguishable — and a value in `secure/.env` silently overrides the canonical `ai.env`. The credential's origin is now reported (source, file path, modification time and the first 8 hex characters of the SHA-256 of the value; the value itself is never shown, logged or returned), and when the same variable is defined in more than one file with a *different* value the engine says so instead of looking healthy — including the case where a stale `providers.zsh` overrides the keys files, which is the same fault with the two sides swapped. The doctor gained an `engine credentials` section, and the engine status carries the same information.
- **The credential precedence is written down.** `docs/CONFIGURATION.md` now states the real order — environment, NexusCrew store, `providers.zsh`, the keys files (`ai.env` then `secure/.env`, which wins), legacy file — and the per-engine `credentialSourcePolicy`, which until now existed in the code and in the UI but was documented nowhere.
- **Removing a model now counts the cells that override it.** The "model in use" check looked only at engines, so a cell that pins the model for its own engine did not stop the removal: the engine kept working and the cell was left pointing at a model that no longer existed, which surfaced at the next start. The refusal now names both — the engines and the cells — and the model stays declared.
- **The tmux server probe says when it could not look.** A server that is not running and a probe that was refused (a permission error on the socket, for instance) were reported as the same thing, so a missing answer read as a healthy server. They are now two distinct outcomes, the second naming its cause.
- **An engine with its own endpoint is reported ready only if that endpoint answers.** A managed engine whose address is declared by hand used to be "ready" as soon as its binary and credential were in place: a router that was switched off looked healthy, and the mistake surfaced when the cell failed to start. NexusCrew now asks the address itself (`GET /v1/models`, 1.5 s budget, one probe per engine every 30 s) and says which host and port did not answer; an endpoint that answers with an authentication challenge counts as alive, because whether the key is the right one is the client's business, not this check's.
- **A model can be tested on an engine with its own endpoint.** The model test used to answer "unknown profile" for any engine that is not in the public catalog — exactly the internal routers nobody watches. It now asks the engine's own address: the model is looked up in the endpoint's list, and where the list is absent or empty it falls back to a single-token completion, so "the model is there" and "the model is not there" stay distinguishable. The credential is used where the test runs and never appears in the outcome.
