...
test('npm updater: riconosce installazioni globali Linux/macOS/Termux, non il checkout', () => {
  ...
  assert.equal(isGlobalInstall('/home/dag/Dev/20_ai-labs/nexuscrew'), false);
});
...
test('update errors redact registry credentials and local home paths', () => {
  const message = core.scrubError(new Error('https://user:password@registry.example /home/dag/.npm/_logs/x Bearer ' + 'Z'.repeat(44)));
