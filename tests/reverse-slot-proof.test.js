...
test('reverse slot proof: MAC valido senza Authorization bearer', () => {
  ...
  assert.deepEqual(proof.verifySlotProof({ secret, expected, response }).owned, true);
});
...
test('reverse slot proof: relay old-to-new non firma la challenge della vecchia slot', () => {
  ...
  assert.equal(proof.verifySlotProof({ secret, expected, response: relayed }).owned, false);
});

test('reverse slot proof: risposta autenticata con generation o MAC sbagliati non possiede la porta', () => {
  const response = proof.respondSlotProof({ secret, expected, request });
  assert.equal(proof.verifySlotProof({ secret, expected: { ...expected, generation: 3 }, response }).owned, false);
  assert.equal(proof.verifySlotProof({ secret, expected, response: { ...response, mac: 'x'.repeat(43) } }).owned, false);
});
