...
test('updater routes: busy is 409 redacted and READONLY blocks before manager', async (t) => {
  const busy = new Error('busy /home/tester/private'); busy.status = 409; busy.code = 'update-busy';
  let calls = 0;
  ...
  const body = await response.json(); assert.equal(body.code, 'update-busy'); assert.equal(body.error.includes('/home/tester'), false);
