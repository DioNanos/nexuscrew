...
async function localDelivery(t) {
  ...
  const r = await fetch(`http://127.0.0.1:${srv.address().port}/api/files/outbox`, {
    ...
    body: JSON.stringify({ session: 'sess1', path: src, caption: 'ready' }),
  });
...
function importedDelivery(t) {
  ...
  const envelope = (eventId, over = {}) => ({
    ...
    frame: { type: 'file-notice', name: 'report.pdf', caption: 'ready' },
    ...over,
  });
...
test('file-notice: consegna locale e file-notice importato sono la STESSA classe di avviso', async (t) => {
  ...
  // Il file importato resta attribuito alla SUA cella: senza questo campo il
  // relay lo contava come avviso di nodo (budget diverso) invece che della
  // cella, che e' cio' che il produttore file dichiara.
  assert.equal(imported.hubFrames[0].originCell, 'dev', 'il file importato conserva la cella');
