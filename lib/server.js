...
const { listSessions, listPanes, attachedClients, setSessionVisibility } = require('./tmux/list.js');
const { statiSupervisore: statiSupervisoreDi, senzaSupervisore } = require('./tmux/supervisor-pane.js');
...
const { leggiAttivita, leggiGenerazione } = require('./files/activity.js');
...
const { createLiveHostFederation } = require('./live-host/federation.js');
...
const { createPushRelay, createPushDedup } = require('./notify/push-relay.js');
const { createAsksStore } = require('./notify/asks.js');
const { createNotifier } = require('./notify/notifier.js');
const { createEventFeed } = require('./notify/event-feed.js');
const { createEventFeedHistory } = require('./notify/event-feed-history.js');
const { buildEnvelope } = require('./notify/event-feed-producers.js');
const { createEventFeedRoutes } = require('./notify/event-feed-routes.js');
const eventFeedAcl = require('./notify/event-feed-acl.js');
const { createEventFeedClient } = require('./notify/event-feed-client.js');
const { createAskReceipts } = require('./notify/ask-receipts.js');
const { createAskAnswerService } = require('./notify/ask-answer-service.js');
const { createEventFeedAsksRoutes } = require('./notify/event-feed-asks-routes.js');
const { createAskRelay } = require('./notify/ask-relay.js');
const { readFederationEventsEnabled } = require('./config.js');

function sanitizeReplyLabel(raw) {
  return String(raw === undefined || raw === null ? 'human' : raw)
    .replace(/[\x00-\x1f\x7f\]]/g, '').slice(0, 32) || 'human';
}
const { notifyRoutes, createClosureFanout } = require('./notify/routes.js');
const { createClosureRetryQueue } = require('./notify/closure-retry.js');
...
// I defaults del bridge ws vengono dalla config del server. E' una funzione a
// se' perche' il cablaggio sia verificabile senza montare una sessione tmux.
function wsBridgeDefaults(cfg = {}) {
  const terminal = cfg.terminal && typeof cfg.terminal === 'object' ? cfg.terminal : {};
  return {
    readonlyDefault: cfg.readonlyDefault,
    tmuxBin: cfg.tmuxBin,
    outputRingBytes: terminal.outputRingBytes || undefined,
    captureLines: terminal.captureLines || undefined,
  };
}

function createServer(opts = {}) {
  const cfg = loadConfig(opts);
  // UNA istanza per server: trattiene le attach federate quando il client cade,
  // cosi' chi torna con la stessa identita' riprende dal buffer. I parametri
  // vengono dalla config del server, non da valori cablati.
  const hubAttaches = federation.createHubAttachStore({
    graceMs: cfg.terminal && cfg.terminal.hubGraceMs,
    ringBytes: cfg.terminal && cfg.terminal.outputRingBytes,
    maxAttaches: 8,
  });
  ...
  // createServer) to close active WS sessions on token rotation.
  let wss = null;
  ...
  // OS alerts for IMPORTED notifications: the same push service (endpoint/DNS/
  // SSRF checks, 404/410 cleanup, READONLY floor) with its OWN queue, its own
  // alert budget and a durable (owner, eventId) registry — an event already
  // alerted never rings twice, across a replay or a restart. It is fed only by
  // the import hub below, so a local emission never takes this path.
  const pushRelay = createPushRelay({
    pushDedup: createPushDedup({ filePath: path.join(notifyDir, 'push-relay-dedup.json') }),
    send: (payload) => pushSvc.sendToAll(payload),
    log: opts.log || console.log,
  });
  const asksStore = createAsksStore({ dir: notifyDir });
  // --- Event feed (owner side) -------------------------------------------------
  // The notifier stays the single emission point; the feed is a SIDE EFFECT of
  // a local emission, never a separate path. Federated ingress and feed import
  // call deliverOnly(): local delivery that does NOT re-enter the publisher —
  // an imported event can never be re-exported, so A→B→A dies by construction.
  const notifierBase = createNotifier({ hub: eventsHub, push: pushSvc });
  const feedHistory = createEventFeedHistory({
    filePath: path.join(notifyDir, 'event-feed-history.json'),
  });
  // Kill switch: strict boolean, read FRESH on every use so flipping it in
  // config.json acts on live streams without a restart.
  const eventsEnabled = () => readFederationEventsEnabled({ configJsonPath: cfg.configPath, federationEventsEnabled: cfg.federationEventsEnabled });
  const eventFeed = createEventFeed({
    log: opts.log || console.log,
    // A heartbeat is also a revoke checkpoint: grants re-read without events.
    validatePeer: (peerNodeId) => {
      if (eventsEnabled() !== true) return false;
      const st = nodesStore.loadStore(nodesPath);
      const grants = st ? eventFeedAcl.peerGrants(st, peerNodeId) : null;
      return !!(grants && grants.configured && grants.eventsAccess === true);
    },
  });
  // Durable receipts + the ONE answer cycle shared by the local route and the
  // federated ask surface. The closure event flows through the notifier
  // wrapper so the feed gets a qualified ask-closed envelope.
  const askReceipts = createAskReceipts({ filePath: path.join(notifyDir, 'ask-receipts.json') });
  // Recapito della chiusura di un ask verso i peer che ne hanno un alias.
  // Assegnato piu' sotto, quando esistono dispatcher e topologia; l'hook lo
  // legge a runtime, quindi la dichiarazione in avanti basta.
  let closureFanout = null;
  // Recapito recuperabile della chiusura (lato owner): la voce si crea dove
  // esiste il fan-out, ma la coda serve prima, sulla lettura degli ask.
  let closureRetry = null;
  const askAnswerService = createAskAnswerService({
    asks: asksStore,
    paste: cfg.pasteSeam || ((session, text) => pasteToSession(cfg.tmuxBin, session, text)),
    receipts: askReceipts,
    // La chiusura esce da QUI: e' il punto in cui OGNI transizione autorevole
    // passa — risposta e scarto, rotta locale e rotta federata. Emetterla dalle
    // due route locali lasciava fuori proprio il caso normale, la risposta che
    // arriva da un altro nodo. Il fan-out e' lo stesso dell'andata e NON parte
    // per un ask importato: la chiusura di quello la decide il suo owner.
    onClosure: (kind, info) => {
      try { notifier.emitRaw({ type: kind, id: info.askId }); } catch (_) {}
      try {
        const ask = asksStore.get(info.askId);
        if (ask && !ask.originNode && closureFanout) {
          return closureFanout.dispatch({
            askId: info.askId,
            outcome: kind === 'ask-dismissed' ? 'dismissed' : 'answered',
            session: (ask && ask.session) || info.cellSession,
          });
        }
      } catch (_) {}
      return null;
    },
    labelPrefix: sanitizeReplyLabel(cfg && cfg.replyLabel),
  });
  const feedSignatures = new Map(); // peerNodeId -> grant signature last seen
  const apiOwnerId = () => (nodesStore.loadStore(nodesPath) || {}).nodeId || null;

  function publishEnvelopeToPeers(envelope) {
    if (eventsEnabled() !== true) {
      eventFeed.invalidateAll('events-disabled');
      return;
    }
    const store = nodesStore.loadStore(nodesPath);
    if (!store) return;
    for (const node of store.nodes) {
      if (!node || node.direction !== 'inbound' || !node.nodeId) continue;
      const grants = eventFeedAcl.peerGrants(store, node.nodeId);
      const usable = grants && grants.configured && grants.eventsAccess === true;
      if (!usable) {
        // Lost the grants: the view dies BEFORE anything else is served.
        if (eventFeed.status(node.nodeId)) {
          eventFeed.invalidatePeer(node.nodeId, 'revoked');
          eventFeed.disablePeer(node.nodeId);
          feedSignatures.delete(node.nodeId);
        }
        continue;
      }
      const seen = feedSignatures.get(node.nodeId);
      if (seen !== undefined && seen !== grants.signature) {
        eventFeed.invalidatePeer(node.nodeId, 'grants-changed');
      }
      feedSignatures.set(node.nodeId, grants.signature);
      if (!eventFeedAcl.allowsEnvelope(grants, envelope)) continue;
      // Per-peer projection BEFORE the ring: a node frame may carry cell
      // lists, and a hidden cell never reaches the peer's view in any form
      // (no names, no counts) — node events do not bypass cell visibility.
      let projected = envelope;
      if (envelope.scope === 'node' && Array.isArray(envelope.frame && envelope.frame.cells)) {
        projected = {
          ...envelope,
          frame: { ...envelope.frame, cells: eventFeedAcl.projectCells(grants, envelope.frame.cells) },
        };
      }
      try {
        if (!eventFeed.status(node.nodeId)) eventFeed.enablePeer(node.nodeId);
        eventFeed.publish(node.nodeId, projected);
      } catch (_) { /* one peer's budget never blocks the others */ }
    }
  }

  // Local emission → envelope + durable history + per-peer publication.
  function recordAndPublish({ producer, cellId, payload }) {
    const ownerId = apiOwnerId();
    if (!ownerId) return null;
    let envelope;
    try {
      envelope = buildEnvelope({ type: producer, payload, ownerId, cellId, now: Date.now });
    } catch (e) {
      try { diagnostics.record('warn', 'notify', 'EVENT_FEED_PRODUCER_REFUSED', String(e.message || e), { producer }); } catch (_) {}
      return null;
    }
    try { feedHistory.record(envelope); } catch (_) { /* history degrades, notify still lives */ }
    publishEnvelopeToPeers(envelope);
    return envelope;
  }

  function cellIdForFrame(frame) {
    try { return cellIdFromTmuxSession(frame && frame.session); } catch (_) { return null; }
  }

  const notifier = {
    // Local emission: UI + push + feed. `federated: true` is an INTERNAL flag
    // of the caller, never a body field — a remote notify cannot ask to be
    // treated as local.
    async emit(frame, opts = {}) {
      const result = await notifierBase.emit(frame);
      if (opts.federated !== true) {
        const cellId = cellIdForFrame(frame);
        recordAndPublish({
          producer: cellId ? 'notify' : 'notify-node',
          cellId,
          payload: { ...frame, ts: Date.now() },
        });
      }
      return result;
    },
    // Raw UI frames: ask cards and their closures carry the ask id into the
    // feed so both sides see the same object.
    emitRaw(frame) {
      const out = notifierBase.emitRaw(frame);
      try {
        if (frame && frame.type === 'ask' && frame.ask) {
          const ask = frame.ask;
          const cellId = cellIdFromTmuxSession(ask.session);
          if (cellId) {
            recordAndPublish({
              producer: 'ask', cellId,
              payload: { askId: ask.id, revision: ask.revision || 0, question: ask.question, options: ask.options, session: ask.session, ts: Date.now() },
            });
          }
        } else if (frame && (frame.type === 'ask-answered' || frame.type === 'ask-dismissed')) {
          const all = asksStore.list({ open: false }).concat(asksStore.list({ open: true }));
          const ask = all.find((a) => a && a.id === frame.id);
          const cellId = ask ? cellIdFromTmuxSession(ask.session) : null;
          if (cellId) {
            recordAndPublish({
              producer: 'ask-closed', cellId,
              payload: { askId: frame.id, revision: (ask && ask.revision) || 0, outcome: frame.type === 'ask-dismissed' ? 'dismissed' : 'answered', ts: Date.now() },
            });
          }
        }
      } catch (_) { /* the local raw frame must not fail because of the feed */ }
      return out;
    },
    // Delivery-only: federated ingress and feed import land here. NO history,
    // NO publisher — an imported event is never re-exported.
    async deliverOnly(frame) { return notifierBase.emit(frame); },
    deliverOnlyRaw(frame) { return notifierBase.emitRaw(frame); },
    publishState: (producer, payload) => recordAndPublish({ producer, cellId: null, payload }),
  };
  ...
  // Multi-node: nodes.json e' la fonte dati. Il proxy risolve <name>
  ...
  const app = express();
  // CSP frame-ancestors (sicurezza 2026-08-16, rimedio 4 del report): la
  ...
  // Porta pannello dedicata (sicurezza 2026-08-16): un iframe
  ...
  api.get('/sessions', async (_req, res) => {
    try {
      ...
      // UNA chiamata per giro per sapere chi ha ancora il suo supervisore.
      const statiSupervisore = sessions.length
        ? statiSupervisoreDi(await listPanes(cfg.tmuxBin))
        : new Map();
      ...
      const enriched = await Promise.all(sessions.map(async (s) => {
        ...
        const statoDelSupervisore = statiSupervisore.get(s.name) || 'assente';
        ...
        return {
          ...
          // Stato del turno pubblicato dagli hook della cella: lavora /
          // ferma / attesa. `null` NON e' «ferma»: e' «non verificato» —
          // nessun evento entro la finestra, oppure una cella che non ha questo
          // canale (codex-vl, pi, shell). La UI non deve dedurre «ferma» da un
          // silenzio: e' esattamente l'errore che il titolo del pane fa oggi.
          //
          // E SI LEGGE SOLO SE IL SUPERVISORE DELLA CELLA ESISTE ANCORA. Con
          // `remain-on-exit` (runtime.js) la sessione sopravvive al suo
          // processo: un supervisore ucciso (SIGKILL, OOM) lascia la sessione
          // «attiva», e lo `Stop` con `exit:1` scritto prima resterebbe «ferma»
          // per sempre — mentre non c'e' nessun client a cui quello stato si
          // riferisca. La verita' e' il pane che esegue cell-exec, marcato al
          // lancio: `#{pane_dead}` della sessione riguarda il pane ATTIVO, e con
          // una seconda finestra viva selezionata direbbe «vivo» a supervisore
          // morto (misurato). La decisione completa sta in
          // `lib/tmux/supervisor-pane.js`: qui si passa lo stato del pane e — per
          // il solo caso «nessun marcatore» — il segno `exit:1` del file, che
          // distingue un lancio nuovo con marcatura fallita (non si legge: il
          // suo `Stop` non scadrebbe) da un lancio VECCHIO (si legge, e il
          // lettore fa scadere tutto a 5 minuti). E' quello che evita di
          // mostrare «non verificato» su ogni cella viva di ogni nodo appena
          // aggiornato.
          attivita: statoDelSupervisore === 'vivo'
            ? leggiAttivita(cfg.filesRoot, s.name)
            : (senzaSupervisore(statoDelSupervisore, leggiGenerazione(path.join(cfg.filesRoot, s.name)).uscitaGarantita)
              ? null
              : leggiAttivita(cfg.filesRoot, s.name)),
        };
      }));
  ...
  api.get('/config', (_req, res) => res.json({
    ...
    // i parametri del terminale sono configurabili dal server e leggibili
    // dal client (che ne ha comunque i default iniettabili).
  ...
  // MCP bridge: /notify, /push/*, /asks — behind the same Bearer
  // of the /api router; READONLY gate on the mutators inside notifyRoutes.
  ...
  // UN SOLO dispatcher per le domande e per le chiusure: sono la stessa strada
  // percorsa in due momenti, e il recapito della chiusura deve usare lo stesso
  // instradamento dell'andata, non una copia con la sua topologia.
  const askDispatcher = createDispatcher({
    ...
    // 'no-delivery' (-): esito legittimo del target — richiesta accettata
  ...
  const askPeerTargets = async () => {
    const list = await federatedPeers();
    return (Array.isArray(list) ? list : []).map((p) => p && (p.nodeId || p.instanceId)).filter(Boolean);
  };
  // Riconciliazione del RICEVENTE: per ogni alias ancora aperto si chiede
  // all'owner quali domande sono ancora aperte. Se l'alias non c'e' piu' la'
  // (l'owner ha risposto o scartato mentre questo nodo era spento), l'alias si
  // chiude qui. E' la via minima al recapito recuperabile: niente coda
  // persistente con backoff, un confronto con la fonte autorevole. Sotto il
  // tetto dello snapshot non si conclude nulla, perche' un'assenza potrebbe
  // essere un taglio della lista e non una chiusura.
  const SNAPSHOT_FULL = 100;
  async function reconcileImportedAsks() {
    const store = nodesStore.loadStore(nodesPath);
    if (!store) return;
    const open = asksStore.list({ open: true }).filter((a) => a.originNode && a.ownerId && a.ownerAskId);
    ...
    for (const [ownerId, aliases] of byOwner) {
      const peer = (store.nodes || []).find((n) => n && n.nodeId === ownerId && n.token && n.localPort);
      if (!peer) continue;
      let snap = null;
      try {
        const res = await fetch(`http://127.0.0.1:${peer.localPort}/federation/route/_/event-feed/snapshot`, {
          headers: { authorization: `Bearer ${peer.token}` }, signal: AbortSignal.timeout(3000),
        });
        if (!res.ok) continue;
        snap = await res.json();
      } catch (_) { continue; }
      const live = new Set((snap && Array.isArray(snap.asks) ? snap.asks : []).map((x) => String(x.id)));
      if (live.size >= SNAPSHOT_FULL) continue;
      for (const alias of aliases) {
        if (live.has(String(alias.ownerAskId))) continue;
        try {
          const closed = asksStore.closeImported({ ownerId, ownerAskId: alias.ownerAskId, outcome: 'dismissed' });
          if (closed.changed && closed.ask) {
            try { notifier.deliverOnlyRaw({ type: 'ask-dismissed', id: closed.ask.id, ownerId }); } catch (_) {}
          }
        } catch (_) { /* un alias non chiudibile non ferma gli altri */ }
      }
    }
  }
  // Assegnato ORA che dispatcher e topologia esistono (l'hook lo legge a runtime).
  closureRetry = createClosureRetryQueue({
    log: opts.log || console.log,
    // Il tentativo nasce DALLA coda: non si riaccoda (sarebbe autoalimentata).
    // `targets` e' l'insieme dei pendenti della voce: il ritentativo va SOLO
    // verso quelli. Senza, ogni ritentativo rimanderebbe la chiusura anche a chi
    // l'ha gia' ricevuta — rumore, non danno, ma nemmeno una coda che sa cosa
    // le manca.
    run: ({ askId, outcome, session, targets }) => (closureFanout
      ? closureFanout.dispatch({ askId, outcome, session, retryOnFailure: false, targets })
      : Promise.resolve([])),
  });
  closureFanout = createClosureFanout({
    dispatcher: askDispatcher,
    peerTargets: askPeerTargets,
    localNodeId: federatedNodeId,
    log: opts.log || console.log,
    retry: closureRetry,
  });
  ...
  api.use(notifyRoutes({
    ...
    asks: asksStore,
    paste: cfg.pasteSeam || ((session, text) => pasteToSession(cfg.tmuxBin, session, text)),
    // Stesso seam dell'attach: i test non devono parlare con tmux vero.
    sessionExists: cfg.sessionExistsSeam || ((name) => sessionExists(cfg.tmuxBin, name)),
    fleetP,
    instanceId: apiNodeId,
    identityMode,
    localNodeId: federatedNodeId,
    ...
    closureFanout,
    peerTargets: askPeerTargets,
    reconcileImported: reconcileImportedAsks,
    closureRetry,
    federatedRate: createSpeakRateLimiter(),
    answerService: askAnswerService,
    receipts: askReceipts,
    // Elenco dei peer autorizzati per il fan-out di default della domanda
    // (nessun `target` → tutti i nodi dell'owner). Il
    // dispatcher resta a target esatto: l'enumerazione la fa la route.
    peerTargets: async () => {
      const list = await federatedPeers();
      return (Array.isArray(list) ? list : [])
        .map((p) => p && (p.nodeId || p.instanceId))
        .filter(Boolean);
    },
    log: opts.log || console.log,
  }));
  // Event feed: the closed SSE/JSON surface, mounted inside the
  // authenticated API. Its own gate demands a proven federated origin and an
  // exact two-hop chain — there is no local fallback.
  api.use('/event-feed/asks', createEventFeedAsksRoutes({
    nodesPath,
    localNodeId: federatedNodeId,
    originResolver: createOriginResolver({
      localNodeId: federatedNodeId,
  ...
  api.use('/event-feed', createEventFeedRoutes({
    nodesPath,
    localNodeId: federatedNodeId,
    originResolver: createOriginResolver({
      localNodeId: federatedNodeId,
  ...
  // Client-side subscriber: for every outbound peer the
  // operator flagged eventsReceive, pull the owner's feed (snapshot first),
  // re-emit imported events LOCALLY with owner attribution. Imports never
  // touch the notifier wrapper, so a client never re-exports what it imports.
  // Ask relay (federated asks, client side): the PWA posts here, the relay
  // resolves the owner from the authorized store and forwards with the peer's
  // own pair token. Uncertain outcomes are parked, never retried silently.
  const askRelay = createAskRelay({
    loadStore: () => nodesStore.loadStore(nodesPath),
    log: opts.log || console.log,
  });
  // Il body va parsato QUI: il router `api` non monta un parser globale, ogni
  // rotta porta il suo (`express.json`) — vedi /sessions, /notify, /asks.
  // Senza, `req.body` e' undefined, il relay legge un body vuoto e risponde
  // 400 «ownerId, askId e text richiesti» a OGNI richiesta: la risposta
  // federata non ha mai potuto attraversare, e nessun test copriva questa
  // rotta. Difetto preesistente, trovato scrivendo il test del ritorno federato.
  api.post('/asks-relay', express.json({ limit: '8kb' }), async (req, res) => {
    try {
      const b = req.body || {};
      if (b.action === 'dismiss') {
        const out = await askRelay.relayDismiss({ ownerId: b.ownerId, askId: b.askId });
        return res.status(out.ok ? 200 : out.code || 500).json(out.ok ? { dismissed: true } : { error: out.error || 'unknown' });
      }
      if (b.action === 'verify') {
        const out = await askRelay.verifyStatus({ ownerId: b.ownerId, askId: b.askId, requestId: b.requestId });
        return res.status(out.ok ? 200 : out.code || 500).json(out.ok ? { state: out.state } : { error: out.error || 'unknown', reason: out.reason });
      }
      const out = await askRelay.relayAnswer({ ownerId: b.ownerId, askId: b.askId, text: b.text });
      return res.status(out.ok ? 200 : out.code || 500).json({
        ...(out.ok ? { status: out.status } : {}),
        requestId: out.requestId || b.requestId,
        ...(out.uncertain ? { uncertain: true, reason: out.reason } : {}),
        ...(!out.ok && !out.uncertain ? { error: out.error, reason: out.reason } : {}),
      });
    } catch (e) { res.status(500).json({ error: String(e.message || e) }); }
  });
  // Riconciliazione dopo refresh: gli esiti incerti noti al relay (solo
  // contatori/ID, mai testo) per ricostruire lo stato «verifica» senza reinvii.
  api.get('/asks-relay/state', (_req, res) => {
    try { res.json({ attempts: askRelay.state() }); }
    catch (e) { res.status(500).json({ error: String(e.message || e) }); }
  });
  // The client re-emits imported events into the local hub (deliverOnly: the
  // notifier is bypassed on purpose). The alert relay hangs off that same call,
  // so ONLY an imported notify can ring this device, and never through the
  // owner-side publisher. READONLY keeps the relay off: its registry is a
  // durable write.
  const importHub = {
    broadcast: (event) => {
      eventsHub.broadcast(event);
      if (!bridgeReadonly()) pushRelay.considerImported(event);
    },
  };
  const eventFeedClient = createEventFeedClient({
    loadStore: () => nodesStore.loadStore(nodesPath),
    token: () => tokenHolder.value,
    eventsHub: importHub,
    log: opts.log || console.log,
    pollMs: cfg.eventFeedClientPollMs,
  });
  eventFeedClient.start();
  // Local aggregated read for the UI: connected views only, with the cursor
  // watermark so the first fetch cannot race the live stream. Local
  // authenticated route, NOT in the federation allowlist.
  api.get('/feed-state', (_req, res) => {
    try { res.json(eventFeedClient.state()); }
    catch (e) { res.status(500).json({ error: String(e.message || e) }); }
  });

  // Local, authenticated observability for the feed (doctor / nodes show feed
  // it). Deliberately NOT in the federation allowlist: diagnostics stay local.
  api.get('/diagnostics/events', (_req, res) => {
    try {
      // Read-only alert-relay counters for the peers the operator is looking
      // at: numbers and reasons only — never an endpoint, a subscription or a
      // key, and nothing here opens a connection anywhere.
      const relayStatus = pushRelay.status();
      const pushRelaySection = {
        global: { ...relayStatus.counters, queued: relayStatus.queued, active: relayStatus.active },
        byOwner: relayStatus.byOwner,
      };
      const store = nodesStore.loadStore(nodesPath);
      const peers = [];
      if (store) {
        for (const node of store.nodes) {
          ...
          peers.push({
            name: node.name,
            nodeId: node.nodeId,
            accessLabel: grants ? grants.label : 'unknown',
            grants: grants ? {
              eventsAccess: grants.eventsAccess,
              nodeEventsAccess: grants.nodeEventsAccess,
              cellVisibility: grants.cellVisibility,
              cells: grants.cells,
              configured: grants.configured,
            } : null,
            accessRevision: nodesStore.accessRevisionOf(store),
            feed: eventFeed.status(node.nodeId),
          });
        }
      }
      res.json({ enabled: eventsEnabled(), peers, pushRelay: pushRelaySection });
    } catch (e) { res.status(500).json({ error: String(e.message || e) }); }
  });
  api.use('/fleet', fleetRoutes(fleetP, {
    ...cfg, diagnostics,
    // Fleet state changes are events too: the owner resolves the cell and
    // publishes the normalized delta; grants are applied per peer on publish.
    onFleetStateChange: (delta) => {
      try {
        notifier.publishState('fleet-state', {
          cells: [{ cell: delta && delta.cell, active: delta && delta.active === true }],
          ts: Date.now(),
        });
      } catch (_) { /* observability must never break the action */ }
    },
  }));
  // Fetta 2b: superficie child del lease Live via canale nativo del
  ...
  // Live host cell: one hostCell per node with CAS. The store
  // lives next to the token (same isolated dir in tests); the proxy refuses
  // /api/live-host via /node (local-only), but the /api/route path routes it
  // behind a per-peer permission denied by default (liveHostAccess).
  ...
  // (rev5): spento non crea connessioni e la Live resta standard.
  // One store instance serves both sides of a remote designation: the routes
  // (owner side) and the forwarder that records the pointer to a peer (client
  // side). Two instances over the same file would work, one is honest.
  const liveHostStore = createLiveHostStore({ filePath: liveHostPath(cfg) });
  api.use('/live-host', liveHostRoutes({
    ...
    // Client side: read the owner's state and forward the bridge resolution to
    // the node the designation points at. Owner side: accept a forwarded
    // resolution behind the same per-peer gate and the same rate limiting as
    // the other federated routes.
    federation: createLiveHostFederation({
      localNodeId: federatedNodeId,
      peers: federatedPeers,
      localPort: () => (server && server.address() ? server.address().port : cfg.port),
      localToken: () => tokenHolder.value,
    }),
    originResolver: createOriginResolver({ localNodeId: federatedNodeId, hopSecret: () => hopSecret }),
    federatedRate: createSpeakRateLimiter(),
  }));
  ...
  // /nodes (read-only, per la settings UI): stesso formato di `nodes list --json`
  ...
  api.get('/nodes', async (_req, res) => {
    try {
      ...
      const nodes = view.nodes.map((n, i) => {
        ...
        // Federated access read view: derived label,
        // configured marker and the effective grant vector. `view.nodes[i]`
        // is the redacted twin of `st.nodes[i]` (same order, redaction keeps
        // tokens out), while grants are decisions, not secrets, so they are
        // derived from the raw record and never contain a credential.
        return { ...n, tunnel: nodesHealth.tunnelFromHealth(h), health: h, ...accessPresets.accessView(st.nodes[i]) };
      });
      // Store-root revision of the peer access policy: the CAS value the UI
      // must send back with every preset write (stale revision -> 409).
      res.json({ nodeId: view.nodeId, nodes, accessRevision: nodesStore.accessRevisionOf(st) });
    } catch (e) { res.status(500).json({ error: String(e.message || e) }); }
  });
  ...
  api.get('/peers', async (_req, res) => {
    try {
      ...
      // Same access read view as /api/nodes: the settings UI consumes this
      // endpoint, so direct peers carry the derived label, the configured
      // marker and the effective grants here too (the inventory spreads the
      // record as-is). No duplication: one helper, two endpoints.
      const direct = nodesStore.redactStore(st).nodes.map(
        (n, i) => ({ ...n, ...accessPresets.accessView(st.nodes[i]) }),
      );
      const healths = await nodesHealth.nodesHealth({
        nodes: st.nodes, home: cfg.home || os.homedir(), fetchImpl: healthFetch, now: Date.now(),
        failureThreshold: cfg.nodeHealthFailureThreshold,
      ...
      res.json({ nodeId: st.nodeId, peers, accessRevision: nodesStore.accessRevisionOf(st) });
    } catch (e) { res.status(500).json({ error: String(e.message || e) }); }
  });
  // Settings API: read-only GET + a closed list of mutators for the
  // wizard/settings UI. Behind the same requireToken of the /api router; the
  // route-level READONLY gate and token redaction live inside settingsRoutes.
  ...
  api.use('/route', federation.localRouter({
    nodesPath, localPort: () => (server && server.address() ? server.address().port : cfg.port), localCredential: () => tokenHolder.value, readonly: proxyReadonly,
    hopSecret: () => hopSecret,
    // A designation forwarded from here to a peer is recorded by the answer
    // the peer gives, not by a local guess: see liveHostLedgerCapture.
    liveHostLedger: {
      recordRemote: (entry) => liveHostStore.setRemoteDesignation(entry),
      clearRemote: (ownerId) => liveHostStore.clearForOwner(ownerId),
    },
  }));
  ...
  // SSE UI events (notify/ask, MCP bridge): EventSource cannot set
  // headers -> the token is accepted in query too, ONLY because the bind is
  // loopback-only (same pattern as the /node proxy WS upgrade). Mounted
  // BEFORE the /api router (which is Bearer-only) and always on the live token.
  ...
  // Single-origin reverse proxy for /node/<name>/… . Local auth FIRST
  // before resolving <name>: requireToken(token) in front of the router, no proxy
  // route mounted before the auth middleware. Mounted BEFORE the static/catch-all.
  app.use('/node', requireToken(tokenStore), createNodeProxy({ resolveNode, readonly: proxyReadonly }));

  app.use(express.static(distDir));
  // Multi-window deck: /deck/<name> serves the SAME SPA (same origin,
  // same token via fragment). <name> is a strict client-side key, never used
  // to build paths: validation ^[a-z0-9-]{1,32}$, invalid name -> hard 404
  // (no traversal, no silent SPA fallback on dirty names).
  ...
  server.on('close', () => {
    ...
    // Nessun timer di ritentativo sopravvive alla chiusura del server.
    if (closureRetry) { try { closureRetry.stop(); } catch (_) {} }
  ...
  function routeUpgrade(req, socket, head) {
    ...
    if (pathname.startsWith('/api/route/')) {
      ...
      federation.forwardUpgrade({ req, socket, head, nodesPath, localPort: runtimePort, localCredential: () => tokenHolder.value, ingress: null, readonly: proxyReadonly, activeSockets: proxySockets, hopSecret: () => hopSecret, hubAttaches });
      return;
    }
    if (pathname.startsWith('/federation/route/')) {
      ...
      federation.forwardUpgrade({ req, socket, head, nodesPath, localPort: runtimePort, localCredential: () => tokenHolder.value, ingress, readonly: proxyReadonly, activeSockets: proxySockets, hopSecret: () => hopSecret, hubAttaches });
      return;
    }
  ...
  wss.on('connection', (ws, req) => {
    ...
    // Preauth via header (attach remoto): quando l'upgrade arriva dal proxy
    // /node/<name> di un hub, il proxy ha gia' iniettato `Authorization: Bearer
    // of THIS node> while the attach frame carries the token
  ...
  server.on('close', () => {
    ...
    // I waiter dei vl-node pendenti si risolvono SUBITO (idle): il processo
    // non resta in piedi per i boundedWait residui.
    try { vlNodeBroker.flushPendingWaiters(); } catch (_) { /* broker gia' chiuso */ }
  });
  ...
  return {
    ...
    // Test/observability seam: publish a normalized state event through the
    // same per-peer projection the local producers use.
    notify: { publishState: notifier.publishState },
  };
}
...
function start(opts = {}) {
  ...
  // Porta pannello (sicurezza 2026-08-16): scelta libera con lo STESSO
...
module.exports = { createServer, start, wsBridgeDefaults };
