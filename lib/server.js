...
function createServer(opts = {}) {
  ...
  // Audio routes (WP2): speak/stop/capability/status. Target gates ACL/consent/
  // READONLY/rate/dedup independently. consent is read from the local node store
  // (audio.consent, default false); ACL/visibility cross-node wiring and the
  // adapter are WP3. No broadcast SSE.
  api.use('/audio', audioRoutes({
    readonly: proxyReadonly,
    localNodeId: cfg.nodeId || null,
    // Verified origin only: the MCP bridge passes its real session; the server
    // trusts it ONLY if it maps to an active Fleet cell (server-side). Fail-
    // closed otherwise. NEVER trusts a client header.
    resolveCaller: (req) => {
      const session = (req.body && req.body.session) || (req.query && req.query.session);
      if (typeof session !== 'string' || !session) return null;
      try {
        const cells = (fleetP && typeof fleetP.listActive === 'function') ? fleetP.listActive() : null;
        if (!Array.isArray(cells) || !cells.some((c) => c && c.tmuxSession === session)) return null;
      } catch (_) { return null; }
      return { cell: session, node: String(cfg.nodeId || '') };
    },
    // Self-consent only: the local node's own audio store. A remote target
    // applies its own self-consent at its endpoint (via the federated route).
    targetConsent: (instanceId) => {
      if (!cfg.nodeId || instanceId !== cfg.nodeId) return false;
      try { return isAudioConsent({ home }, home); } catch (_) { return false; }
    },
  }));
