...
function createServer(opts = {}) {
  ...
  async function stopRotatableReverse(name, { remotePort = null, generation = null } = {}) {
    ...
    for (const existing of selected) {
      ...
      if (!result.stopped && !['no pidfile', 'stale (pid dead)'].includes(result.reason)) { quarantinedAny = true; continue; }
    ...
    // Successo solo se qualcosa e' stato spento E niente e' rimasto in
    // quarantena. Durante la grace di una rotazione due entry coesistono:
    // dichiarare successo perche' UNA e' stata chiusa lascerebbe l'altra viva
    // mentre chi chiama registra il canale come privato. Il caso "nessuna
    // entry" resta false come prima: non si puo' dimostrare nulla.
    return stoppedAny && !quarantinedAny;
  }
