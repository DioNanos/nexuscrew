...
function handleSpeak(input, deps) {
  ...
  // 0) text validation: required, 1..320 (MCP and REST). No persistence/logging.
  if (typeof opts.text !== 'string' || opts.text.length < 1 || opts.text.length > 320) {
  ...
  // 5) idempotency BEFORE rate/gates that mutate quota: identical retry does NOT
  //    consume rate and returns the existing receipt; collision is fail-closed.
  if (opts.utteranceId && d.findReceipt) {
    const found = d.findReceipt(opts.utteranceId, opts.origin && opts.origin.cell, target);
    if (found === 'collision') {
      // Collision is fail-closed and MUST NOT register a new receipt (it would
      // throw in the store). Return an inline redacted refusal.
      const ts = typeof d.now === 'function' ? d.now() : Date.now();
      return { status: 'refused', reason: 'utterance-collision', utteranceId: opts.utteranceId, origin: { node: opts.origin && opts.origin.node, cell: opts.origin && opts.origin.cell }, target, timestamp: ts };
    }
    if (found) return found;
  }
  // 6) rate limit (dedicated speak buckets; urgency never bypasses)
  if (d.rateLimiter) {
...
function handleStop(input, deps) {
  ...
  if (opts.utteranceId && d.findReceipt) {
    const found = d.findReceipt(opts.utteranceId, opts.origin && opts.origin.cell, target);
    if (!found || found === 'collision') return d.receipt('unknown', { reason: 'utterance-not-found', target, origin: opts.origin, utteranceId: opts.utteranceId });
  }
