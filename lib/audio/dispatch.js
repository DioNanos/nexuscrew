...
function createDispatcher(opts = {}) {
  ...
  // Gli esiti che un endpoint puo' legittimamente restituire. Parametrizzato
  // perche' l'instradamento non e' audio-specifico: le notifiche riusano lo
  // stesso percorso e rispondono `delivered`. Il default resta quello audio,
  // cosi' nessun chiamante esistente cambia comportamento.
  const statuses = opts.statuses instanceof Set ? opts.statuses : PER_ENDPOINT_STATUS;
  ...
  async function forward(resource, route, body) {
    ...
    if (!payload || typeof payload !== 'object' || !statuses.has(payload.status)) {
