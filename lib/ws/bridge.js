...
function bindWs(ws, deps) {
  ...
  function onMessage(data, isBinary) {
    ...
    // msg.count (opzionale, scroll) moltiplica le righe in un unico send-keys;
    // validato/bound lato runAction. Per le azioni di nav window/pane è undefined.
    else if (msg.type === 'action') runAction(session, msg.name, msg.count); // nav + scroll server-side
  }
