'use strict';
// Web Push for the MCP bridge. VAPID keys generated LAZY on first use
// (never at startup: tests creating the server with isolated paths must not
// write to ~/.nexuscrew) and persisted in <dir>/vapid.json 0600; subscriptions
// in <dir>/push.json 0600, dedup by endpoint. On a failed push 404/410 the
// subscription is removed (NOT in READONLY: the cleanup is a write).
// The sender is injectable (webpushImpl) so tests NEVER touch the network;
// the PRIVATE key never leaves this module.
//
// da revisione (threat model SSRF): sendNotification fa una richiesta server-side.
...
function createPushService(opts = {}) {
  ...
  // da revisione: closure iniettata dal server — in READONLY questo modulo non scrive MAI
  ...
  function loadVapid() {
    ...
    const cur = readJsonSafe(vapidPath); // Fail-closed su mode/owner/symlink
    ...
    // da revisione: la generazione e' una SCRITTURA — vietata in READONLY (503 esplicito,
  ...
  // subscription si rimuove, MA NON in READONLY (da revisione: riscrivere push.json e' una
