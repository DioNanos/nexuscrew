...
const MAX_LEN = 240;
const CONTENT_WORKING_RE = /^[\u2800-\u28ff]\s+Working(?:\.\.\.|\s*\()/iu;
...
// Fallback per client che non aggiornano l'OSC terminal title (Pi usa un
// titolo statico). Il match è ancorato alla riga di stato della TUI: non basta
// trovare parole come "Working" nel transcript, che potrebbe citarle.
function analyzeCapture(raw) {
  const lines = String(raw || '').split('\n').map(sanitizePreview);
  let preview = '';
  let status = '';
  for (let i = lines.length - 1; i >= 0; i -= 1) {
    const line = lines[i];
    if (!preview && line) preview = line;
    if (!status && CONTENT_WORKING_RE.test(line)) {
      status = line.replace(/^[\u2800-\u28ff]\s*/u, '').slice(0, 160);
    }
    if (preview && status) break;
  }
  return { preview, working: !!status, status };
}
...
function createPreviewSampler(tmuxBin, { ttlMs = 3000, timeoutMs = 1500, maxConcurrent = 4 } = {}) {
  ...
  return {
    getState,
    get: async (session) => (await getState(session))?.preview ?? null,
    close: () => cache.clear(),
  };
}

module.exports = { sanitizePreview, analyzeCapture, createPreviewSampler };
