'use strict';
// Service companion (boot) generation and migration gate.
//
...
function generateFleetLinux(opts) {
  ...
  // Parita' di hardening con service.js: reject char non gestibili in systemd.
...
// Collection (NON ingoiare activation —), execImpl iniettabile (test). Differenze
// companion:
...
function fleetFileMode(platform) {
  if (platform === 'termux') return 0o755; // boot script must stay executable (chmod 755)
...
// E' PRESERVATO e le failure raccolte per diagnosi (it is never swallowed, non si rollback).
// da revisione: `activate: false` (speculare a installService) scrive la definizione SENZA
...
function installFleetService(platform, content, ctx, {
  ...
  // Lstat: reject symlink preesistente (no-symlink atomic, parita' service.js)
  try {
  ...
  // Exec service manager — raccogli failure per diagnosi (NON ingoiare)
  const failures = [];
  if (platform === 'mac') {
...
// Stessa disciplina di service.js (da revisione): la riga identificativa e'
