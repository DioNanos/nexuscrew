...
// Dettaglio e motivo nel commento sopra il kill in killPidfile.
...
// Misurato in revisione: con lo spread di extra per ultimo,
...
// 3. da revisione: claimSchemaMarker (sotto) crea il marker al KILL, non solo
...
// da revisione interna: misurato in revisione
...
//
// da revisione interna: questa funzione NON
...
// da revisione: LA decisione di sicurezza. checkSchemaMarker (sopra) guarda
...
function killPidfile(p, signal = 'SIGTERM', impl = {}) {
  ...
  // esattamente il buco piu' largo trovato in revisione, perche'
  ...
  // riuscito in QUESTO istante", l'errore di categoria che la revisione ha
  ...
  if (meta.processStart) {
    ...
    // a se', con un suo giro di revisione dedicato (non eseguito).
    ...
    // attestare" sarebbe l'errore di categoria contestato in revisione:
    ...
    //
    // da revisione: la domanda che decide non e' piu' "il marker e' gia'
    ...
    //
    // MISURATO IN REVISIONE: trattare OGNI meta senza attestazione come
    // indeterminate rompe l'aggiornamento automatico per ogni nodo il cui
    // runtime e' ancora precedente a quando processStart e' nato (introdotto in
    ...
    // corruzione e downgrade — la revisione l'ha contestata); un marker
    // controllato ma mai riprovato al kill (da revisione:
    // una scrittura fallita in silenzio riapre
