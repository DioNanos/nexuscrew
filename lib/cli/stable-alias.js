'use strict';
// Alias stabili per i path del servizio di boot.
//
...
//
// Tre vincoli non negoziabili (briefing):
...
// L'attuale vive (intatto), altrimenti il primo candidato stabile che
...
function resolveLiveBootPaths({ nodeBin, entryPath, env = process.env, realpath, exists } = {}) {
  ...
  const revive = (current, candidates, what) => {
    try {
      ...
      return null; // Vivo: decide il criterio di resolveBootPaths
