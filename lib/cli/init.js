'use strict';
// Nexuscrew init: orchestrazione setup portatile.
// detectPlatform -> prereq (Node>=18 abort, tmux abort-before-service) ->
// migration rule (porta da service esistente) -> config.json (preserva) ->
// token (preserva) -> NexusFiles -> generateService -> installService (skip dry-run) ->
// fleet companion (only when the provider is builtin and the gate passes;
//   never fails the main init) -> print URL #token. Termux boot: best-effort.
const fs = require('node:fs');
const os = require('node:os');
const path = require('node:path');
const crypto = require('node:crypto');
const { detectPlatform, nodeBin, repoRoot, uid } = require('./platform.js');
const { loadOrCreateToken } = require('../auth/token.js');
const { generateService, installService, fileMode, installPath: svcInstallPath } = require('./service.js');
// Fleet boot companion. Injectable seams for tests.
const {
  generateFleetService, installFleetService, migrationGate,
  selectProviderModeSync, fleetFileMode,
} = require('./fleet-service.js');
const { resolveBootPaths } = require('./stable-alias.js');
const { aggiornaDefinizioni } = require('../fleet/definitions.js');
const { defaultDefinitions } = require('../fleet/managed.js');
const { commandExists, resolveCommand } = require('./path.js');

function haveTmux(tmuxBin, env = process.env) {
  return commandExists(tmuxBin, env);
}

// Messaggio per l'utente quando tmux risulta assente. SOLO quando siamo NOI
// a fare la verifica (nessun tmuxOk imposto dal chiamante) proviamo ad
// arricchirlo: se resolveCommand trova entry del PATH dove non e' stato
// possibile guardare (permessi, symlink rotto, directory non
// attraversabile), il messaggio deve dire "non verificabile", non "non
// trovato" — altrimenti manda l'operatore a installare un pacchetto che
// magari e' gia' li'.
function tmuxUnavailableMessage(tmuxBin, env) {
  const r = resolveCommand(tmuxBin || 'tmux', env);
  if (r.blocked && r.blocked.length) {
    const detail = r.blocked.map((b) => `${b.path} (${b.code})`).join('; ');
    return `tmux non verificabile su PATH, non "non trovato" (${detail})`;
  }
  return 'tmux non trovato su PATH (installa tmux)';
}

function nodeMajor() {
  return parseInt(String(process.versions.node).split('.')[0], 10);
}

function writeConfigAtomic(configPath, value) {
  try {
    if (fs.lstatSync(configPath).isSymbolicLink()) throw new Error('refusing symlink config target');
  } catch (e) {
    if (e.code !== 'ENOENT') throw e;
  }
  fs.mkdirSync(path.dirname(configPath), { recursive: true });
  const tmp = path.join(path.dirname(configPath), `.${path.basename(configPath)}.${crypto.randomBytes(6).toString('hex')}.tmp`);
  try {
    fs.writeFileSync(tmp, `${JSON.stringify(value, null, 2)}\n`, { mode: 0o600 });
    fs.chmodSync(tmp, 0o600);
    fs.renameSync(tmp, configPath);
  } catch (e) {
    try { fs.unlinkSync(tmp); } catch (_) {}
    throw e;
  }
}

// Bootstrap/migration idempotente degli artefatti Fleet. Un file presente non
// viene MAI riscritto qui: se e' invalido, il provider e doctor devono restare
// fail-closed e mostrarne la causa. Questo helper copre anche installazioni
// parziali in cui config+token esistono ma fleet.json non fu mai creato.
function ensureFleetDefaults(opts = {}) {
  const home = opts.home || os.homedir();
  const configDir = opts.configDir || path.join(home, '.nexuscrew');
  const fleetDefsPath = opts.fleetDefsPath || path.join(configDir, 'fleet.json');
  const enabled = opts.fleetEnabled !== false
    && opts.builtinEnabled !== false
    && process.env.NEXUSCREW_FLEET !== '0';
  if (!enabled) return { path: fleetDefsPath, created: false, enabled: false };

  try {
    fs.lstatSync(fleetDefsPath);
    return { path: fleetDefsPath, created: false, enabled: true };
  } catch (e) {
    if (e.code !== 'ENOENT') throw e;
  }

  // Fra il controllo qui sopra e la scrittura c'e' una finestra: se un altro
  // processo crea le definizioni nel frattempo, scrivere i default sopra le sue
  // significa cancellare una configurazione intera — non perdere una modifica.
  // La creazione avviene quindi sotto lock, con l'esistenza RIVERIFICATA
  // dentro: `seMancante` viene chiamata solo se il file davvero non c'e'.
  // `propaga` perche' `created` dica il vero: senza, una rinuncia del lock
  // restituiva lo stato riletto — truthy — e l'init dichiarava `created: true`
  // per un file che non aveva creato.
  let creato = null;
  try {
    creato = aggiornaDefinizioni(fleetDefsPath, () => null, {
      seMancante: () => defaultDefinitions(),
      propaga: true,
      log: opts.log,
    });
  } catch (_) { creato = null; }
  return { path: fleetDefsPath, created: Boolean(creato), enabled: true };
}

// Migration rule: se non c'è config.json, parse la porta dal service file esistente.
function readExistingPort(platform, home, installPathOverride) {
  const p = installPathOverride || svcInstallPath(platform, home);
  let raw;
  try { raw = fs.readFileSync(p, 'utf8'); } catch (_) { return null; }
  if (platform === 'linux') {
    const m = raw.match(/Environment=NEXUSCREW_PORT=(\d+)/);
    return m ? Number(m[1]) : null;
  }
  if (platform === 'mac') {
    const m = raw.match(/<key>NEXUSCREW_PORT<\/key>\s*<string>(\d+)/);
    return m ? Number(m[1]) : null;
  }
  if (platform === 'termux') {
    const m = raw.match(/export NEXUSCREW_PORT=(\d+)/);
    return m ? Number(m[1]) : null;
  }
  return null;
}

function runInit(opts = {}) {
  const platform = opts.platform || detectPlatform();
  const home = opts.home || os.homedir();
  const configDir = opts.configDir || path.join(home, '.nexuscrew');
  const configPath = opts.configPath || path.join(configDir, 'config.json');
  const tokenPath = opts.tokenPath || path.join(configDir, 'token');
  const filesRoot = opts.filesRoot || path.join(home, 'NexusFiles');
  const dryRun = !!opts.dryRun;
  // Write the boot definitions without asking the service manager to load them.
  // Needed by smoke runs and temporary prefixes, where the per-UID manager would
  // otherwise act on the real installation (see planActivation in service.js).
  const noActivate = !!opts.noActivate || opts.activate === false;
  const installBoot = opts.installBoot !== false;
  const log = opts.log || (() => {});
  const tmuxOk = opts.tmuxOk !== undefined ? opts.tmuxOk : haveTmux(opts.tmuxBin || 'tmux', opts.env || process.env);

  // Prereq Node (abort before any write)
  if (nodeMajor() < 18) {
    throw new Error(`Node >= 18 richiesto (attuale ${process.versions.node}). Aggiorna Node prima di init.`);
  }

  const actions = [];
  // Structured, value-free install outcomes for callers that must fail closed
  // (notably smart-up migrations). Human-readable actions remain compatible.
  const installFailures = [];
  // Outcome of the boot-definition activation: 'activated', 'skipped-by-flag'
  // (--no-activate) or 'skipped-foreign-manager' (the manager loads another
  // definition). Declared to callers that must not confuse a deliberate skip
  // with a fresh activation.
  let serviceActivation = null;

  // porta: opts.port > migration rule > config esistente > default 41820
  let port = opts.port;
  if (!port) {
    const migrated = readExistingPort(platform, home, opts.installPath);
    if (migrated) { port = migrated; actions.push(`migration: porta ${port} letta dal service esistente`); }
  }

  // Config.json (scrivi se non esiste; preserva se esiste)
  if (!dryRun) {
    fs.mkdirSync(configDir, { recursive: true });
    if (!fs.existsSync(configPath)) {
      if (!port) port = 41820;
      writeConfigAtomic(configPath, { port });
      actions.push(`created config ${configPath} (port ${port})`);
    } else {
      let current;
      try {
        current = JSON.parse(fs.readFileSync(configPath, 'utf8'));
      } catch (_) { current = {}; }
      if (opts.port) {
        current.port = opts.port;
        port = opts.port;
        writeConfigAtomic(configPath, current);
        actions.push(`updated config ${configPath} (port ${port})`);
      } else if (current && current.port) port = current.port;
      actions.push(`preserved config ${configPath} (port ${port})`);
    }
  }

  // Fleet app defaults: soltanto i quattro client nativi. Provider cloud/Z.AI sono
  // disponibili nel catalogo managed ma vanno aggiunti esplicitamente.
  const fleetDefsPath = opts.fleetDefsPath || path.join(configDir, 'fleet.json');
  if (!dryRun) {
    try {
      const fleetBootstrap = ensureFleetDefaults({ ...opts, home, configDir, fleetDefsPath });
      if (!fleetBootstrap.enabled) actions.push('fleet defaults: disabilitati intenzionalmente');
      else if (fleetBootstrap.created) actions.push(`created fleet defaults ${fleetDefsPath} (claude.native, codex.native, codex-vl.native, pi.native)`);
      else actions.push(`preserved fleet definitions ${fleetDefsPath}`);
    } catch (e) {
      actions.push(`WARN: fleet defaults non creati: ${e.message}`);
    }
  }
  if (!port) port = 41820;

  // Token (preserva esistente)
  let token;
  if (!dryRun) {
    token = loadOrCreateToken(tokenPath);
    actions.push(`token ok (${tokenPath})`);
  } else {
    token = null;
  }

  // NexusFiles dir
  if (!dryRun) {
    fs.mkdirSync(filesRoot, { recursive: true });
    actions.push(`files root ${filesRoot}`);
  }

  // Nodes.json: nodeId STABILE per installazione (generato qui se manca) +
  // migrazione esplicita guarded dei nodes legacy da config.json. READONLY/dry-run
  // saltano; ogni errore -> WARN, MAI far fallire l'init.
  if (!dryRun && process.env.NEXUSCREW_READONLY !== '1' && !opts.readonly) {
    try {
      const nstore = require('../nodes/store.js');
      const nodesPath = opts.nodesPath || path.join(configDir, 'nodes.json');
      const st = nstore.initStore(nodesPath);
      actions.push(`nodes.json ok (${nodesPath}, nodeId ${st.nodeId.slice(0, 8)}…)`);
      const mig = nstore.migrateLegacyNodes(configPath, nodesPath);
      if (mig.migrated) actions.push(`nodes: migrati ${mig.count} nodi legacy da config.json`);
    } catch (e) {
      actions.push(`WARN: nodes.json init/migrazione fallita: ${e.message} (init prosegue)`);
    }
    try {
      const dstore = require('../decks/store.js');
      const decksPath = opts.decksPath || path.join(configDir, 'decks.json');
      dstore.initStore(decksPath);
      actions.push(`decks.json ok (${decksPath})`);
    } catch (e) {
      actions.push(`WARN: decks.json init fallita: ${e.message} (init prosegue)`);
    }
  }

  // service generation
  const svcCtx = {
    repoRoot: repoRoot(),
    nodeBin: nodeBin(),
    port, home, uid: uid(),
    installPath: opts.installPath,
  };
  // 1-bis: la STESSA classe di difetto del companion di boot — nodeBin
  // e entry finiscono versionati nell'unit/plist del servizio per-nodo. Si
  // risolve qui, alla scrittura, e si dichiara cio' che non ha alias stabile:
  // i due punti di scrittura non devono divergere (correggerne uno solo
  // lascerebbe il difetto vivo).
  const svcBoot = resolveBootPaths({
    nodeBin: svcCtx.nodeBin,
    entryPath: path.join(svcCtx.repoRoot, 'bin', 'nexuscrew.js'),
  });
  svcCtx.nodeBin = svcBoot.nodeBin;
  svcCtx.entryPath = svcBoot.entryPath;
  const content = generateService(platform, svcCtx);

  if (!installBoot) {
    actions.push(`boot opt-in: service ${platform} non installato`);
  } else if (dryRun) {
    actions.push(`DRY-RUN service (${platform}) generato, NON installato`);
  } else if (!tmuxOk) {
    // Tmux mancante: abort before service install (config/token gia' creati)
    // Se il chiamante ha imposto tmuxOk direttamente (test/seam), non
    // abbiamo l'informazione per distinguere assenza da verifica bloccata:
    // il messaggio resta quello storico.
    const reason = opts.tmuxOk === undefined
      ? tmuxUnavailableMessage(opts.tmuxBin || 'tmux', opts.env || process.env)
      : 'tmux non trovato su PATH (installa tmux)';
    actions.push(`WARN: ${reason} -> service NON installato (ri-runna init)`);
  } else {
    for (const w of svcBoot.warnings) actions.push(`WARN service: ${w}`);
    try {
      const r = installService(platform, content, svcCtx, {
        execImpl: opts.execImpl,
        activate: !noActivate,
      });
      serviceActivation = r.activation || null;
      if (r.activation && r.activation.warning) {
        const skip = r.activation.outcome === 'skipped-by-flag'
          ? r.activation.warning
          : `WARN: ${r.activation.warning}`;
        actions.push(skip);
      }
      if (r.failures && r.failures.length) {
        // File installato MA activation (systemctl/launchctl) fallita
        installFailures.push({ component: 'service', phase: 'activation' });
        actions.push(`WARN: service file installato ${r.target} MA activation fallita: ${r.failures.map((f) => f.cmd).join('; ')} (file preservato, diagnosi)`);
      } else {
        actions.push(`service installed ${r.target} (mode 0${fileMode(platform).toString(8)})`);
      }
    } catch (e) {
      // Failure: preserve file + diagnosi (no rollback)
      installFailures.push({ component: 'service', phase: 'install' });
      actions.push(`WARN: service install fallito: ${e.message} (file generati preservati)`);
    }
  }

  // --- Fleet boot companion service ---
  // The companion installs ONLY when the provider is builtin and the migration
  // gate finds no enabled legacy cloud-cell@*.service units (no double boot).
  // READONLY blocks every companion action. Never let it fail the main init:
  //   any companion error -> WARN action + normal return.
  try {
    const readonly = process.env.NEXUSCREW_READONLY === '1' || opts.readonly;
    if (!installBoot) {
      actions.push('fleet companion: boot opt-in, non installato');
    } else if (readonly) {
      actions.push('fleet companion: READONLY, non installato');
    } else {
      // provider mode: companion ONLY when builtin. selectProviderModeSync is
      // the synchronous default (runInit is sync: the bin calls process.exit after
      // dispatch, an async selectProvider would not survive). Seam opts.selectProvider for tests.
      const selectProvider = opts.selectProvider || selectProviderModeSync;
      const fleetCfg = opts.fleetCfg || {
        home,
        fleetEnabled: opts.fleetEnabled !== undefined ? opts.fleetEnabled : true,
        builtinEnabled: opts.builtinEnabled,
        fleetDefsPath,
      };
      const sel = selectProvider(fleetCfg) || {};
      if (sel.mode !== 'builtin') {
        actions.push(`fleet companion: non installato (provider ${sel.mode})`);
      } else {
        // migration gate: seam opts.migrationGate injectable; the fleet-service.js
        // one (injectable exec via opts.execImpl). Blocked -> do NOT
        // install (never a silent double boot).
        const gate = (opts.migrationGate || migrationGate)({ exec: opts.execImpl, platform });
        if (gate.blocked) {
          actions.push(`WARN: fleet companion NON installato — migration gate bloccato: ${(gate.units || []).join(', ')} (${gate.remediation || 'disabilita le unit legacy cloud-cell@*.service'})`);
        } else if (dryRun) {
          actions.push('DRY-RUN fleet companion generato, NON installato');
        } else {
          // I path del boot si risolvono alla SCRITTURA del servizio (mai
          // a ogni avvio): un alias stabile che risolva allo stesso file, se
          // c'e'; altrimenti il path attuale, DICHIARATO — un avviso adesso e'
          // meglio di un path che scade in silenzio (vedi stable-alias.js).
          const bootEntry = path.join(svcCtx.repoRoot, 'bin', 'nexuscrew.js');
          const boot = resolveBootPaths({ nodeBin: svcCtx.nodeBin, entryPath: bootEntry });
          for (const w of boot.warnings) actions.push(`WARN fleet companion: ${w}`);
          const fleetContent = generateFleetService({
            platform,
            nodeBin: boot.nodeBin,
            entryPath: boot.entryPath,
            repoRoot: svcCtx.repoRoot,
            home,
          });
          const fr = installFleetService(
            platform, fleetContent,
            { home, uid: uid(), installPath: opts.fleetInstallPath },
            { execImpl: opts.execImpl, activate: !noActivate },
          );
          if (fr.skippedActivation) {
            actions.push(`fleet companion ${fr.target}: activation skipped (--no-activate)`);
          } else if (fr.failures && fr.failures.length) {
            // File installato MA activation (systemctl/launchctl) fallita
            installFailures.push({ component: 'fleet-companion', phase: 'activation' });
            actions.push(`WARN: fleet companion file installato ${fr.target} MA activation fallita: ${fr.failures.map((f) => f.cmd).join('; ')} (file preservato, diagnosi)`);
          } else {
            actions.push(`fleet companion installed ${fr.target} (mode 0${fleetFileMode(platform).toString(8)})`);
          }
        }
      }
    }
  } catch (e) {
    // qualunque errore del companion: WARN + init principale prosegue intatto.
    installFailures.push({ component: 'fleet-companion', phase: 'install' });
    actions.push(`WARN: fleet companion fallito: ${e.message} (init principale prosegue)`);
  }

  // Termux:boot best-effort detection
  if (platform === 'termux' && installBoot) {
    const bootDir = path.join(home, '.termux', 'boot');
    const bootOk = fs.existsSync(bootDir);
    actions.push(bootOk
      ? `Termux:boot dir presente — verifica app Termux:Boot installata/abilitata per l'avvio al reboot`
      : `Termux:boot dir MANCANTE — installa/apri l'app Termux:Boot una volta per l'avvio automatico`);
  }

  const url = `http://127.0.0.1:${port}/`;
  const urlWithToken = token ? `${url}#token=${token}` : url;
  actions.push(`platform: ${platform}`);
  // printUrl:false -> non stampare l'URL col token (usato da smart-up, che presenta
  // URL base + QR da se': cosi' l'output di smart-up non contiene il token in chiaro).
  if (opts.printUrl !== false) actions.push(`URL: ${urlWithToken}`);

  for (const a of actions) log(a);

  return {
    platform, port, token, url: urlWithToken, actions, installFailures,
    serviceActivation, tmuxOk, dryRun,
  };
}

module.exports = { runInit, readExistingPort, haveTmux, nodeMajor, writeConfigAtomic, ensureFleetDefaults };
