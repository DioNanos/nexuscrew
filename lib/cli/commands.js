...
function autoUpdateCommand(args, opts = {}) {
  ...
  if (!attivo) {
    ...
    fs.writeFileSync(configPath, `${JSON.stringify({ ...cfg, autoUpdate: acceso }, null, 2)}\n`, { mode: 0o600 });
...
function dispatch(argv, opts = {}) {
  ...
  if (cmd === 'restart') {
    ...
    return (opts.waitForRuntimeImpl || waitForNexusCrew)(port, token, {
      ...
      // Il messaggio dice DOVE guardare, perche' «riavvio fallito» da solo
      // manda a cercare nel posto sbagliato: il tunnel puo' essere su e il
      // servizio giu', e sono due guasti con due rimedi diversi.
      log(`restart: il servizio NON risponde su 127.0.0.1:${port} dopo il riavvio.`);
      log('  Il comando di riavvio e\' partito, il processo non e\' tornato su.');
