'use strict';
// Minimal public CLI plus internal service/MCP entry points.
...
Usage:
  ...
  nexuscrew init [--dry-run] [--port PORT] [--no-activate]
                     initialize missing stores idempotently
...
Usage:
  ...
  nexuscrew nodes edit <name|nodeId> --access-role admin|user|nexushost
  nexuscrew nodes edit <name|nodeId> --events-access on|off [...grant flags]
  nexuscrew nodes edit <name|nodeId> --events-receive on|off
  nexuscrew nodes access [--json]
...
const CLI_VALUE_FLAGS = new Set([
  ...
  'name', 'local-name', 'local-label', 'identity-file', 'port', 'dir', 'events-receive',
]);
...
// Ruoli client/node dal config.json (default entrambi off — li popola dal wizard UI).
...
// Rilievo della revisione: il caso token-assente era gia' coperto, questo no.
...
// Token rotation: atomic write of the new token plus real invalidation of the
// active sessions (service restart when running). The new token is never printed.
...
function autoUpdateCommand(args, opts = {}) {
  ...
  if (!attivo) {
    ...
    // proprio il guasto che questo comando serve a evitare. Rilievo della revisione.
    try {
...
// rimedio da revisione, difetto 1: la via manuale attraversa la STESSA
...
// Fleet boot companion: starts the boot:true cells of the selected provider.
...
async function runFleetBoot(opts = {}) {
  ...
  // collected in failed[] -> exit 1 (no special short-circuit by design).
...
async function dispatchNodes(rest, flags, opts = {}) {
  ...
  if (sub === 'inspect' || sub === 'show') {
    const feedDiagnostics = await nodesCmds.fetchFeedDiagnostics(opts);
    return { code: (await nodesCmds.nodesInspect({ ...opts, log, ref, json: flags.json === true, feedDiagnostics })).code };
  }
  if (sub === 'doctor') {
    ...
    const feedDiagnostics = await nodesCmds.fetchFeedDiagnostics(opts);
    if (ref) {
      const inspected = await nodesCmds.nodesInspect({ ...opts, log: () => {}, ref });
    ...
    if (flags.json === true) log(JSON.stringify({ nodeId: view.nodeId, checks }, null, 2));
      ...
      log(`${check.name}: ${check.ok ? 'OK' : 'KO'} — ${check.result}${check.detail ? ` · ${check.detail}` : ''}`);
      // Feed section per direct peer: kill-switch, cursor, last failure,
      // counters for observability. Read-only, no secrets.
      const peer = peers.find((p) => (p.nodeId || null) === (check.nodeId || null) && p.name === check.name);
      if (peer && peer.kind === 'direct') {
        for (const line of nodesCmds.feedLinesFor(feedDiagnostics, peer.nodeId)) log(`  ${line}`);
      }
    }
  ...
  if (sub === 'edit') {
    ...
    // Federated access: a preset sets the whole vector, the granular flags
    // refine one that was already set explicitly.
    if (flags['access-role'] !== undefined) patch.accessRole = String(flags['access-role']);
    for (const [flag, key] of [
      ['events-access', 'eventsAccess'],
      ['node-events-access', 'nodeEventsAccess'],
      ['ask-reply-access', 'askReplyAccess'],
      ['files-read-access', 'filesReadAccess'],
      ['peer-operator-access', 'peerOperatorAccess'],
    ]) {
      if (flags[flag] === undefined) continue;
      try { patch[key] = boolFlag(flags[flag], `--${flag}`); }
      catch (e) { log(e.message); return { code: 1 }; }
    }
    // The client's OWN choice to import a peer's feed (never an owner grant):
    // on|off, same shape as the other boolean switches.
    if (flags['events-receive'] !== undefined) {
      try { patch.eventsReceive = boolFlag(flags['events-receive'], '--events-receive'); }
      catch (e) { log(e.message); return { code: 1 }; }
    }
    const expectedRevision = flags['access-revision'] === undefined ? undefined : Number(flags['access-revision']);
    if (expectedRevision !== undefined && !Number.isInteger(expectedRevision)) {
      log('nodes edit: --access-revision vuole un intero (la revision letta da `nodes access`)');
      return { code: 1 };
    }
    return { code: nodesCmds.nodesEdit({ ...opts, log, ref, patch, expectedRevision }).code };
  }
  if (sub === 'rename') {
  ...
  // Accesso ai pannelli (da revisione): dietro un pannello c'e' un browser con sessioni
  ...
  // Inventario read-only delle concessioni: chi puo' fare cosa su QUESTO nodo,
  // con la revisione della policy. E' il punto di partenza della migrazione
  // esplicita: i peer non configurati restano negati finche' non lo sono.
  if (sub === 'access') {
    return { code: nodesCmds.nodesAccess({ ...opts, log, json: flags.json === true }).code };
  }
  if (sub === 'remove') {
...
function dispatch(argv, opts = {}) {
  ...
  // Help-as-flag handling come flag, deciso qui e solo qui: dopo parseFlags `--help`
  ...
  if (cmd === 'init') {
    ...
    if (port !== undefined && (!Number.isInteger(port) || port < 1 || port > 65535)) {
      log('usage: nexuscrew init [--dry-run] [--port PORT] [--no-activate]');
      return { code: 1 };
    }
    // --no-activate writes the boot definitions without asking the service
    // manager to load them: the per-UID manager belongs to the logged-in user,
    // not to the HOME being initialized.
    (opts.runInitImpl || runInit)({
      ...opts,
      dryRun: flags['dry-run'] === true,
      port,
      noActivate: flags['no-activate'] === true,
      log,
    });
    return { code: 0 };
  }
  ...
  if (cmd === 'restart') {
    ...
    // il problema non e'. Rilievo della revisione. Il riavvio e' comunque partito,
    ...
    const continua = () => {
      ...
      //
      // Invariante: il ramo managed non chiama `startPortable`; un fallback
      // locale creerebbe un secondo processo fuori dal controllo del gestore.
  ...
  if (cmd === 'identity') {
    const sub = rest[1];
    if (sub !== 'provision') {
      log('usage: nexuscrew identity provision [--force] [--no-write-config] [--dir <path>]');
      return { code: sub === undefined ? 0 : 1 };
    }
    const provision = require('../fleet/identity-provision.js').provisionIdentityAuthority;
    const home = opts.home || require('node:os').homedir();
    const result = provision({
      home,
      dir: flags.dir,
      force: flags.force === true,
      writeConfig: flags['no-write-config'] !== true,
    });
    if (!result.ok) {
      log(`identity provision: ${result.message}`);
      log(`  existing credential files:`);
      for (const filePath of result.existing) log(`    ${filePath}`);
      return { code: 1 };
    }
    log(`identity provision: credentials written under ${result.dir} (0600, dir 0700)`);
    log('identity provision: fleet.identity.mode = "authority"');
    if (result.configUpdated) {
      log(`identity provision: config updated (${result.mode})`);
    } else {
      log('identity provision: config NOT written (--no-write-config). Apply manually:');
      log('  fleet.identity.mode = "authority"');
      log(`  keep both credential files in place under ${result.dir}`);
    }
    log('identity provision: restart the NexusCrew service to activate authority mode');
    return { code: 0 };
  }
  if (cmd === 'doctor') {
