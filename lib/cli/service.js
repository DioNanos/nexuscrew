'use strict';
// Service generation per-platform: systemd --user (linux), launchd plist (mac),
// Termux:boot script. Escaping per-platform + install no-symlink atomic.
const fs = require('node:fs');
const os = require('node:os');
const path = require('node:path');
const crypto = require('node:crypto');
const { execFileSync } = require('node:child_process');
const { detectPlatform, uid } = require('./platform.js');

const LAUNCHD_SLEEP_BUFFER = new Int32Array(new SharedArrayBuffer(4));
const DEFAULT_LAUNCHD_WAIT_ATTEMPTS = 100;
const DEFAULT_LAUNCHD_WAIT_MS = 50;

// --- Escaping helpers ---

// systemd WorkingDirectory: escape % (percent specifier). Spazi ok.
function escapeSystemdPath(s) {
  return String(s).replace(/%/g, '%%');
}

// systemd ExecStart arg: escape backslash, %, spazio.
function escapeSystemdExec(s) {
  return String(s)
    .replace(/\\/g, '\\\\')
    .replace(/%/g, '%%')
    .replace(/ /g, '\\ ');
}

// launchd plist XML escape.
function escapeXml(s) {
  return String(s)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;');
}

// POSIX shell single-quote.
function shellQuote(s) {
  return "'" + String(s).replace(/'/g, "'\\''") + "'";
}

// systemd ExecStart non gestisce bene alcuni char (", $, ;, backtick, newline, apice)
// Nemmeno con escaping ad hoc: reject esplicito con errore chiaro.
// space e % sono ok (escaped). Path repo normali non hanno questi char.
const SYSTEMD_FORBIDDEN = /[";$`\n']/;
function assertSystemdSafe(label, p) {
  if (SYSTEMD_FORBIDDEN.test(String(p))) {
    throw new Error(`${label} path contiene caratteri non supportati in systemd (", $, ;, backtick, newline, apice): "${p}". Usa un path senza questi caratteri (spazi e % sono ok).`);
  }
}

// --- Templates ---

// Entry del servizio: di norma derivato da repoRoot; i punti di scrittura
// Che risolvono l'alias stabile (/1-bis, stable-alias.js) passano
// l'entryPath gia' risolto. Contratto vecchio intatto: senza entryPath si
// deriva da repoRoot come prima.
function serviceEntryPath(ctx) {
  return ctx.entryPath || path.join(ctx.repoRoot, 'bin', 'nexuscrew.js');
}

function generateService(platform, ctx) {
  if (platform === 'linux') return generateLinux(ctx);
  if (platform === 'mac') return generateMac(ctx);
  if (platform === 'termux') return generateTermux(ctx);
  throw new Error(`unsupported platform: ${platform}`);
}

function generateLinux(ctx) {
  assertSystemdSafe('repoRoot', ctx.repoRoot); // Reject char non gestibili
  assertSystemdSafe('nodeBin', ctx.nodeBin);
  // HOME is stable across init/repair/update. The runtime directory may be
  // atomically replaced; keeping it as the service cwd leaves the HTTP process
  // and a shared tmux server holding an orphaned inode, so every later pane can
  // fail getcwd(3). Runtime state still lives in runtimeDir; only cwd changes.
  assertSystemdSafe('home', ctx.home);
  const runtime = escapeSystemdPath(ctx.home);
  const node = escapeSystemdExec(ctx.nodeBin);
  const repoBin = escapeSystemdExec(serviceEntryPath(ctx));
  const nodeDir = escapeSystemdPath(path.dirname(ctx.nodeBin));
  return `# NexusCrew service (systemd --user, loopback, solo tunnel SSH/VPN)
[Unit]
${LINUX_SERVICE_DESCRIPTION}
Wants=network-online.target
After=network-online.target

[Service]
Type=simple
# The HTTP service may be the process that creates the shared tmux server.
# Restart only NexusCrew itself; tmux sessions are independent user workloads.
KillMode=process
WorkingDirectory=${runtime}
Environment=PATH=${nodeDir}:/usr/local/bin:/usr/bin:/bin
ExecStart=${node} ${repoBin} serve
Restart=on-failure
RestartSec=3

[Install]
WantedBy=default.target
`;
}

function generateMac(ctx) {
  const nodeXml = escapeXml(ctx.nodeBin);
  const repoBinXml = escapeXml(serviceEntryPath(ctx));
  // HOME remains stable when init or repair atomically replaces the runtime
  // directory; launchd must not retain an unlinked runtime-directory cwd.
  const runtimeXml = escapeXml(ctx.home);
  const homeXml = escapeXml(ctx.home);
  const launchPath = [...new Set([
    path.dirname(ctx.nodeBin), '/opt/homebrew/bin', '/usr/local/bin', '/usr/bin', '/bin',
  ])].join(':');
  const launchPathXml = escapeXml(launchPath);
  return `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>Label</key>
  <string>com.mmmbuto.nexuscrew</string>
  <key>ProgramArguments</key>
  <array>
    <string>${nodeXml}</string>
    <string>${repoBinXml}</string>
    <string>serve</string>
  </array>
  <key>WorkingDirectory</key>
  <string>${runtimeXml}</string>
  <key>EnvironmentVariables</key>
  <dict>
    <key>PATH</key>
    <string>${launchPathXml}</string>
    <key>LANG</key>
    <string>en_US.UTF-8</string>
    <key>LC_CTYPE</key>
    <string>UTF-8</string>
  </dict>
  <key>RunAtLoad</key>
  <true/>
  <key>KeepAlive</key>
  <dict>
    <key>SuccessfulExit</key>
    <false/>
  </dict>
  <key>StandardOutPath</key>
  <string>${homeXml}/.nexuscrew/nexuscrew.log</string>
  <key>StandardErrorPath</key>
  <string>${homeXml}/.nexuscrew/nexuscrew.log</string>
</dict>
</plist>
`;
}

function generateTermux(ctx) {
  const nodeQ = shellQuote(ctx.nodeBin);
  const repoBinQ = shellQuote(serviceEntryPath(ctx));
  return `#!/data/data/com.termux/files/usr/bin/sh
${TERMUX_SERVICE_HEADER}
export PATH=/data/data/com.termux/files/usr/bin:$PATH
export HOME=/data/data/com.termux/files/home
export PREFIX=/data/data/com.termux/files/usr
export TMPDIR=$PREFIX/tmp
export TMUX_TMPDIR=$PREFIX/var/run
export LANG=\${LANG:-en_US.UTF-8}
export LC_CTYPE=\${LC_CTYPE:-en_US.UTF-8}
termux-wake-lock 2>/dev/null || true
mkdir -p "$HOME/.nexuscrew" "$TMPDIR" "$TMUX_TMPDIR"
cd -- "$HOME"
exec ${nodeQ} ${repoBinQ} serve --pidfile >> "$HOME/.nexuscrew/nexuscrew.log" 2>&1
`;
}

// --- Install paths ---

function installPath(platform, home) {
  if (platform === 'linux') return path.join(home, '.config', 'systemd', 'user', 'nexuscrew.service');
  if (platform === 'mac') return path.join(home, 'Library', 'LaunchAgents', 'com.mmmbuto.nexuscrew.plist');
  if (platform === 'termux') return path.join(home, '.termux', 'boot', 'nexuscrew.sh');
  throw new Error(`unsupported platform: ${platform}`);
}

function fileMode(platform) {
  if (platform === 'termux') return 0o700;
  return 0o644; // systemd unit + launchd plist
}

const LINUX_TMUX_SURVIVAL_DROPIN = `[Service]\n# NexusCrew owns the HTTP process, not the shared tmux server it may start.\nKillMode=process\n`;

function linuxTmuxSurvivalPath(home) {
  return path.join(home, '.config', 'systemd', 'user', 'nexuscrew.service.d', '10-tmux-survival.conf');
}

// Upgrade existing Linux installations before stop/restart.  Merely updating
// the npm package does not rewrite an already-installed unit, so the new CLI
// installs a narrow drop-in and reloads systemd before any destructive service
// lifecycle operation.  The target is atomic and never follows a symlink.
function ensureLinuxTmuxSurvival(opts = {}) {
  const home = opts.home || os.homedir();
  const target = opts.dropInPath || linuxTmuxSurvivalPath(home);
  const execImpl = opts.execImpl || execFileSync;
  let current = '';
  try {
    const st = fs.lstatSync(target);
    if (st.isSymbolicLink() || !st.isFile()) throw new Error(`refusing unsafe systemd drop-in: ${target}`);
    current = fs.readFileSync(target, 'utf8');
  } catch (error) {
    if (error.code !== 'ENOENT') throw error;
  }

  let written = false;
  if (current !== LINUX_TMUX_SURVIVAL_DROPIN) {
    fs.mkdirSync(path.dirname(target), { recursive: true, mode: 0o700 });
    const tmp = `${target}.tmp.${process.pid}.${crypto.randomBytes(6).toString('hex')}`;
    try {
      fs.writeFileSync(tmp, LINUX_TMUX_SURVIVAL_DROPIN, { mode: 0o644, flag: 'wx' });
      fs.chmodSync(tmp, 0o644);
      fs.renameSync(tmp, target);
      written = true;
    } catch (error) {
      try { fs.unlinkSync(tmp); } catch (_) {}
      throw error;
    }
  }
  execImpl('systemctl', ['--user', 'daemon-reload'], { stdio: 'ignore' });
  const effective = String(execImpl('systemctl', [
    '--user', 'show', 'nexuscrew.service', '--property=KillMode', '--value',
  ], { encoding: 'utf8' }) || '').trim();
  if (effective !== 'process') {
    throw new Error(`unsafe effective systemd KillMode=${effective || 'unknown'}; refusing NexusCrew lifecycle operation`);
  }
  return { target, written, killMode: effective };
}

function sleepSync(ms) {
  if (ms > 0) Atomics.wait(LAUNCHD_SLEEP_BUFFER, 0, 0, ms);
}

function normalizedWaitAttempts(value) {
  const parsed = Number(value);
  return Number.isFinite(parsed) && parsed >= 1
    ? Math.floor(parsed)
    : DEFAULT_LAUNCHD_WAIT_ATTEMPTS;
}

function normalizedWaitMs(value) {
  const parsed = Number(value);
  return Number.isFinite(parsed) && parsed >= 0
    ? parsed
    : DEFAULT_LAUNCHD_WAIT_MS;
}

// `launchctl bootout` can return while a running KeepAlive job is still being
// removed. An immediate bootstrap then fails and leaves the service unloaded.
// Poll the launchd namespace with a bounded wait before loading the new plist.
function waitForLaunchdUnload(label, opts = {}) {
  const execImpl = opts.execImpl || execFileSync;
  const sleepImpl = opts.sleepImpl || sleepSync;
  const attempts = normalizedWaitAttempts(opts.attempts);
  const delayMs = normalizedWaitMs(opts.delayMs);
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      execImpl('launchctl', ['print', label], { stdio: 'ignore' });
    } catch (_) {
      return true;
    }
    if (attempt + 1 < attempts) sleepImpl(delayMs);
  }
  return false;
}

function reinstallLaunchAgent(target, serviceName, ctx, opts = {}) {
  const execImpl = opts.execImpl || execFileSync;
  const domain = `gui/${ctx.uid || uid()}`;
  const label = `${domain}/${serviceName}`;
  const waitAttempts = normalizedWaitAttempts(opts.waitAttempts);
  const waitMs = normalizedWaitMs(opts.waitMs);
  let bootoutSucceeded = false;
  try {
    execImpl('launchctl', ['bootout', label], { stdio: 'ignore' });
    bootoutSucceeded = true;
  } catch (_) {
    // Idempotent when the job is not loaded; bootstrap below remains authoritative.
  }
  if (bootoutSucceeded && !waitForLaunchdUnload(label, {
    execImpl,
    sleepImpl: opts.sleepImpl,
    attempts: waitAttempts,
    delayMs: waitMs,
  })) {
    return [{
      cmd: `launchctl print ${label}`,
      error: `launchd job still loaded after ${waitAttempts} checks following successful bootout`,
    }];
  }
  try {
    execImpl('launchctl', ['bootstrap', domain, target], { stdio: 'ignore' });
    return [];
  } catch (error) {
    return [{
      cmd: `launchctl bootstrap ${domain} ${target}`,
      error: error.message,
    }];
  }
}

// Install no-symlink + atomic rename.
// - lstat target: reject symlink (no follow)
// - write temp file nella stessa dir -> chmod mode -> atomic rename
// - exec service manager (systemctl/launchctl); skip in dryRun
// execImpl per test (default execFileSync).
function installService(platform, content, ctx, {
  dryRun = false,
  execImpl = execFileSync,
  sleepImpl,
  launchdWaitAttempts,
  launchdWaitMs,
  activate = true,
} = {}) {
  const home = ctx.home || os.homedir();
  const target = ctx.installPath || installPath(platform, home);
  const mode = fileMode(platform);

  // lstat: reject symlink preesistente
  try {
    const st = fs.lstatSync(target);
    if (st.isSymbolicLink()) {
      throw new Error(`refusing symlink install target: ${target}`);
    }
  } catch (e) {
    if (e.code !== 'ENOENT') throw e;
  }

  if (dryRun) {
    return { target, mode, written: false, failures: [], note: 'dry-run: nessuna scrittura' };
  }

  const runtimeDir = ctx.runtimeDir || path.join(home, '.nexuscrew');
  // A real install always has an existing home. Test/dry relocation may pass a
  // synthetic home together with a custom installPath; do not escape that
  // relocation merely to create a WorkingDirectory that will never be used.
  if (fs.existsSync(home) || ctx.runtimeDir) fs.mkdirSync(runtimeDir, { recursive: true, mode: 0o700 });
  fs.mkdirSync(path.dirname(target), { recursive: true });
  // temp file stessa dir (per atomic rename su stesso filesystem)
  const tmp = target + '.tmp.' + process.pid;
  try {
    fs.writeFileSync(tmp, content, { mode });
    fs.chmodSync(tmp, mode);
    fs.renameSync(tmp, target); // atomic
  } catch (e) {
    try { fs.unlinkSync(tmp); } catch (_) {} // cleanup temp su failure [m1]
    throw e;
  }

  // Exec service manager — NON ingoiare failure: raccogli per diagnosi
  const cmds = installCommands(platform, target, ctx);
  const activation = planActivation(platform, target, ctx, { execImpl, activate });
  if (!activation.activate) {
    // The manager will not run what this HOME just wrote, or the caller asked to
    // write without activating: the skipped plan stays declared, and a skip for a
    // foreign HOME says why instead of touching a service that is not ours.
    return {
      target,
      mode,
      written: true,
      failures: [],
      skippedActivation: cmds.map(([bin, args]) => `${bin} ${args.join(' ')}`),
      activation,
    };
  }
  const failures = [];
  if (platform === 'mac') {
    failures.push(...reinstallLaunchAgent(target, 'com.mmmbuto.nexuscrew', ctx, {
      execImpl,
      sleepImpl,
      waitAttempts: launchdWaitAttempts,
      waitMs: launchdWaitMs,
    }));
  } else {
    for (const [bin, args] of cmds) {
      try { execImpl(bin, args, { stdio: 'ignore' }); }
      catch (e) {
        failures.push({ cmd: `${bin} ${args.join(' ')}`, error: e.message });
      }
    }
  }

  return { target, mode, written: true, failures, activation };
}

function installCommands(platform, target, ctx) {
  if (platform === 'linux') {
    return [
      ['systemctl', ['--user', 'daemon-reload']],
      ['systemctl', ['--user', 'enable', 'nexuscrew']],
      ['systemctl', ['--user', 'restart', 'nexuscrew']], // restart carica nuovo codice (drop-in)
    ];
  }
  if (platform === 'mac') {
    const domain = `gui/${ctx.uid || uid()}`;
    const label = `${domain}/com.mmmbuto.nexuscrew`;
    // bootout (ignore se non esiste) poi bootstrap
    return [
      ['launchctl', ['bootout', label]],
      ['launchctl', ['bootstrap', domain, target]],
    ];
  }
  if (platform === 'termux') {
    return []; // nessun service manager; boot script + start manuale (nohup+pidfile)
  }
  return [];
}


// --- Activation guard -------------------------------------------------------
// The user service manager is per-UID, not per-HOME. `systemctl --user` and
// `launchctl` resolve our unit by NAME inside the namespace of the logged-in
// user, while this module writes the definition under the HOME it was given.
// A run with a temporary HOME therefore wrote a definition the manager never
// loads, and the activation commands still hit the real manager: the REAL
// service was restarted twice during a single smoke run on the live node.
//
// Decide ONCE, before the activation commands, reading back WHICH definition the
// manager has loaded. If it is not the one just written, activation is skipped
// and said out loud, instead of touching a service this HOME does not own.
const ACTIVATION_OUTCOMES = Object.freeze({
  activated: 'activated',
  skippedByFlag: 'skipped-by-flag',
  skippedForeignManager: 'skipped-foreign-manager',
  notApplicable: 'not-applicable',
});

const NO_ACTIVATE_WARNING = (target) =>
  `unit installed at ${target}, activation skipped: no service manager command was run`;

function foreignManagerWarning(target, managerPath) {
  return `unit installed at ${target} but the user service manager loads ${managerPath}: `
    + 'activation skipped, run `nexuscrew restart` from that home or use --no-activate';
}

// Two paths name the same file when they resolve to the same inode path; the
// manager reports its own canonical form, so compare canonically on both sides.
function sameDefinitionPath(a, b) {
  const canonical = (p) => {
    try { return fs.realpathSync(p); } catch (_) { return path.resolve(String(p)); }
  };
  return canonical(a) === canonical(b);
}

// Which definition does the user service manager have loaded for our unit?
//   absent   nothing loaded, or no readable answer: activation stays as it was
//   same     it loads the definition just written
//   foreign  it loads a DIFFERENT definition: this HOME does not own that service
//
// The guard fires on POSITIVE evidence only. An answer we cannot read as a
// definition path is never turned into a silent behaviour change: it keeps the
// previous activation path instead of guessing.
function readLoadedDefinition(platform, target, ctx, execImpl) {
  if (platform === 'linux') {
    let raw;
    try {
      raw = execImpl('systemctl', [
        '--user', 'show', 'nexuscrew.service', '--property=FragmentPath', '--value',
      ], { encoding: 'utf8' });
    } catch (_) {
      // No probing possible (no user manager): the activation commands below fail
      // the same way and are reported as failures, exactly as they were before.
      return { state: 'absent', path: null };
    }
    const loaded = typeof raw === 'string' ? raw.trim() : '';
    if (!loaded) return { state: 'absent', path: null };
    return sameDefinitionPath(loaded, target)
      ? { state: 'same', path: loaded }
      : { state: 'foreign', path: loaded };
  }
  if (platform === 'mac') {
    const label = `gui/${ctx.uid || uid()}/com.mmmbuto.nexuscrew`;
    let raw;
    try {
      raw = execImpl('launchctl', ['print', label], { encoding: 'utf8' });
    } catch (_) {
      return { state: 'absent', path: null }; // not loaded
    }
    const match = typeof raw === 'string' ? raw.match(/^\s*path = (.+?)\s*$/m) : null;
    if (!match) return { state: 'absent', path: null };
    return sameDefinitionPath(match[1], target)
      ? { state: 'same', path: match[1] }
      : { state: 'foreign', path: match[1] };
  }
  return { state: 'absent', path: null };
}

// The single decision point for activation: callers write the definition, then
// ask this whether the manager may be touched at all.
function planActivation(platform, target, ctx, { execImpl = execFileSync, activate = true } = {}) {
  if (platform === 'termux') {
    return { activate: false, outcome: ACTIVATION_OUTCOMES.notApplicable, managerPath: null, warning: null };
  }
  if (!activate) {
    return {
      activate: false,
      outcome: ACTIVATION_OUTCOMES.skippedByFlag,
      managerPath: null,
      warning: NO_ACTIVATE_WARNING(target),
    };
  }
  const loaded = readLoadedDefinition(platform, target, ctx, execImpl);
  if (loaded.state === 'foreign') {
    return {
      activate: false,
      outcome: ACTIVATION_OUTCOMES.skippedForeignManager,
      managerPath: loaded.path,
      warning: foreignManagerWarning(target, loaded.path),
    };
  }
  return {
    activate: true,
    outcome: ACTIVATION_OUTCOMES.activated,
    managerPath: loaded.path,
    warning: null,
  };
}


// --- Riconoscimento di PROPRIETA' (da revisione) -------------------------
// La sanazione automatica riscrive una definizione solo se e' NOSTRA. La prima
// versione cercava /nexuscrew/i nel contenuto: un unit di terzi che ci nomina
// in un commento («Avviato dopo NexusCrew») veniva sovrascritto. La parola non
// e' un titolo di proprieta'.
//
// L'ancora e' STRUTTURALE e sta ACCANTO al template che la produce, cosi' non
// possono divergere: chi cambia il template e non l'ancora fa diventare rosso
// il test che genera e riconosce (tests/npm-updater.test.js), invece di
// scoprirlo sul campo con una sanazione che smette di funzionare.
//   - systemd : il valore di Description= COMINCIA con «NexusCrew» (un terzo
//               descrive se' stesso, non noi) — riga intera, non sottostringa.
//   - launchd : il Label e' esattamente il nostro reverse-DNS. Usarlo sarebbe
//               impersonazione, non coincidenza.
//   - termux  : la prima riga di commento del nostro script di boot.
// Le righe identificative sono definite QUI e usate SIA dal template SIA
// dall'ancora: cosi' non possono divergere per costruzione, e il test guardia
// diventa una seconda difesa invece dell'unica.
//
// da revisione: l'ancora linux era un PREFISSO
// (`/^Description=NexusCrew\b/`) e un servizio di terzi che comincia la sua
// Description con il nostro nome — «NexusCrew-compatible proxy», «NexusCrew
// fork by tizio» — veniva riconosciuto come nostro e sovrascritto. Il mac era
// gia' esatto (il Label reverse-DNS completo non e' una coincidenza, e' una
// impersonazione): stesso standard ovunque, ancora = RIGA ESATTA.
const LINUX_SERVICE_DESCRIPTION = 'Description=NexusCrew - browser tmux client (loopback, solo tunnel SSH/VPN)';
const TERMUX_SERVICE_HEADER = '# NexusCrew boot (Termux) - loopback, localhost del telefono';

function exactLine(line) {
  const escaped = line.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return new RegExp(`^${escaped}$`, 'm');
}

const OUR_SERVICE_ANCHOR = Object.freeze({
  linux: exactLine(LINUX_SERVICE_DESCRIPTION),
  mac: /^\s*<string>com\.mmmbuto\.nexuscrew<\/string>\s*$/m,
  termux: exactLine(TERMUX_SERVICE_HEADER),
});

function isOurService(platform, content) {
  const re = OUR_SERVICE_ANCHOR[platform];
  if (!re || typeof content !== 'string' || !content) return false;
  return re.test(content);
}

module.exports = {
  isOurService, OUR_SERVICE_ANCHOR,
  generateService, generateLinux, generateMac, generateTermux,
  installService, installPath, installCommands, fileMode,
  planActivation, readLoadedDefinition, sameDefinitionPath, ACTIVATION_OUTCOMES,
  ensureLinuxTmuxSurvival, linuxTmuxSurvivalPath, LINUX_TMUX_SURVIVAL_DROPIN,
  reinstallLaunchAgent, waitForLaunchdUnload,
  escapeSystemdPath, escapeSystemdExec, escapeXml, shellQuote,
  assertSystemdSafe, SYSTEMD_FORBIDDEN,
};
