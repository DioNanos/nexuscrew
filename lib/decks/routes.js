...
function decksRoutes({ cfg = {}, decksPath }) {
  ...
  const checkRevision = (deck, rev) => {
    if (deck.revision !== rev) {
      // Traccia il conflitto di revisione con i campi che servono a
      // capire chi lo ha causato (nessun merge lato server in questo ciclo).
      console.log(JSON.stringify({
        event: 'deck-revision-conflict',
        name: deck.name,
        expectedRevision: rev,
        currentRevision: deck.revision,
      }));
      throw httpError(409, 'deck modificato da un’altra finestra', { current: deck });
    }
  };
