...
function proxyHttp(req, res, { port, path, credential, visited = null, hopProof = null, setCookiePrefix = null, panelCookie = null, capture = null }) {
...
function routeHandler({ nodesPath, localPort, localCredential, ingress = null, readonly = () => false, hopSecret = null, liveHostLedger = null }) {
  return (req, res) => {
    ...
    const capture = liveHostLedger && (parsed.resource === '/live-host/designate' || parsed.resource === '/live-host/clear')
      ...
      : null;
    proxyHttp(req, res, { port: next.localPort, path, credential: next.token, visited, setCookiePrefix, panelCookie, capture });
  };
}
...
function forwardUpgrade({ req, socket, head, nodesPath, localPort, localCredential, ingress, readonly = () => false, activeSockets = null, hopSecret = null }) {
  ...
  // and a limited peer must not reach an operator-class channel through it.
  {
    ...
    if (!gate.ok) return reject(socket, gate.status === 404 ? 404 : 403);
  }
  const st = store.loadStore(nodesPath);
