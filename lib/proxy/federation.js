  ...
  r.post('/share', express.json({ limit: '1kb' }), async (req, res) => {
    ...
    try {
      if (body.shared) {
        ...
        if (health.status !== 'healthy') {
          // Codice tipizzato: "non ancora pronto" e' ritentabile, e chi chiama
          // deve poterlo distinguere da un guasto definitivo invece di dedurlo
          // da una stringa.
          return res.status(409).json({
            error: 'canale share non raggiungibile',
            code: SHARE_NOT_READY_CODE,
...
module.exports = {
  MAX_HOPS, ROUTE_DELIMITER, TOPOLOGY_PEER_TIMEOUT_MS, SHARE_NOT_READY_CODE,
