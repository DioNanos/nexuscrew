...
const { allowedByClass, classifyResource } = require('./resource-acl.js');
...
// Budget per hop della topologia federata: quando la richiesta arriva da un
// peer, il fan-out verso i propri figli si tiene questo margine sotto il
// budget del chiamante, cosi' la risposta rientra sempre in chi chiama.
const HOP_MARGIN_MS = 500;
const CHILD_BUDGET_FLOOR_MS = 300;
// Ultimo snapshot riuscito per figlio diretto (cache in memoria, chiave
// store+figlio): serve i rami lenti come stale invece di farli sparire dal
// ramo del chiamante.
const CHILD_SNAPSHOT_MAX = 64;
const childSnapshotsByNodeId = new Map();

function recordChildSnapshot(nodesPath, nodeId, nodes) {
  childSnapshotsByNodeId.set(`${nodesPath}::${nodeId}`, { ts: Date.now(), nodes });
  if (childSnapshotsByNodeId.size <= CHILD_SNAPSHOT_MAX) return;
  let oldest = null;
  for (const [id, snap] of childSnapshotsByNodeId) if (!oldest || snap.ts < oldest[1].ts) oldest = [id, snap];
  if (oldest) childSnapshotsByNodeId.delete(oldest[0]);
}
...
function knownResource(resource) {
  return resource === '/sessions'
    ...
    // target, dedicated budget. It is NOT the authority-model queue: that one
    ...
    // Event feed: a closed GET surface inside the authenticated API.
    // The handler demands a proven last hop on top of the class gate — the
    // allowlist only admits the resources exist at all.
    || resource === '/event-feed'
    || resource === '/event-feed/snapshot'
    // Federated ask write-back: answer, dismiss, attempt status.
    || /^\/event-feed\/asks\/[a-f0-9]{8}(?:\/answer|\/requests\/[0-9a-f-]{16,64})?$/.test(resource)
...
// delle risorse e' chiuso: quattro endpoint fissi, non un percorso arbitrario.
const LIVE_HOST_RESOURCES = new Set(['/live-host', '/live-host/designate', '/live-host/clear', '/live-host/bridge']);
...
// One decision, used by BOTH federated entry points: the HTTP relay and the
// WebSocket upgrade. A resource whose class is unknown, or whose grants the
// owner has not conceded, is refused before anything is dispatched.
function classGateCheck(resource, method, ingress) {
  const cls = classifyResource(resource, method);
  if (!cls) return { ok: false, status: 404, reason: 'resource-not-classified' };
  if (!allowedByClass(cls, ingress)) return { ok: false, status: 403, reason: `grant-required:${cls}` };
...
function allowedResource(resource, method = 'GET') {
  ...
  // The bridge resolution travels to the owner without parameters: the
  // designation picks the target, so a forwarded call cannot add one.
  if (resource === '/live-host/bridge') return method === 'POST';
  ...
  if (/^\/event-feed\/node\/[a-f0-9]{32}$/.test(resource)) return method === 'GET';
...
const EVENT_FEED_CURSOR_RE = /^[0-9]{1,12}:[0-9]{1,12}$/;

function allowedQuery(resource, method, rawUrl) {
  // The event feed has a CLOSED query surface: only `after=<epoch>:<seq>` on
  // the SSE stream, nothing at all on the snapshot. A token in a query is
  // refused on these paths whatever it looks like.
  if (resource === '/event-feed' || resource === '/event-feed/snapshot'
    ...
    if (keys.some((k) => !/^[A-Za-z0-9_]+$/.test(k))) return false;
    if (params.has('token') || params.has('access_token')) return false;
    if (resource !== '/event-feed') return keys.length === 0;
    if (params.getAll('after').length > 1) return false;
    if (keys.length === 0) return true;
    if (keys.length !== 1 || !params.has('after')) return false;
    return EVENT_FEED_CURSOR_RE.test(params.get('after') || '');
  }
...
// What this node records when a designation is created or removed on a PEER
// through it: `ownerId` is the peer the forward went to, and the revision is
// the owner's own, taken from its 2xx answer — never a local guess, because the
// owner is the one that decides. A non-2xx answer, or a body without
// `{hostCell, revision}`, records nothing: an absent pointer is honest, an
// invented one is not.
function liveHostLedgerCapture({ ledger, ownerId, resource }) {
  return (status, text) => {
    if (!ledger || !(status >= 200 && status < 300)) return;
    if (resource === '/live-host/clear') {
      if (typeof ledger.clearRemote === 'function') ledger.clearRemote(ownerId);
      return;
    }
    let body = null;
    try { body = JSON.parse(String(text || '')); } catch (_) { return; }
    if (!body || typeof body.hostCell !== 'string' || !Number.isInteger(body.revision)) return;
    if (typeof ledger.recordRemote === 'function') {
      ledger.recordRemote({ ownerId, hostCell: body.hostCell, revision: body.revision });
    }
  };
}

function proxyHttp(req, res, { port, path, credential, visited = null, hopProof = null, setCookiePrefix = null, panelCookie = null, capture = null, getTimeoutMs = 10000 }) {
  // A body a local middleware has already read is no longer in the stream: the
  // headers would announce bytes that nothing sends, and the peer would wait for
  // a body that never arrives. Say it at once, with a name, instead of letting
  // the forward sit until the upstream timeout expires.
  if (Number(req.headers['content-length'] || 0) > 0 && req.readableEnded) {
    return res.status(502).json({ error: 'request body already read by a local middleware', reason: 'federation-body-consumed' });
  }
  const up = http.request({ host: '127.0.0.1', port, method: req.method, path, headers: cleanHeaders(req.headers, credential, visited, hopProof, panelCookie) }, (r) => {
    ...
    if (!capture) { res.writeHead(r.statusCode, sanitizeResponseHeaders(headers)); r.pipe(res); return; }
    // Buffered only where a capture asks for it (the two live-host mutations):
    // the owner's answer carries the revision that has to be recorded.
    const chunks = [];
    r.on('data', (chunk) => chunks.push(chunk));
    r.on('end', () => {
      const body = Buffer.concat(chunks);
      try { capture(r.statusCode, body.toString('utf8')); } catch (_) { /* the ledger never breaks the forward */ }
      if (res.writableEnded) return;
      res.writeHead(r.statusCode, sanitizeResponseHeaders(headers));
      res.end(body);
    });
  });
  // La GET federata (decks, fleet/status, sessions…) deve degradare in tempo
  // quando il tunnel del peer è «su a metà» (socket aperto, nessuna risposta):
  // 10 s e 504 — prima era un unico 30 s che chiudeva con 502. Gli altri
  // metodi mantengono il tetto largo di prima.
  const FORWARD_TIMEOUT_MS = req.method === 'GET' ? getTimeoutMs : 30000;
  up.setTimeout(FORWARD_TIMEOUT_MS, () => {
    if (!res.headersSent) {
      res.status(504).json({ error: 'peer non risponde entro il timeout', reason: 'federation-peer-timeout' });
      up.destroy();
      return;
    }
    up.destroy();
  });
  up.on('error', () => { if (!res.headersSent) res.status(502).json({ error: 'peer non raggiungibile', reason: 'federation-peer-unreachable' }); else res.destroy(); });
  req.pipe(up);
}

function routeHandler({ nodesPath, localPort, localCredential, ingress = null, readonly = () => false, hopSecret = null, liveHostLedger = null, forwardGetTimeoutMs = 10000 }) {
  return (req, res) => {
    ...
    // Class gate, before the dispatch: every federated resource belongs to a
    // class, and the class decides which grants the owner must have conceded.
    // A resource that is allowlisted but not classified is DENIED — a new route
    // must not reach a limited peer by accident — and the coverage test fails
    // when the allowlist grows without the classification.
    if (parsed && allowedResource(parsed.resource, req.method)) {
      const gate = classGateCheck(parsed.resource, req.method, ingress);
      if (!gate.ok) {
        return res.status(gate.status).json({
          error: gate.status === 404 ? 'not found' : 'risorsa non concessa a questo nodo',
          reason: gate.reason,
        });
      }
    }
    ...
    if (parsed.route.length === 0) {
      ...
      return proxyHttp(req, res, {
        ...
        panelCookie,
        getTimeoutMs: forwardGetTimeoutMs,
      });
    }
    ...
    const setCookiePrefix = !ingress && isPanelResource(parsed.resource)
      ...
      : null;
    // A designation forwarded from here lands on the owner: what this node ends
    // up pointing at is decided THERE, so the pointer is recorded from the
    // owner's answer (liveHostLedgerCapture) and not from a local guess.
    const capture = liveHostLedger && (parsed.resource === '/live-host/designate' || parsed.resource === '/live-host/clear')
      ? liveHostLedgerCapture({ ledger: liveHostLedger, ownerId: next.nodeId, resource: parsed.resource })
      : null;
    proxyHttp(req, res, { port: next.localPort, path, credential: next.token, visited, setCookiePrefix, panelCookie, capture, getTimeoutMs: forwardGetTimeoutMs });
  };
}
...
// Share-off reconciliation runner at boot.
...
async function fetchPeerTopology({ node, ttl, seen, fetchImpl, timeoutMs }) {
  ...
  const request = (async () => {
    const u = `http://127.0.0.1:${node.localPort}/federation/topology?ttl=${ttl - 1}&visited=${encodeURIComponent([...seen].join(','))}&budget=${budget}`;
...
// Validazione dei nodi figli riportati da un peer (stesso gate del loop
// storico): identita' strict, route valida e aciclica verso il padre, nome
// coerente con l'ultimo hop. Le entry valide sono relative al figlio.
// Lo stato stale/lastSeen che il salto precedente aveva gia' calcolato
// VIAGGIA con il figlio (ben formato: stale===true, lastSeen intero): senza
// questa propagazione l'ultimo hop ricompatterebbe tutto come fresco e il
// badge veritiero non arriverebbe mai a chi guarda.
function validatedChildNodes(n, seen, ttl, body) {
  if (!body || body.instanceId !== n.nodeId || !Array.isArray(body.nodes)) return null;
  const out = [];
  for (const child of body.nodes) {
    ...
    out.push({
      instanceId: child.instanceId, name: child.name, route: [...child.route],
      label: store.validLabel(child.label) ? child.label.trim() : '',
      ...(child.stale === true
        ? { stale: true, ...(Number.isSafeInteger(child.lastSeen) ? { lastSeen: child.lastSeen } : {}) }
        : {}),
    });
  }
  return out;
...
async function collectTopologyDetailed({
  ...
  timeoutMs = TOPOLOGY_PEER_TIMEOUT_MS, childSnapshots = true,
}) {
  const st = store.loadStore(nodesPath);
  ...
  const budget = Number.isFinite(timeoutMs)
    ? Math.max(1, Math.min(30000, Math.floor(timeoutMs))) : TOPOLOGY_PEER_TIMEOUT_MS;
  // Richiesta da un peer: il fan-out verso i figli usa un budget inferiore a
  // quello del chiamante, cosi' questa risposta rientra sempre nel suo budget.
  const childBudget = ingress
    ? Math.max(CHILD_BUDGET_FLOOR_MS, budget - HOP_MARGIN_MS) : budget;
  const out = [];
  const authoritative = [];
  const probes = [];
  for (const n of st.nodes) {
    ...
    probes.push({ n, pending: fetchPeerTopology({ node: n, ttl, seen, fetchImpl, timeoutMs: childBudget }) });
  }
  ...
  for (const { n, response, body } of results) {
    const children = response && response.ok ? validatedChildNodes(n, seen, ttl, body) : null;
    if (children) {
      authoritative.push(n.name);
      recordChildSnapshot(nodesPath, n.nodeId, children);
    ...
    // Figlio che non ha risposto in tempo (failure di trasporto: response null):
    // si serve l'ultimo snapshot riuscito marcando le entry stale con lastSeen,
    // invece di far sparire l'intero ramo dalla topologia di chi chiama. Una
    // risposta ARRIVATA ma non valida (binding instanceId sbagliato, errore
    // HTTP) non resuscita nulla: il ramo resta assente, come prima. (Raccolta
    // locale esclusa: la sua cache su disco ha gia' questa semantica con il
    // proprio clock.)
    const snapshot = childSnapshots && !response
      ? childSnapshotsByNodeId.get(`${nodesPath}::${n.nodeId}`) : null;
    if (!snapshot) continue;
    const lastSeen = Math.floor(snapshot.ts / 1000);
    for (const child of snapshot.nodes) {
      if (child.instanceId === n.nodeId || seen.has(child.instanceId)) continue;
      if (!Array.isArray(child.route) || child.route.length < 1 || child.route.length >= ttl
        || child.route.includes(n.name)) continue;
      out.push({
        instanceId: child.instanceId, name: child.name,
        route: [n.name, ...child.route], direct: false,
        label: child.label, stale: true,
        // il lastSeen piu' veritiero e' quello che il figlio stesso aveva
        // riportato (se era gia' stale); altrimenti il momento dello snapshot
        lastSeen: Number.isSafeInteger(child.lastSeen) ? child.lastSeen : lastSeen,
      });
    }
  }
...
// transitive nodes. Slow children fall back to the last good in-memory
// snapshot (stale:true); the peer endpoint returns them too, so a branch that
// misses one poll degrades to stale instead of disappearing.
async function collectLocalTopology({
  ...
  const live = await collectTopologyDetailed({ nodesPath, fetchImpl, timeoutMs, childSnapshots: false });
  const st = store.loadStore(nodesPath);
  ...
  for (const n of live.nodes) {
    ...
    // togliere. Resta un dato riferito, non una prova. Per i nodi arrivati
    // gia' stale dal salto precedente, il lastSeen ORIGINALE e' il dato vero:
    // "ora" mentirebbe su quando sono stati visti l'ultima volta.
    if (n.route.length > 1) {
      next.set(n.instanceId, {
        instanceId: n.instanceId, name: n.name, route: [...n.route],
        ...(n.label ? { label: n.label } : {}),
        lastSeen: n.stale === true && Number.isSafeInteger(n.lastSeen) ? n.lastSeen : now,
      });
    }
  }
  ...
  // Solo i nodi visti DIRETTAMENTE vivi prendono stale:false + lastSeen=now:
  // un nodo che il peer aveva gia' marcato stale resta stale col suo lastSeen.
...
function peerRouter({ nodesPath, localPort, localCredential, fetchImpl, readonly = () => false, version = null, roles = null, hopSecret = null, diagnostics = null }) {
  ...
  r.get('/health', (_req, res) => {
    ...
    // eventFeedV1: the peer checks this capability BEFORE subscribing to the
    // event feed. An older node reports no capability and the client marks the
    // feed unsupported instead of retrying forever.
  ...
  r.get('/topology', async (req, res) => {
    ...
    // Il budget residuo del chiamante viaggia nella richiesta: il fan-out
    // interno se ne tiene un margine (budget per hop) per rispondere in tempo.
    const rawBudget = Number(req.query.budget);
    const timeoutMs = Number.isFinite(rawBudget)
      ? Math.max(CHILD_BUDGET_FLOOR_MS, Math.min(30000, Math.floor(rawBudget))) : TOPOLOGY_PEER_TIMEOUT_MS;
    res.json(await collectTopology({ nodesPath, ingress: req.peer, ttl, visited, fetchImpl, timeoutMs }));
  });
...
function localRouter({ nodesPath, localPort, localCredential, readonly, hopSecret = null, liveHostLedger = null }) {
  const r = express.Router();
  r.use((req, res) => routeHandler({ nodesPath, localPort, localCredential, readonly, hopSecret, liveHostLedger })(req, res));
  return r;
}

function forwardUpgrade({ req, socket, head, nodesPath, localPort, localCredential, ingress, readonly = () => false, activeSockets = null, hopSecret = null, hubAttaches = null }) {
  ...
  // The same class decision as the HTTP relay: an upgrade is an entry point too,
  // and a limited peer must not reach an operator-class channel through it.
  {
    const gate = classGateCheck(parsed.resource, 'GET', ingress);
    if (!gate.ok) { if (parsed.resource === '/ws') return rejectAsNoSuchSession(socket, req); return reject(socket, gate.status === 404 ? 404 : 403); }
  }
  const st = store.loadStore(nodesPath);
  if (!st) return reject(socket, 503);
  const visited = controlledVisited(req, ingress, st.nodeId);
  if (!visited) return reject(socket, 409);
  // L'id e la sessione viaggiano SOLO nella query (niente de-mascheramento dei
  // frame); il token si confronta per IMPRONTA e non entra mai in chiaro.
  const attachId = parseAttachId(queryParam(req.url, 'attachId'));
  const attachSession = parseAttachSession(queryParam(req.url, 'attachSession'));
  // Sul percorso del browser il token e' in query; su quello FEDERATO e' il
  // Bearer del peer che ha originato l'attach (la query non lo porta). Senza
  // questo, l'ingresso federato non entrava mai nel ramo trattenuto.
  const attachToken = queryParam(req.url, 'token') || bearerFrom(req) || null;
  const attachRoute = parsed.route.join('/');
  const hubMode = Boolean(hubAttaches && attachId && attachSession && attachToken);
  ...
  // Aggancio alla attach trattenuta — SOLO dopo i gate d'ingresso appena
  // eseguiti (route/next/privateInbound/canTransit) e SOLO se il record e'
  // STACCATO: un secondo upgrade con la stessa identita' non deve rubare lo
  // stream a un client ancora connesso.
  if (hubMode) {
    const record = hubAttaches.take({
      attachId, tokenHash: tokenFingerprint(attachToken), route: attachRoute, session: attachSession,
    });
    if (record) {
      const handshake = wsHandshakeResponse(req);
      if (!handshake) return reject(socket, 403);
      // 101 a mano, poi passthrough: l'input del nuovo client passa com'e' e
      // l'upstream de-maschera per frame (la chiave di mascheramento puo'
      // cambiare), l'output va scritto grezzo (i frame li ha gia' incorniciati
      // l'upstream).
      try { socket.write(handshake); } catch (_) { /* morto */ }
      // Gli eventuali byte gia' arrivati dopo la richiesta di upgrade vanno
      // inoltrati come nel ramo normale, non persi.
      if (head && head.length) { try { record.up.write(head); } catch (_) { /* morto */ } }
      socket.pipe(record.up, { end: false });
      // Identity check: la caduta di QUESTO socket non deve staccare il record
      // se nel frattempo ne ha preso uno nuovo.
      const onGoneResume = () => { if (record.socket === socket) hubAttaches.detach(record); };
      socket.on('end', onGoneResume);
      socket.on('close', onGoneResume);
      socket.on('error', () => { /* il close segue */ });
      // Anche questo socket entra in activeSockets, o la chiusura ordinata
      // del server non lo vede.
      if (activeSockets && typeof activeSockets.add === 'function') {
        activeSockets.add(socket); activeSockets.add(record.up);
        const remove = () => { activeSockets.delete(socket); activeSockets.delete(record.up); };
        socket.once('close', remove); record.up.once('close', remove);
      }
      hubAttaches.attach(record, socket);
      return;
    }
  }
  const up = net.connect({ host: '127.0.0.1', port });
  // Osservatore NON consumante della testa di risposta dell'upstream: serve
  // solo a sapere se il 101 e' transitato (client in stato WebSocket) o se
  // l'hop e' morto prima. La pipe resta l'unico trasporto dei byte: il
  // listener non sottrae nulla, guarda gli stessi chunk.
  let saw101 = false;
  let headBuf = Buffer.alloc(0);
  let gone = false;
  up.on('data', (chunk) => {
    if (saw101 || headBuf.length >= 1024) return;
    headBuf = Buffer.concat([headBuf, chunk.subarray(0, Math.max(0, 1024 - headBuf.length))]);
    if (headBuf.includes(' 101')) saw101 = true;
  });
  // L'hop e' morto (errore di connessione o chiusura a stream avviato): il
  // client deve riceverlo come codice, non come troncamento.
  const upstreamGone = () => {
    if (gone) return;
    gone = true;
    if (saw101) {
      // Handshake gia' transitato: un frame di close e' cio' che il browser
      // sa leggere. Se l'upstream aveva gia' chiuso con un suo codice, quel
      // frame e' arrivato prima e vince (il browser si ferma al primo).
      try { socket.write(wsCloseFrame(4402, 'upstream gone')); } catch (_) { /* socket morto */ }
      try { socket.end(); } catch (_) { /* idem */ }
      return;
    }
    acceptWsThenClose(socket, req, 4402, 'upstream gone');
  };
  up.once('connect', () => {
    ...
    if (hubMode) {
      // Attach trattenuta: l'output passa dal ring quando il client non c'è,
      // e il record entra nello store sotto la sua identità.
      const record = hubAttaches.hold({
        attachId, tokenHash: tokenFingerprint(attachToken), route: attachRoute, session: attachSession, up,
      });
      up.on('data', (chunk) => hubAttaches.feed(record, chunk));
      up.on('close', () => hubAttaches.destroy(record));
      up.on('error', () => hubAttaches.destroy(record));
      // end:false: la caduta del CLIENT non deve chiudere l'upstream — è
      // esattamente ciò che la grazia deve trattenere.
      socket.pipe(up, { end: false });
      hubAttaches.attach(record, socket);
      // Senza un handler d'errore il RST del client morto resta senza 'close':
      // la caduta non verrebbe mai rilevata e la grazia non partirebbe.
      socket.on('error', () => { /* il close segue */ });
      // 'end' E 'close': con la pipe a end:false il socket resta HALF-OPEN
      // quando il browser se ne va, quindi il solo 'close' puo' non arrivare
      // mai e la grazia non partirebbe. Il FIN in lettura e' la caduta.
      const onGoneHeld = () => { if (record.socket === socket) hubAttaches.detach(record); };
      socket.on('end', onGoneHeld);
      socket.on('close', onGoneHeld);
      if (activeSockets && typeof activeSockets.add === 'function') {
        activeSockets.add(socket); activeSockets.add(up);
        const remove = () => { activeSockets.delete(socket); activeSockets.delete(up); };
        socket.once('close', remove); up.once('close', remove);
    ...
    socket.pipe(up);
    // end:false: la chiusura verso il browser la governiamo noi, o la pipe
    // chiuderebbe il socket PRIMA che il frame di close 4402 possa partire.
    up.pipe(socket, { end: false });
  });
  if (!hubMode) {
    // Nel ramo trattenuto la morte dell'upstream la governa lo store (destroy
    // del record): qui il close leggibile verso il browser non ha più un
    // destinatario quando il client è già caduto.
    up.on('error', () => upstreamGone());
    up.on('end', () => upstreamGone());
    up.on('close', () => upstreamGone());
  }
  socket.on('error', () => up.destroy());
}

// Indistinguibile da «sessione inesistente» (lib/ws/bridge.js): le stesse
// chiamate della libreria ws usata dal bridge (sendJson + close), cosi' i byte
// sul canale sono quelli del percorso locale. Un rifiuto HTTP grezzo sul
// canale WS rivelerebbe l'esistenza o lo stato della risorsa.
const REJECT_WSS = new WebSocketServer({ noServer: true });
function rejectAsNoSuchSession(socket, req) {
  // Solo un VERO upgrade WS ha una chiave: per qualunque altra richiesta su /ws
  // (non-upgrade) resta il rifiuto di classe di prima — non c'e' un canale
  // WebSocket da chiudere, e rispondere come il bridge non avrebbe senso.
  if (!req || !req.headers || !req.headers['sec-websocket-key']) return reject(socket, 403);
  try {
    REJECT_WSS.handleUpgrade(req, socket, Buffer.alloc(0), (ws) => {
      try { ws.send(JSON.stringify({ type: 'error', reason: 'no such session' })); } catch (_) { /* socket gia' morto */ }
      try { ws.close(4404, 'no such session'); } catch (_) { /* idem */ }
    });
  } catch (_) { try { socket.destroy(); } catch (_) { /* idem */ } }
}

// ---- attach trattenuta nell'hub --------------------------------------
// Quando il client cade, l'upstream federato NON viene distrutto subito: resta
// vivo per la grazia e il suo output va in un ring. Chi torna con la STESSA
// identità (hash del token, stessa route, stessa sessione) riprende da qui
// senza riaprire l'attach; chiunque altro fa una attach nuova.
const HUB_ATTACH_MAX = 8;
const HUB_ATTACH_GRACE_MS = 60000;
const HUB_ATTACH_RING_BYTES = 256 * 1024;
...
function queryParam(url, name) {
  ...
  for (const part of q.split('&')) {
    ...
    try { key = decodeURIComponent(key); } catch (_) { /* chiave grezza */ }
    if (key !== name) continue;
    try { return decodeURIComponent(part.slice(i + 1)); } catch (_) { return part.slice(i + 1); }
  }
  return null;
}
// 128 bit in esadecimale, e nient'altro: un id malformato non è un attachId.
const ATTACH_ID_RE = /^[0-9a-f]{32}$/;
const parseAttachId = (raw) => (typeof raw === 'string' && ATTACH_ID_RE.test(raw) ? raw : null);
// Nome sessione con lo stesso alfabeto accettato dalle risorse federate.
const ATTACH_SESSION_RE = /^[\w.@%:+-]{1,128}$/;
const parseAttachSession = (raw) => (typeof raw === 'string' && ATTACH_SESSION_RE.test(raw) ? raw : null);
// Il token non entra MAI in memoria in chiaro: si confronta la sua impronta.
const tokenFingerprint = (token) => crypto.createHash('sha256').update(String(token)).digest('hex');
// Diagnostica: mai l'id completo, mai il token.
const shortId = (id) => (typeof id === 'string' ? id.slice(0, 6) : '-');

// Ricompone i confini di frame del protocollo WS leggendo SOLO la lunghezza
// dall'header. Server->client non e' mascherato; se lo fosse, l'header e' piu'
// lungo e i 4 byte di maschera si saltano senza toccare il payload.
    ...
    push(chunk) {
      ...
      while (off + 2 <= buf.length) {
        ...
        if (len === 126) {
          ...
          len = Number(buf.readBigUInt64BE(off + 2)); head = 10;
        }
        if (b1 & 0x80) head += 4;
        const total = head + len;
        if (off + total > buf.length) break;
        frames.push(buf.subarray(off, off + total));
        off += total;
      }
      buf = off ? buf.subarray(off) : buf;
      return frames;
    },
    pending: () => buf,
  };
...
function createHubAttachStore({ graceMs = HUB_ATTACH_GRACE_MS, maxAttaches = HUB_ATTACH_MAX, ringBytes = HUB_ATTACH_RING_BYTES } = {}) {
  const grace = Math.max(1, Number(graceMs) || HUB_ATTACH_GRACE_MS);
  const cap = Math.max(1, Math.floor(Number(maxAttaches) || HUB_ATTACH_MAX));
  const ringLimit = Math.max(1, Math.floor(Number(ringBytes) || HUB_ATTACH_RING_BYTES));
  ...
  const destroy = (record) => {
    ...
    if (records.get(record.attachId) === record) records.delete(record.attachId);
    // Se un client e' ATTACCATO, la morte dell'upstream va DETTA subito: senza,
    // il browser resta appeso finche' la liveness non scade (10 s) — e in
    // 0.9.39 il 4402 arrivava invece immediatamente. Se nessuno e' attaccato
    // non c'e' nulla da avvisare.
    if (record.socket && record.socket.writable) {
      try { record.socket.write(wsCloseFrame(4402, 'upstream gone')); } catch (_) { /* morto */ }
      try { record.socket.end(); } catch (_) { /* idem */ }
      record.socket = null;
    }
    try { record.up.destroy(); } catch (_) { /* già chiuso */ }
  };
  // Il ring tiene FRAME INTERI: un ring di soli chunk TCP puo' consegnare un
  // replay che comincia a meta' frame, e il browser chiude con 1002. Il parser
  // legge SOLO la lunghezza dagli header (server->client non maschera) e non
  // tocca i payload: nessun de-mascheramento.
  const pushRing = (record, frame) => {
    record.ring.push(frame);
    record.ringBytes += frame.length;
    while (record.ringBytes > ringLimit && record.ring.length > 1) record.ringBytes -= record.ring.shift().length;
  };
  const hold = ({ attachId, tokenHash, route, session, up }) => {
    // Cap: si chiude la PIÙ VECCHIA, non si rifiuta la nuova.
    while (records.size >= cap) {
      const oldest = records.keys().next().value;
      const rec = oldest ? records.get(oldest) : null;
      if (!rec) break;
      destroy(rec);
    }
    const record = {
      attachId, tokenHash, route, session, up, socket: null,
      ring: [], ringBytes: 0, timer: null, ended: false, splitter: createFrameSplitter(),
    };
    records.set(attachId, record);
    return record;
  };
  // Scrive il pregresso al nuovo client: prima il ring, poi lo stream live.
  const attach = (record, socket) => {
    if (!record || record.ended) return false;
    if (record.timer) { clearTimeout(record.timer); record.timer = null; }
    record.socket = socket;
    for (const frame of record.ring) { try { socket.write(frame); } catch (_) { break; } }
    // I byte già letti ma non ancora completati in un frame vanno consegnati
    // ora: senza, il client perderebbe la coda dell'ultimo frame.
    const pending = record.splitter.pending();
    if (pending && pending.length) { try { socket.write(pending); } catch (_) { /* morto */ } }
    record.splitter = createFrameSplitter();
    record.ring = []; record.ringBytes = 0;
    return true;
  };
  // Il client è caduto: da qui l'output va nel ring, e parte la grazia.
  const detach = (record) => {
    if (!record || record.ended) return;
    record.socket = null;
    if (record.timer) clearTimeout(record.timer);
    record.timer = setTimeout(() => destroy(record), grace);
    if (typeof record.timer.unref === 'function') record.timer.unref();
  };
  const feed = (record, chunk) => {
    if (!record || record.ended) return;
    if (record.socket && record.socket.writable) {
      // Passthrough: i byte vanno al client come sono arrivati.
      try { record.socket.write(chunk); return; } catch (_) { /* cade: si passa al ring */ }
    }
    for (const frame of record.splitter.push(chunk)) pushRing(record, frame);
  };
  // Aggancio: SOLO identità identica (impronta token, route, sessione).
  const take = ({ attachId, tokenHash, route, session }) => {
    const record = records.get(attachId);
    if (!record || record.ended) return null;
    // Solo un record STACCATO (in grazia): agganciare un record ancora
    // attaccato vorrebbe dire rubare lo stream a un client vivo. Un socket
    // DITRUTTO non e' un client vivo — e il suo 'close' puo' arrivare dopo,
    // quindi lo stato del socket e' la verita' osservabile, non l'evento.
    if (record.socket && record.socket.destroyed !== true) return null;
    if (record.tokenHash !== tokenHash || record.route !== route || record.session !== session) return null;
    return record;
  };
  return {
    hold, attach, detach, feed, take, destroy,
    size: () => records.size,
    get: (id) => records.get(id) || null,
  };
}

// Handshake WS del ramo di ripresa: identico a quello che scrive REJECT_WSS
// (stesso Sec-WebSocket-Accept), ma SENZA creare un oggetto ws. Il socket del
// nuovo client resta un PASSTHROUGH di frame come nel ramo normale: niente
// de-mascheramento, niente re-incapsulamento.
const WS_ACCEPT_GUID = '258EAFA5-E914-47DA-95CA-C5AB0DC85B11';
function wsHandshakeResponse(req) {
  const key = req && req.headers && req.headers['sec-websocket-key'];
  if (!key) return null;
  const accept = crypto.createHash('sha1').update(String(key) + WS_ACCEPT_GUID).digest('base64');
...
// Frame di close costruito a mano: sul ramo "hop morto DOPO il 101" non c'e'
// piu' una richiesta di upgrade da consegnare a handleUpgrade (l'handshake e'
// gia' transitato), quindi il frame va scritto direttamente.
function wsCloseFrame(code, reason = '') {
  const text = String(reason).slice(0, 100);
  const payload = Buffer.alloc(2 + Buffer.byteLength(text));
  payload.writeUInt16BE((Number(code) || 1000) & 0xffff, 0);
  payload.write(text, 2, 'utf8');
  const len = payload.length;
  return len < 126
    ? Buffer.concat([Buffer.from([0x88, len]), payload])
    : Buffer.concat([Buffer.from([0x88, 126, (len >> 8) & 0xff, len & 0xff]), payload]);
}

// Hop morto e il client NON ha ancora visto il 101: completiamo l'handshake
// qui, cosi' il browser entra in stato WebSocket e puo' leggere un frame di
// close LEGGIBILE (4402) invece di un 502 grezzo o di un EOF muto. L'accept
// lo calcola la libreria (stesso REJECT_WSS del rifiuto "no such session"),
// cosi' il GUID della RFC 6455 non vive in questo file. Per una richiesta che
// non e' un vero upgrade resta il rifiuto HTTP di prima.
function acceptWsThenClose(socket, req, code, reason) {
  const key = req && req.headers && req.headers['sec-websocket-key'];
  if (!key) return reject(socket, 502);
  try {
    REJECT_WSS.handleUpgrade(req, socket, Buffer.alloc(0), (ws) => {
      try { ws.send(JSON.stringify({ type: 'error', reason })); } catch (_) { /* socket morto */ }
      try { ws.close(code, reason); } catch (_) { /* idem */ }
    });
  } catch (_) { try { socket.destroy(); } catch (_) { /* idem */ } }
}
...
module.exports = {
  ...
  collectTopology, collectTopologyDetailed, collectLocalTopology, peerRouter, localRouter, forwardUpgrade, createHubAttachStore,
