...
function classifyShareFailure(health) {
  ...
  if (health && health.auth === 'failed') {
    return { code: 'share-peer-unauthorized', detail };
  }
  if (health && health.slotProof === true) {
    return { code: 'share-slot-proof-failed', detail };
  }
  if (health && health.transport === 'down') {
    return { code: SHARE_NOT_READY_CODE, detail };
  }
  if (health && health.reachability === 'failed') {
    return { code: 'share-peer-mismatch', detail };
  }
