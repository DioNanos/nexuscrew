...
function classifyShareFailure(health) {
  ...
  if (health.slotProof === true) {
    ...
    return health.code === 'reverse-slot-proof-unavailable'
  ...
  // Il peer risponde ma con un errore server: e' indisponibilita' temporanea,
  // non il nodo sbagliato in fondo al tunnel.
  if (Number.isInteger(health.httpStatus) && health.httpStatus >= 500) {
