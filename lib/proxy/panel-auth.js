...
// Richiesta e' entrata + la cella (2026-08-17): `/api/panel/<cella>`
...
function createPanelAuth({
  ...
  // Il Path deve valere per il mount da cui la richiesta e' ENTRATA, non
  ...
  function consumeCore(req, res, next, cellId, provenienza) {
    ...
    if (qTicket) {
      // Il mount si verifica PRIMA di toccare il ticket — un mount non
  ...
  // — SOLO consumo: per la porta pannello (sicurezza 2026-08-16). Nessuna
