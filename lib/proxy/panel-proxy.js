...
// Hop-by-hop (RFC 7230 section 6.1) + Proxy-*: never forwarded end-to-end.
...
// Il frame NON deve finire carico-bianco su un pannello morto: il 502 qui è
// UNA PAGINA (l'iframe spara `load` anche sulle risposte d'errore), quindi la
// pagina dice sé stessa a chi la monta — un postMessage con la causa. Il
// contenuto è fisso e NON sensibile (un tipo, nessun dato), quindi il
// targetOrigin è '*': può nascere un padre di origine diversa (il frame viaggia
// sulla porta pannello separata proprio per isolare le origini). La fiducia la
// mette chi RICEVE: source del frame attivo + origine del frame + tipo atteso.
function respondPaginaNonRaggiungibile(res) {
  res.writeHead(502, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' });
  res.end('<!doctype html><html><body><p>pannello non raggiungibile</p><script>('
    + 'function(){try{window.parent.postMessage({type:"nc-panel-unreachable"},"*");}catch(_){}}'
    + ')()</script></body></html>');
}
...
function createPanelProxy({ resolveCellPanel, log = () => {}, requestImpl }) {
  return async function panelProxy(req, res) {
    ...
    try {
      ...
      if (!res.headersSent) respondPaginaNonRaggiungibile(res);
      return;
    }
...
function handlePanelUpgrade({ req, socket, head, resolveCellPanel, verifyToken, authorize, log = () => {}, requestImpl }) {
  ...
  // /panel nudo sulla porta pannello dedicata (sicurezza 2026-08-16).
