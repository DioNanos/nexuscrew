...
function startThreadOnControlSocket({
  ...
  WebSocket = defaultWebSocket(), now = () => Date.now(),
}) {
  return new Promise((resolve, reject) => {
    ...
    const done = (err, value) => {
      ...
      settled = true;
      clearTimeout(timer);
      try { if (ws && ws.readyState === WebSocket.OPEN) ws.close(1000); } catch (_) { /* best effort */ }
    ...
    ws.on('open', async () => {
      try {
        ...
        const out = await request('thread/start', params);
...
function createLiveBridge({
  ...
  async function resolveForLive() {
    ...
    if (!cell || cell.active !== true) return none('host-ineligible');
