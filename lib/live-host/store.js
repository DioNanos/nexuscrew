...
function normalize(raw) {
  ...
  const hostCell = raw && typeof raw.hostCell === 'string' && CELL_ID_RE.test(raw.hostCell)
    ? raw.hostCell : null;
  // La designazione puo' puntare a una cella di un ALTRO nodo. `ownerId`
  // assente/null = cella locale (comportamento storico); presente = puntamento
  // remoto: `revision` e' quella del PROPRIETARIO (il client non la avanza,
  // rispecchia lo stato di chi possiede la cella).
  const ownerId = raw && typeof raw.ownerId === 'string' && NODE_ID_RE.test(raw.ownerId)
    ? raw.ownerId.toLowerCase() : null;
  return { revision, hostCell, ownerId };
}
...
function createLiveHostStore({ filePath, now = () => Date.now() } = {}) {
  ...
  // Registra/rispecchia il puntamento remoto dopo che il PROPRIETARIO ha
  // accettato la designazione (via federata). Nessun CAS locale: la revision
  // e' quella del proprietario, il client la rispecchia.
  function setRemoteDesignation({ ownerId, hostCell, revision }) {
    return serialize(() => {
      if (!NODE_ID_RE.test(String(ownerId || ''))) {
        return { ok: false, conflict: true, revision: 0, hostCell: null };
      }
      const cell = typeof hostCell === 'string' && CELL_ID_RE.test(hostCell) ? hostCell : null;
      const rev = Number.isInteger(revision) && revision >= 0 ? revision : 0;
      const revised = { revision: rev, hostCell: cell, ownerId: cell == null ? null : ownerId.toLowerCase(), updatedAt: now() };
      atomicWriteJson(filePath, revised);
      return { ok: true, revision: revised.revision, hostCell: revised.hostCell, ownerId: revised.ownerId, at: revised.updatedAt };
    });
  }

  // Cancella il puntamento remoto SOLO se appartiene a quel proprietario
  // (una designazione locale non toccata da un clear federato altrui).
  function clearForOwner(ownerId) {
    return serialize(() => {
      const cur = readLiveHost(filePath);
      if (cur.ownerId && NODE_ID_RE.test(String(ownerId || '')) && cur.ownerId !== String(ownerId).toLowerCase()) {
        return { ok: false, revision: cur.revision, hostCell: cur.hostCell, ownerId: cur.ownerId };
      }
      if (!cur.ownerId) return { ok: true, revision: cur.revision, hostCell: cur.hostCell, ownerId: null };
      const revised = { revision: 0, hostCell: null, updatedAt: now() };
      atomicWriteJson(filePath, revised);
      return { ok: true, revision: 0, hostCell: null, ownerId: null, at: revised.updatedAt };
    });
  }

  return { snapshot, compareAndSet, setRemoteDesignation, clearForOwner, filePath };
}
