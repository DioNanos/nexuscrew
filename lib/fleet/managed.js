...
function credentialSources(cfg, home, out) {
  let local = {};
  ...
  return {
    ...
    legacy: parseEnvFile(secretsPath(cfg, home), {}, out),
  };
}
...
function credential(profile, spec, cfg, home, out) {
  ...
  const sources = credentialSources(cfg, home, out);
...
function describeManaged(spec, cfg = {}) {
  ...
  if (!binary) {
    reason = binaryBlocked.length
      ? `client ${profile.client} non trovato ma ${binaryBlocked.length === 1 ? 'un candidato non verificabile' : 'alcuni candidati non verificabili'} (${binaryBlocked.map((b) => `${b.path} (${b.code})`).join('; ')}), non "assente"`
      : `client ${profile.client} not found`;
  } else if (!authConfigured) {
    reason = cred.source === 'unreadable'
      ? `credential ${cred.envKey} non verificabile (file presente ma illeggibile: ${credBlocked.map((b) => `${b.path} (${b.code})`).join('; ')}), non "missing"`
...
// ~/.codex/custom_provider_model_catalog.json: era una patch locale di
// SysAdmin, non un contratto; il catalog è generato dai descrittori dichiarati
