...
const { argomentiHookCodex, gateVersione, gatePiattaforma, EVENTI_CODEX } = require('./codex-hooks.js');
...
const OLLAMA_CLOUD_MODELS = Object.freeze([
  ...
  // era ancora 404). Gia` usata da celle esistenti su questo engine: senza
  // voce in OLLAMA_CONTEXT il launch ricadeva sul fallback 200000 in silenzio.
  'glm-5.3-flash',
  ...
  'deepseek-v4.1-flash',
  // Forme nude gemma4 e qwen3.5: MISURATE vive sull'API cloud
  // (/v1/chat/completions -> 200, official measured sample 2026-09-12: 22
  // live / 8 dead variants). Le varianti con tag servite in cloud restano
  // `gemma4:31b` e `qwen3.5:397b`: `gemma4:12b`/`gemma4:e4b` e
  // `qwen3.5:27b`/`:35b`/`:122b` sono 404 (fuori lista di proposito).
  // DICHIARATO 2026-09-12 https://ollama.com/library/gemma4
  // DICHIARATO 2026-09-12 https://ollama.com/library/qwen3.5
  'gemma4', 'qwen3.5',
]);
...
// La wire Responses nativa di Z.AI (https://api.z.ai/api/v1). Gli id e le
// finestre sono misurati sul listing reale del server (catalogo Codex-style,
// 2026-09-04): glm-5.3 e glm-5.3-flash a 1M. In questa catena non c'e'
// AnthMorph e i tool restano mcp__<server>__* (namespace_tools=false era una
// necessita' del proxy, non del backend).
const ZAI_RESPONSES_MODELS = Object.freeze(['glm-5.3', 'glm-5.3-flash']);
const ZAI_RESPONSES_CONTEXT = 1048576;
const OLLAMA_CONTEXT = Object.freeze({
  ...
  // gemma4 (nudo, tag default `latest`): scheda «128K context window»,
  // Input Text, Image (2026-09-12); convenzione binaria 128K -> 131072.
  'gemma4': 131072,
  // qwen3.5 (nudo, tag default `latest`): scheda «256K context window»,
  // Input Text, Image (2026-09-12); come la sorella :397b, 256K -> 262144.
  'qwen3.5': 262144,
});
...
const OLLAMA_MODEL_CAPABILITIES = Object.freeze({
  ...
  // gemma4: badge vision/tools/thinking/audio/cloud (2026-09-12); scheda Input
  // Text, Image. PARALLEL NON dichiarato: default conservativo false.
  'gemma4': Object.freeze({ input: ['text', 'image'], reasoning: true, supportsParallelToolCalls: false }),
  // qwen3.5: badge vision/tools/thinking/cloud (2026-09-12); scheda Input
  // Text, Image. PARALLEL NON dichiarato: default conservativo false.
  'qwen3.5': Object.freeze({ input: ['text', 'image'], reasoning: true, supportsParallelToolCalls: false }),
});
...
// `extraModels` e' indicizzata per ENGINE ID (e' la chiave con cui
// definitions.js e `extraModelsFrom` la costruiscono), non per profilo: per un
// engine custom `profile.id` e' la chiave condivisa (`codex-vl.custom`) e con
// quella sola un modello dichiarato per QUELL'engine non viene mai visto.
// La ricerca prova prima l'ID dell'engine e ricade sul profilo: un engine che
// dichiara i propri modelli li trova, e una dichiarazione legacy fatta sul
// profilo continua a valere.
function declaredFor(extraModels, profileId, model, engineId = null) {
  if (!extraModels || typeof extraModels.get !== 'function') return false;
  const keys = engineId && engineId !== profileId ? [engineId, profileId] : [profileId];
  return keys.some((k) => {
    const set = extraModels.get(k);
...
//
// da revisione: Map<engine, Map<id, model>> — porta il descrittore intero, non solo
...
// Stessa chiave di `declaredFor` — prima l'ID dell'engine, poi il
// profilo. L'unione deduplica per `id`: se lo stesso id compaia sia sotto
// l'engine sia sotto il profilo e' lo stesso modello, e il catalogo non deve
// contenerlo due volte.
function declaredModelsFor(extraModels, profileId, engineId = null) {
  if (!extraModels || typeof extraModels.get !== 'function') return [];
  const keys = engineId && engineId !== profileId ? [engineId, profileId] : [profileId];
  const out = [];
  const seen = new Set();
  for (const k of keys) {
    const byId = extraModels.get(k);
    if (!byId || typeof byId.values !== 'function') continue;
    for (const m of byId.values()) {
      if (m && m.id != null && !seen.has(m.id)) { seen.add(m.id); out.push(m); }
    }
  }
  return out;
}
...
//    `input_image`.
//
//  DELTA 2026-09-12 («a segnale»): il piano Go non genera oggi
//  (401 CreditsError su ogni generazione, chiave valida solo sul listing
//  /models -> 200). Le aggiunte qui sotto NON sono misure e2e: sono
//  SIGNAL-ONLY — nessun rifiuto di formato osservato sul gateway
//  (CreditsError = wire accettata, crediti mancati dopo) e id presenti sia
//  sul listing gateway (37) sia nella CLI `opencode models` 1.2.6 (27).
//  eccezione: `qwen3.8-flash` rifiuta Responses con ModelError «format
//  openai» (misurato 2026-09-12): entra SOLO in MESSAGES e CHAT.
//  Rimossi i deprecated `glm-5`, `kimi-k2.5`, `minimax-m2.5`,
//  `qwen3.5-plus`: status=deprecated upstream, assenti dalla CLI 1.2.6
//  (sul gateway restano solo come listing). Storico LIMITS conservato nel
//  commento della tabella.
const OPENCODE_GO_MESSAGES_MODELS = Object.freeze([
  // Misurati e2e 2026-08-11 (matrice completa nel commento sopra):
  'deepseek-v4-flash', 'deepseek-v4-flash-vision-exp', 'deepseek-v4-pro',
  'glm-5.2', 'glm-5.1', 'minimax-m3', 'minimax-m2.7',
  'qwen3.8-max', 'qwen3.7-max', 'qwen3.7-plus', 'qwen3.6-plus',
  // Signal-only 2026-09-12 (CreditsError, nessun rifiuto formato):
  'glm-5.3', 'glm-5.3-flash', 'longcat-2.0', 'deepseek-v4.1-flash',
  'hy4-preview', 'qwen3.8-flash',
]);
const OPENCODE_GO_RESPONSES_MODELS = Object.freeze([
  // Misurati e2e 2026-08-11:
  'deepseek-v4-flash', 'deepseek-v4-flash-vision-exp', 'deepseek-v4-pro',
  'gpt-5.6-luna', 'grok-4.6', 'glm-5.2', 'glm-5.1',
  // Signal-only 2026-09-12. qwen3.8-flash ESCLUSO di proposito: Responses
  // rifiutata (ModelError «format openai», misurato 2026-09-12).
  'glm-5.3', 'glm-5.3-flash', 'longcat-2.0', 'deepseek-v4.1-flash',
  'hy4-preview',
]);
const OPENCODE_GO_CHAT_MODELS = Object.freeze([
  ...
  'qwen3.7-plus', 'qwen3.6-plus', 'mimo-v2.5', 'mimo-v2.5-pro', 'hy3',
  'gpt-5.6-luna',
  // Signal-only 2026-09-12:
  'glm-5.3', 'glm-5.3-flash', 'longcat-2.0', 'deepseek-v4.1-flash',
  'hy4-preview', 'qwen3.8-flash',
]);
...
// Storico rimosso il 2026-09-12 (id deprecated, fuori CLI 1.2.6): glm-5
// 202752/32768 · kimi-k2.5 262144/65536 · minimax-m2.5 204800/65536 ·
// qwen3.5-plus 262144/65536. Rollback: reinserirli qui e nelle tre liste.
const OPENCODE_GO_LIMITS = Object.freeze({
  ...
  // hy3: NON allineato a models.dev (che ora dichiara output 128000):
  // fonte divergente, resta il valore trascritto 2026-08-11 finche' non
  // ricontrollato alla fonte. (divergenza segnalata in referto del 2026-09-12)
  ...
  // --- DICHIARATI models.dev 2026-09-12, NON misurati da noi: il piano Go
  // rispondeva 401 CreditsError a ogni generazione il 2026-09-12, quindi
  // nessun numero qui sotto e' stato verificato sul comportamento reale.
  // Wire: nessun rifiuto di formato osservato (signal-only), tranne
  // qwen3.8-flash su Responses (ModelError «format openai», misurato).
  ...
  'longcat-2.0': { context: 1000000, output: 131072 },
  'qwen3.8-flash': { context: 1000000, output: 131072 },
  'hy4-preview': { context: 1024000, output: 64000 },
});
...
// Massimo numero di nomi in `envPassthrough`. L'allowlist e' opt-in e per
...
// Pre- fleet.json migrates no-op: no existing cell changes resolution.
...
const CATALOG = Object.freeze([
  ...
  // EnvPassthrough, mai come valore, mai su argv. vl.native e' «usa la tua
...
// Validazione unica dell'allowlist envPassthrough (nomi, mai valori): la
...
function normalizeManagedSpec(value, { extraModels = null, engineId = null, onIssue = null } = {}) {
  // Un rifiuto senza causa produce «validazione fallita» generica e
  // l'operatore cerca a vuoto. Se il chiamante passa `onIssue`, i rifiuti
  // NOMINATI riportano campo, valore e regola prima di tornare null; senza
  // sink il comportamento resta identico (null, nessun messaggio).
  const issue = (field, valueIssue, rule) => {
    if (typeof onIssue === 'function') onIssue({ field, value: valueIssue, rule });
    return null;
  };
  ...
  if (profile.strictModels && !(profile.models || []).includes(model)
    && !declaredFor(extraModels, profile.id, model, engineId)) return issue('model', model, 'modello non dichiarato: non e\' nel catalogo del profilo e non risulta nell\'array "models" della definizione');
  ...
  if (profile.credentialEnv) {
    ...
    if (!ENV_KEY_RE.test(envKey || '')) return issue('envKey', envKey, `consentito ${ENV_KEY_RE}`);
    out.envKey = envKey;
  }
  if (profile.custom) {
    ...
    // Altri client) ma per NOME via envPassthrough — un custom vl verso un
    ...
    if (profile.client === 'vl' ? (envKey !== '' && !ENV_KEY_RE.test(envKey)) : !ENV_KEY_RE.test(envKey)) return issue('envKey', envKey, `consentito ${ENV_KEY_RE} (obbligatorio; vuoto ammesso solo per il client vl)`);
    if (!PROVIDER_ID_RE.test(providerId)) return issue('providerId', providerId, `consentito ${PROVIDER_ID_RE}`);
    if (!model || !(profile.protocols || [profile.protocol]).includes(protocol)) return issue('protocol', protocol, `protocollo non supportato dal profilo (ammessi: ${(profile.protocols || [profile.protocol]).join(', ')})`);
    Object.assign(out, { displayName, baseUrl, ...(envKey ? { envKey } : {}), protocol, providerId });
  }
  // EnvPassthrough e' un'allowlist di NOMI di variabili d'ambiente che il
...
// stata un'eccezione. Stesso principio gia' applicato in checkTermuxExec.
...
function describeManaged(spec, cfg = {}) {
  ...
  if (!normalized) {
    // da revisione: il rifiuto resta (i descrittori NON appartengono al profilo di una
  ...
  // Che sale per NOME via envPassthrough/ quando il profilo la dichiara). Il
  // verdetto configured non puo' dipendere da una credenziale che NexusCrew non
  // Possiede: il fail-closed giusto e' quello del, che NOMINA il nome mancante.
...
// davvero, e l'operatore avrebbe creduto di averli esclusi. Trovato da una revisione.
//
...
//
// Il client `claude` li riceve via `--settings` (sotto). Il client codex e
// codex-vl li riceve in forma `-c` (lib/fleet/codex-hooks.js), con la stessa
// matrice evento->stato e lo stesso scrittore — e li riceve SOLO su versioni
// provate, perche' la fiducia dell'hook dipende dall'hash di quella versione.
// Gli altri motori (pi, shell) restano senza canale: la UI li mostra «non
// verificati», che e' cio' che si sa di loro. Non si inventa uno stato che
// nessuno pubblica.
...
function cellActivityHooks(cell, cfg) {
  ...
  // QUI NON SI SCRIVE NIENTE SU DISCO. La generazione viaggia negli argv degli
  // hook e nel payload del broker; a scriverla e' `cell-exec`, che gira DENTRO
  // la sessione tmux appena creata — quindi solo per il vincitore della
  // new-session — e lo fa prima di avviare il client (cell-exec.js: la
  // generazione su disco e' quella del client che gira davvero). Scriverla qui
  // significava scriverla anche per chi stava per perdere la corsa: sul disco
  // restava la generazione del perdente, che non e' quella di nessun lancio
  // vivo, e ogni evento del vincitore veniva scartato al lettore.
  const hooks = {};
  ...
  // `script` e `generazione` escono insieme agli hooks perche' servono anche al
  // canale codex, che ricompone lo stesso comando in forma `-c`.
  return { hooks, dirSessione, script, generazione };
}
...
// il nome vince). Le SORGENTI sono le stesse dei nomi in `knownMcpServerNames`
// (config utente, suo ramo per-progetto, .mcp.json della cwd), precedute da
// eventuali file extra — per i profili Claude isolati la propria config, che
// altrimenti sparirebbe dall'enumerazione. Solo LETTURA: le definizioni non
// vengono mai modificate, riprese e ridistribuite tali quali.
function collectMcpServerDefs(home, cwd, extraFiles = []) {
  const leggi = (file) => {
    try {
      const st = fs.lstatSync(file);
      if (st.isSymbolicLink() || !st.isFile() || st.size > 8 * 1024 * 1024) return null;
      const parsed = JSON.parse(fs.readFileSync(file, 'utf8'));
      return parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed : null;
    } catch (_) { return null; /* assente o illeggibile non e' un errore qui */ }
  };
  const defs = new Map();
  const raccogli = (servers) => {
    if (servers && typeof servers === 'object' && !Array.isArray(servers)) {
      for (const [nome, def] of Object.entries(servers)) {
        if (def && typeof def === 'object' && !Array.isArray(def) && !defs.has(nome)) defs.set(nome, def);
      }
    }
  };
  for (const file of extraFiles) {
    const parsed = typeof file === 'string' ? leggi(file) : null;
    if (parsed) raccogli(parsed.mcpServers);
  }
  const utente = leggi(path.join(home, '.claude.json'));
  if (utente) {
    raccogli(utente.mcpServers);
...
// Il file MCP PER CELLA del materializzatore: contiene le definizioni dei SOLO
// server concessi, copiate tali quali dalle sorgenti (che portano `env` con
// credenziali per alcuni server). Duplicare quelle definizioni e' il prezzo
// del sottoinsieme esatto — il file per profilo unico non puo' esprimere un
// sottoinsieme per cella senza un allowlist del client, e l'allowlist esclusiva
// vuole proprio un file. Il prezzo si paga nel regime di permessi piu' stretto
// che esiste fuori dallo store credenziali: directory 0700, file 0600, tutto
// sotto ~/.nexuscrew (la stessa classe di privacy di providers.env), scritto
// atomicamente e RIGENERATO a ogni up (nessun valore accumulato, nessuna
// copia stantia), mai loggato: il contenuto non tocca nessun log né output.
// DICHARIAZIONE DI PROVENIENZA: questo file e' DERIVATO — la fonte di verita'
// restano le sorgenti originali, e la provenienza delle credenziali (doctor)
// non lo elenca e non lo considerera' mai una sorgente di conflitto.
function writeCellMcpConfig(cellId, concessi, home, defs) {
  const dir = path.join(home, '.nexuscrew', 'cell-mcp');
  try {
    const st = fs.lstatSync(dir);
    if (st.isSymbolicLink() || !st.isDirectory()) throw new Error('unsafe per-cell MCP directory');
    // Strettisce, mai allarga: permessi piu' larghi di 0700 tornano a 0700; una
    // directory resa NON scrivibile (operatore, manutenzione) resta tale — e
    // il fallimento di scrittura e' il rifiuto del launch, non un launch con
    // la superficie sbagliata.
  ...
  const servers = {};
  // Il canale operatore (nexuscrew) e' CORE: entra nel file anche se la cella
  // non lo ha nominato — senza, la cella dichiarata perderebbe `nc_*`.
  const nomi = new Set(concessi);
  if (defs.has('nexuscrew')) nomi.add('nexuscrew');
  for (const nome of [...nomi].sort()) {
    const def = defs.get(nome);
    if (!def) continue; // difesa residua: il preflight ora rifiuta i nomi dichiarati sconosciuti prima di arrivare qui
    servers[nome] = def;
  }
  const target = path.join(dir, `${cellId}.json`);
  const tmp = path.join(dir, `.${cellId}.${crypto.randomBytes(6).toString('hex')}.tmp`);
  try {
...
// La superficie codex NOTA alla config utente, letta con un parse TOML
// minimale e DELIBERATAMENTE superficie: servono solo i NOMI dei server
// (`[mcp_servers.<nome>]`) e delle skill (`[[skills.config]]` con name) da
// poter spegnere nel profilo per cella. La config utente non viene mai
// toccata: il profilo e' un LAYER di riduzione che la legge e basta.
function codexSurfaceNames(configToml) {
  const testo = typeof configToml === 'string' ? configToml : '';
  const servers = [...testo.matchAll(/^[ \t]*\[mcp_servers\.([A-Za-z0-9_-]+)\][ \t]*$/gm)].map((m) => m[1]);
  const skills = [...testo.matchAll(/\[\[skills\.config\]\]([^[]*)/g)]
    .map((m) => (m[1].match(/^[ \t]*name[ \t]*=[ \t]*"([^"]+)"/m) || [])[1])
    .filter(Boolean);
  return { servers: [...new Set(servers)], skills: [...new Set(skills)] };
}

// Il profilo per cella di codex/codex-vl (profilo v2: <CODEX_HOME>/<nome>
// .config.toml, lanciato con -p). STESSA SEMANTICA DEL CLAUDE, tradotta nei
// meccanismi nativi del client: la chiave `mcp` spegne i server NON concessi
// (enabled = false — i concessi non si ridichiarano, ereditano dal layer
// base); `ondemand` mette omit_tools_from = ["direct"] (scopribili via tool
// search, non attivi); `skills` spegne le skill NON concesse enumerate nella
// config. Chiave assente = nessuna riduzione per quella chiave. RESIDUO
// DICHIARATO: server/skill definiti fuori dalla config utente (config di
// progetto, config gestita) non sono enumerabili da qui e restano — come per
// il claude legacy, chi vuole il caso esatto dichiara e verifica. Il file e'
// atomico, 0600, rigenerato a ogni up, mai loggato; la config utente non e'
// mai modificata.
// La config utente di codex letta con gli stessi criteri del materializzatore:
// mai modificata, mai loggata; assente/symlink/oversize = catalogo vuoto.
function readCodexUserToml(codexHome) {
  try {
    const st = fs.lstatSync(path.join(codexHome, 'config.toml'));
    if (!st.isSymbolicLink() && st.isFile() && st.size <= 8 * 1024 * 1024) {
      return fs.readFileSync(path.join(codexHome, 'config.toml'), 'utf8');
    }
  } catch (_) { /* config assente: niente da enumerare, il profilo e' vuoto */ }
  return '';
}
...
// da revisione: il file .ts generato NON e' il consumatore — Pi lo E'. Un test che
...
// da revisione: `declaredModels` (opzionale) sono i descrittori dichiarati per l'ENGINE
...
// toccato dal difetto da revisione. declaredModels, quando presente, ha priorita', ed e'
...
// Per-cell prompt for the vl runtime: since 0.3.1 vl reads
// VL_SYSTEM_APPEND_FILE and composes the file on its own system (VL_SYSTEM
// keeps its "replace" meaning and is untouched: a live wrapper depends on it).
// The file carries the cell prompt and the MCP companion instructions: vl has
// no MCP client, that is the only point where the text reaches it.
...
// (fix definitivo — una revisione del pacchetto aveva bocciato la prima versione:
...
function resolveManagedEngine(engine, cell, cfg = {}) {
  ...
  // E' il difetto che la revisione ha ripreso due volte: la vista diceva di si',
  // il boot diceva di no, e nessun test guardava il boot.
  const spec = normalizeManagedSpec(engine.managed, { extraModels: cfg.extraModels || null, engineId: engine.id });
  ...
  // da revisione: i descrittori dichiarati per QUESTO profilo (client.provider — la
  ...
  // Prima i modelli dichiarati per QUESTO engine (per un custom e' la
  // chiave che conta: 'codex-vl.custom' e' condivisa), poi il fallback profilo.
  const declaredModels = declaredModelsFor(cfg.extraModels, profile.id, engine.id);
  ...
  // Identita' della cella protetta (solo codex-vl, l'unico client con un
  // REQUIRED). La DECISIONE sul canale nasce qui, dove nasce il valore di
  // CODEX_APP_SERVER_IDENTITY_REQUIRED, e viaggia col payload: se la derivasse
  // il supervisore (cell-exec aggiungeva sempre fd 3:4) i due potevano
  // divergere. Divergono, e una cella standalone muore sull'handshake.
  let identityChannel = null;
  let identityAuthorityUnavailable = '';
  // Why the vl per-cell prompt was NOT composed (runtime version that cannot
  // hold it, or a file that cannot be written safely).
  // Empty string = delivered. Surfaces with the engine resolved as vlPromptDegraded.
  ...
  if (spec.client === 'shell') {
    ...
    // Tre strade. LEGACY (niente capabilities, niente profilo): il filtro per
    // NEGAZIONE di oggi, che agisce sui nomi dei tool e lascia le sorgenti
    // dove sono. CELLA DICHIARATA con `mcp` (anche vuoto): allowlist ESCLUSIVA
    // — un file per cella con i soli server concessi (+ il canale operatore,
    // sempre) e `--strict-mcp-config`, che esclude anche cio' che il deny non
    // sa enumerare (plugin, connettori). CHIAVE `mcp` ASSENTE ma cella
    // dichiarata (solo skills/ondemand): comportamento di sempre per QUESTA
    // chiave — l'alias `cell.mcp` resta il filtro per negazione, in linea con
    // l'override sostitutivo per chiave del modello.
    // FAIL-CLOSED: se il file per cella non si scrive la cella NON parte.
    // Partire senza la allowlist esporrebbe l'intera superficie proprio dove
    // l'operatore l'ha ristretta: peggio della legacy, e senza che nessuno
    // l'abbia chiesto.
    ...
    if (eff.legacy || eff.declared.mcp === undefined) {
      ...
      let file;
      try {
        const extraFiles = typeof env.CLAUDE_CONFIG_DIR === 'string'
          ? [path.join(env.CLAUDE_CONFIG_DIR, '.claude.json')] : [];
        const defs = collectMcpServerDefs(home, cell?.cwd, extraFiles);
        // Un nome dichiarato che il client non definisce sparisce in silenzio
        // dal file strict: rifiuta qui, prima di scrivere qualsiasi cosa.
        const sconosciute = eff.declared.mcp.filter((n) => !defs.has(n));
        if (sconosciute.length) {
          const motivo = `capability mcp sconosciuta al client: ${sconosciute.join(', ')}`;
          return { ok: false, reason: motivo, mcpCellRefused: motivo, mcpCellRefusedCode: 'CAPABILITY_UNKNOWN_NAME' };
        }
        file = writeCellMcpConfig(String(cell?.id || 'cella'), eff.declared.mcp, home, defs);
      } catch (e) {
        const motivo = `file MCP per cella non scrivibile: ${e.message}`;
    ...
    // Canale di attivita' per le celle codex: gli stessi eventi che le celle
    // Claude pubblicano via `--settings`, qui come hook `-c` con la fiducia che
    // il client pretende (senza, apre un dialogo di revisione e la cella resta
    // ferma su una domanda). Vince SOLO su versioni provate: su ogni altra non
    // si inietta niente e la cella torna «non verificato» come prima — un
    // degrado dichiarato, mai un dialogo che blocca il turno.
    {
      // `attivita` e' dichiarata nel ramo claude e qui non e' in scope: si
      // ricalcola. E' pura e non scrive niente su disco: calcola gli stessi
      // argv (stessa generazione) per il client codex.
      const attivita = cellActivityHooks(cell, cfg);
      // Il gate riceve il binario RISOLTO per questa cella (`info.binary`,
      // lo stesso che finisce in argv a fine funzione), non il nome del
      // client: col nome si interrogherebbe il PATH del processo, che puo'
      // essere un eseguibile DIVERSO da quello che la cella lancera'. La
      // guardia esiste per non far bloccare la cella su un dialogo di
      // revisione, e una guardia che guarda un altro binario da' il via
      // libero sbagliato: e' il caso che deve impedire.
      const piattaforma = gatePiattaforma(cfg);
      const gate = piattaforma.ok ? gateVersione(spec.client, info.binary, cfg) : piattaforma;
      if (!gate.ok) {
        if (typeof cfg.log === 'function') cfg.log(`hook codex non iniettati: ${gate.reason}`);
      } else if (attivita) {
        args.push(...argomentiHookCodex(EVENTI_CODEX,
          (evento) => comandoHook(evento, attivita.script, attivita.dirSessione, attivita.generazione)));
        // Qui si dichiara che QUESTA cella ha un canale di attivita': e' il
        // `dir` che fa viaggiare verso il supervisore il dove e il
        // con-che-generazione dell'evento di uscita (runtime.js) e la
        // scrittura di `activity.gen` da parte di cell-exec, e senza di esso un
        // `Stop` dell'ultimo turno sopravviverebbe al riavvio interno del
        // client.
        activityDir = attivita.dirSessione;
      }
    }
    if (profile.localProvider) args.push('--oss', '--local-provider', profile.localProvider);
    ...
    } else if (spec.provider === 'zai-a' || spec.provider === 'zai-p') {
      // Wire Responses nativa di Z.AI — niente AnthMorph e nessuna
      // traduzione: previous_response_id e streaming restano semantica nativa.
      // namespace_tools=false NON serve (era del proxy); i parametri di
      // robustezza sono quelli degli altri wrapper Responses misurati, e il
      // catalogo porta i metadata reali del server (context window 1M).
      const zaiId = `zai_${spec.provider.slice(-1)}`;
      const zaiEnvKey = spec.provider === 'zai-a' ? 'ZAI_API_KEY_A' : 'ZAI_API_KEY_P';
      env[zaiEnvKey] = cred.value;
      args.push(...codexProviderArgs(zaiId, 'Z.AI (Responses nativo)', profile.endpoint, zaiEnvKey));
      args.push('-c', `model_providers.${zaiId}.request_max_retries=4`);
      args.push('-c', `model_providers.${zaiId}.stream_max_retries=10`);
      args.push('-c', `model_providers.${zaiId}.stream_idle_timeout_ms=300000`);
      const zaiCatalog = path.join(__dirname, 'catalogs', 'zai-responses.json');
      if (fs.existsSync(zaiCatalog)) {
        args.push('-c', `model_catalog_json=${JSON.stringify(zaiCatalog)}`);
        args.push('-c', `model_context_window=${ZAI_RESPONSES_CONTEXT}`);
      }
      ...
      // da revisione: onora `models` (engine definition, già validato) come gli altri
    ...
    // Materializzatore per cella (stesse regole del ramo claude, meccanismi
    // nativi del client): profilo v2 sotto CODEX_HOME lanciato con -p. Il
    // profilo RIDUCE la superficie della config utente (mai toccata);
    // FAIL-CLOSED come il claude: profilo non scrivibile = launch rifiutato,
    // mai una cella con la superficie intera dove l'operatore ha ristretto.
    {
      const { effectiveCapabilities } = require('./definitions.js');
      const eff = effectiveCapabilities(cell || {}, { capabilityProfiles: cfg.capabilityProfiles || null });
      const daMaterializzare = eff.declared.mcp !== undefined || eff.declared.skills !== undefined || eff.declared.ondemand !== undefined;
      if (!eff.legacy && daMaterializzare) {
        // Stessa regola del ramo claude: un nome dichiarato che la config
        // utente non definisce sparisce in silenzio dal profilo — rifiuta
        // qui, prima di scrivere qualsiasi cosa. Catalogo vuoto (config
        // assente/vuota) + nomi dichiarati = nomi sconosciuti; `mcp: []`
        // resta valido (niente nomi da confrontare).
        const { servers, skills } = codexSurfaceNames(readCodexUserToml(path.join(home, '.codex')));
        const parti = [];
        const sconosciuteMcp = (eff.declared.mcp || []).filter((n) => !servers.includes(n));
        const sconosciuteOndemand = (eff.declared.ondemand || []).filter((n) => !servers.includes(n));
        const sconosciuteSkills = (eff.declared.skills || []).filter((n) => !skills.includes(n));
        if (sconosciuteMcp.length) parti.push(`capability mcp sconosciuta al client: ${sconosciuteMcp.join(', ')}`);
        if (sconosciuteOndemand.length) parti.push(`capability ondemand sconosciuta al client: ${sconosciuteOndemand.join(', ')}`);
        if (sconosciuteSkills.length) parti.push(`capability skills sconosciuta al client: ${sconosciuteSkills.join(', ')}`);
        if (parti.length) {
          const motivo = parti.join('; ');
          return { ok: false, reason: motivo, mcpCellRefused: motivo, mcpCellRefusedCode: 'CAPABILITY_UNKNOWN_NAME' };
        }
        let file;
        try {
          file = writeCellCodexProfile(String(cell?.id || 'cella'), eff.declared, path.join(home, '.codex'));
        } catch (e) {
          const motivo = `profilo per cella non scrivibile: ${e.message}`;
          return { ok: false, reason: motivo, mcpCellRefused: motivo, mcpCellRefusedCode: 'CAPABILITY_PROFILE_UNWRITABLE' };
        }
        args.push('-p', path.basename(file).replace(/\.config\.toml$/, ''));
      }
    }
    ...
    // VL/Vivling runtime TUI: auth is handled by the runtime in every variant
    // (config.toml / VL_API_KEY by name via envPassthrough), never
    // credentials on argv. The model chosen in the UI travels via VL_MODEL
    // and the per-cell prompt via a file on the runtime system
    // (VL_SYSTEM_APPEND_FILE, gate below); `vl --profile` remains
    // the operator's choice. No --model on argv: the TUI starts without arguments.
    ...
    // Per-cell prompt: file composed on the runtime system
    // (VL_SYSTEM_APPEND_FILE). The version gate lives in code, not in hope:
    // below 0.3.1 the variable is silently ignored, and a cell starting without
    // its own prompt without saying so is the defect this branch closes,
    // not the remedy. Degrading = NOT composing the env (never a promise the
    // runtime discards) + declaring the reason.
  ...
  if (spec.client === 'codex-vl') {
    // REQUIRED=1 ha senso SOLO quando l'authority mode e' attivo e
    // l'authority e' costruibile (credenziali presenti). Fuori di quello il
    // relay non puo' emettere subject: forzare 1 produce daemon che non
    // partono (fail-closed senza canale) o attach con `revoked` sistematico.
    const identityMode = cfg.fleetIdentityMode
      || (cfg.fleet && cfg.fleet.identity && cfg.fleet.identity.mode)
      || 'legacy';
    const identityCredentialsDistinct = (a, b) => (a && b
      ? Buffer.from(String(a)).compare(Buffer.from(String(b))) !== 0
      : false);
    const identityAuthorityReady = !!(cfg.identityAuthority
      || (cfg.identityDaemonCredential && cfg.identityLauncherCredential
        && identityCredentialsDistinct(cfg.identityDaemonCredential, cfg.identityLauncherCredential)));
    const identityAuthorityFault = cfg.identityAuthorityFault
      || (cfg.identityDaemonCredential && cfg.identityLauncherCredential
        && !identityCredentialsDistinct(cfg.identityDaemonCredential, cfg.identityLauncherCredential)
        ? 'daemon and launcher credentials are identical' : null);
    const logLine = typeof cfg.log === 'function' ? cfg.log : null;
    ...
    const authorityActive = identityMode === 'authority' && identityAuthorityReady;
    // Authority configurata ma non costruibile (credenziali assenti, identiche
    // o con permessi sbagliati): la cella protetta NON parte. Esportare '0' con
    // una riga di log era un degrado silenzioso — il child partiva standalone e
    // il motivo viveva solo nel log. Il rifiuto lo applica il launcher, col
    // codice IDENTITY_AUTHORITY_UNAVAILABLE (runtime.js); qui si dichiara il
    // MOTIVO, senza valori.
    const authorityUnavailable = identityMode === 'authority' && !identityAuthorityReady
      ? (identityAuthorityFault || 'authority credentials missing')
      : '';
    if (hasDefinitionOverride && typeof definitionEnv[CODEX_APP_SERVER_IDENTITY_REQUIRED] === 'string') {
      ...
      identityChannel = env[CODEX_APP_SERVER_IDENTITY_REQUIRED] === '1';
      if (!authorityActive && identityChannel && logLine) {
        logLine(`fleet identity: override definition force CODEX_APP_SERVER_IDENTITY_REQUIRED=1 `
          + `but identity mode is '${identityMode}' without authority credentials`);
      }
      ...
      identityChannel = env[CODEX_APP_SERVER_IDENTITY_REQUIRED] === '1';
      if (!authorityActive && identityChannel && logLine) {
        logLine(`fleet identity: override runtime force CODEX_APP_SERVER_IDENTITY_REQUIRED=1 `
          + `but identity mode is '${identityMode}' without authority credentials`);
      }
    } else if (authorityActive) {
      env[CODEX_APP_SERVER_IDENTITY_REQUIRED] = CODEX_APP_SERVER_IDENTITY_REQUIRED_DEFAULT;
      identityChannel = env[CODEX_APP_SERVER_IDENTITY_REQUIRED] === '1';
    } else if (authorityUnavailable) {
      // Nessuna esportazione: il lancio e' rifiutato prima dello spawn, quindi
      // il valore non arriverebbe mai a un child. Un override esplicito (rami
      // sopra) resta una scelta dell'operatore e non entra qui.
      identityChannel = false;
      identityAuthorityUnavailable = authorityUnavailable;
      if (logLine) {
        logLine('fleet identity: protected cell launch refused — authority configured but unavailable ('
          + authorityUnavailable + ')');
      }
    } else {
      env[CODEX_APP_SERVER_IDENTITY_REQUIRED] = '0';
      identityChannel = false;
      if (logLine) {
        logLine(`fleet identity: CODEX_APP_SERVER_IDENTITY_REQUIRED=0 (identity mode '${identityMode}')`);
      }
    }
  }
  ...
  // send-keys does not apply: the deliverBootstrapPrompt classifier covers
  // only kimi/claude, and vl starts immediately without waits.
  ...
  // Resolve envPassthrough AFTER the provider branches. Each declared name is
  ...
  return { ok: true, info, activityDir, engine: {
    ...
    // il payload resta quello di prima e il supervisore conserva il default.
    ...(identityChannel === null ? {} : { identityChannel }),
    ...(identityAuthorityUnavailable ? { identityAuthorityUnavailable } : {}),
...
function publicCatalog() {
  return CATALOG.filter((p) => !p.legacy && (p.core || p.default || p.custom)).map((p) => ({
    ...
    // Solo il client claude riceve MCP gestito da NexusCrew (cellMcpArgs
...
module.exports = {
  ...
  codexSurfaceNames, writeCellCodexProfile,
  writeVlCellPrompt, vlSystemAppendGate,
  extraModelsFrom, declaredModelsFor, declaredFor,
