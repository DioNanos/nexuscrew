...
const MANAGED_KEYS = new Set(['client', 'provider', 'credentialProfile', 'model', 'permissionPolicy', 'credentialSource', ...CUSTOM_KEYS]);
...
function normalizeManagedSpec(value) {
  ...
  const credentialSource = value.credentialSource === undefined ? 'auto' : value.credentialSource;
  if (!CREDENTIAL_SOURCES.includes(credentialSource)) return null;
  ...
  // fleet.json (no credentialSource) migrates truly no-op: no added field, no
  // change in resolution. Explicit environment|nexuscrew-store are recorded.
  if (credentialSource !== 'auto') out.credentialSource = credentialSource;
...
function credential(profile, spec, cfg, home) {
  ...
  const policy = spec && CREDENTIAL_SOURCES.includes(spec.credentialSource) ? spec.credentialSource : 'auto';
...
function applyStoreNeutralization(env, spec, profile) {
  if (!env || !spec || spec.credentialSource !== 'nexuscrew-store') return env;
