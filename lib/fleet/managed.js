...
// la configurazione utente e, se c'e', il file di progetto nella cwd della
// cella. Non si legge nient'altro di quei file: servono solo i nomi.
function knownMcpServerNames(home, cwd) {
  ...
  const leggi = (file) => {
    ...
    } catch (_) { /* un file assente o illeggibile non e' un errore qui */ }
  };
  leggi(path.join(home, '.claude.json'));
  if (typeof cwd === 'string' && cwd) leggi(path.join(cwd, '.mcp.json'));
  return nomi;
}
...
// client installato: un `deny` piu' largo VINCE su un `allow` piu' specifico —
// `deny:["mcp__*"]` insieme a `allow:["mcp__nexuscrew"]` non espone nulla.
//
...
// (configurazione utente + file di progetto nella cwd). RESIDUO DICHIARATO: un
// server che arrivasse da una sorgente non enumerata qui non verrebbe negato.
// Non lo nascondo dietro un nome rassicurante: chi vuole la garanzia forte usa
// `mcp: []` e concede da li'.
//
