...
// D2: onora `models` (definizione engine, già validato da parseModel) nel ramo
// custom, come gli altri provider (opencode-go/ollama-cloud/alibaba/openrouter).
...
// Se `models` non è dichiarato -> null (comportamento invariato, NESSUNA regressione
// per chi non lo usa). NON consacra ~/.codex/custom_provider_model_catalog.json:
// era una patch locale di SysAdmin, non un contratto; il catalog è generato dai
// descrittori dichiarati in fleet.json. I valori enum/default rispecchiano i
// cataloghi spediti (validati dal test fleet-catalog-schema).
function customCatalogFor(spec, model, home) {
  const models = Array.isArray(spec && spec.models) ? spec.models : [];
...
function resolveManagedEngine(engine, cell, cfg = {}) {
  ...
  if (spec.client === 'shell') {
    ...
    if (profile.localProvider) args.push('--oss', '--local-provider', profile.localProvider);
      ...
      const customMeta = customCatalogFor(spec, model, home);
      if (customMeta) {
