...
//
// Security model — unchanged versus builtin.js:
...
//    (dead session) it does not type.
//  - redactSecrets/sanitizeEarlyDiagnostic: stderr/stdout of failed tmux
...
// Minimal service-controlled env. HARD allowlist: the definitions
...
// Arbitrary API detail for the response body; `cause` is the OPTIONAL
...
// Redaction marker: stderr/stdout of failed tmux commands
...
// of the definitions replaced by '‹redacted›'. Covered secrets:
...
// tmux identity migration: the whole plan is built and validated first.
...
// send-keys prompt character policy: printable + \t \n \r allowed;
...
function composeClientInvocation(engine, cell) {
  ...
  // SECURITY: promptMode 'flag' puts the prompt in ARGV -> it is visible
...
// Contratto submitToSession: output session_name + pane_dead + pane_id,
...
async function securePaste(tmuxBin, session, text, { env, exec, tmpdir, target, fsImpl } = {}) {
  ...
  try {
    ...
    loaded = true;
    // Da qui in poi il paste e' stato TENTATO: qualunque throw/rejection
    ...
    // Completo e un retry duplicherebbe il prompt.
    try {
...
// Custom promptMode 'send-keys': paste SENZA Enter, invariato — 0.8.47).
// Readiness best-effort: se la sessione non e' viva quando paste-iamo (command
// already exited) does not type. Returns {injected, reason}.
