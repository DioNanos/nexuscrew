...
function createBuiltinRuntime(ctx) {
  ...
  async function cellStatus() {
    ...
    const cells = cache.defs.cells.map((c) => {
      ...
      return {
        ...
        cwd: resolveCwd(c.cwd, home) || null,
  ...
  async function up(cellId /* , { engine, boot } = {} */) {
    ...
    ticket = await launchBroker.issue({
        ...
        ...(leaseInfo ? { lease: { cellId: cell.id, launchEpoch: leaseInfo.launchEpoch, capability: leaseInfo.capability, stablePath: leaseInfo.stablePath } } : {}),
        supervise: {
