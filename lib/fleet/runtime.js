...
function createBuiltinRuntime(ctx) {
  ...
  async function up(cellId /* , { engine, boot } = {} */) {
    ...
    if (engine.managed) {
      ...
      // V-69: una cella vl il cui runtime non regge il prompt per-cella
      // (VL_SYSTEM_APPEND_FILE, 0.3.1+) DEGRADA dichiarando — stessa forma di
      // readinessDegraded (DEC1): la cella parte senza la propria identita' e
      // l'operatore lo vede (log qui + flag nel risultato, consumato dalla UI).
      // Mai in silenzio: la partenza silenziosa senza prompt e' il difetto che
      // V-69 chiude.
    ...
    return {
      ...
      // V-69: prompt di cella non consegnato a una cella vl (runtime vecchio o
      // file non scrivibile). Booleano strict come readinessDegraded: il
      // payload puo' arrivare da un nodo federato e il testo e' i18n locale.
