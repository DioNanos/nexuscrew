...
function createBuiltinRuntime(ctx) {
  ...
  async function up(cellId /* , { engine, boot } = {} */) {
    ...
    if (engine.managed) {
      const resolved = resolveManagedEngine(engine, cell, { ...cfg, home, extraModels: extraModelsFrom(defs), capabilityProfiles: defs.capabilityProfiles || null });
    ...
    ticket = await launchBroker.issue({
        ...
        ...(launchEngine.promptMode === 'send-keys' && cell.prompt ? {
          restartPrompt: {
            ...
            // Send-keys conservano il reinject legacy senza Enter (0.8.47).
...
module.exports = { createBuiltinRuntime, findCell, findEngine, identityLaunchRefusal, STATUS_TTL_MS };
