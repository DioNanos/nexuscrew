...
// Contract rev1: the premise and the items below.
//
...
//
// La chiave verifier vive in un file DEDICATO separato 0o600, distinto
...
//    sistema.
// Lo stato durevole contiene l'identificativo e l'impronta, MAI il
...
// La codifica canonica e' length-prefixed con proofKind come primo tag.
...
// Expiry = issuedAt + 60s, calcolabile all'emissione. «Ultimo-live+60»
//    e' la proprieta' che si vuole, non la formula che si scrive: non e'
//    calcolabile nel momento in cui il proof va firmato.
// Fail-closed sulla verifica. La verifica prova TUTTE le chiavi vive
//    (oggi una sola: la rotazione e' sospesa per scelta dichiarata, contratto
//; la forma e' gia' quella a due chiavi di).
// La verifica rende osservabile QUALE chiave ha firmato (keyId), cosi'
...
// Vita di un proof emesso. Il refresh gira ogni 20s (REFRESH_MS): un proof
...
// Campi firmati per proofKind, IN ORDINE, proofKind primo. Per kind la
...
// Opzionali.: nel kind 'lease' l'identita' del lease e' 'leaseId' — non
...
const KIND_FIELDS = Object.freeze({
  ...
  // Tupla del child: autorizza register/refresh/recovery. incarnationId e'
  // Per-registration, mai globale.
...
function loadOrCreateVerifier({ dir, fsImpl = fs, log = () => {}, now = Date.now } = {}) {
  ...
  // Il meta persiste id + impronta (MAI il segreto) per diagnostica e
  // Osservabilita'. Best-effort: se e' assente o divergente si riscrive;
...
// Fail-closed: ogni difetto e' un motivo, non un'eccezione. `verifiers` e'
// La lista delle chiavi vive (oggi una; -ready per due). `expect` porta i
...
// E' la finestra di recovery del child — un proof la cui firma e' valida e
...
function verifyProof(verifiers, candidate, { now = Date.now, expect = {}, graceMs = 0 } = {}) {
  ...
  // ExpiresAt non e' un campo qualunque: deve essere ESATTAMENTE
  ...
  // Firma contro OGNI chiave viva: la prima che passa vince (/).
