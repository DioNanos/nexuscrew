'use strict';
const express = require('express');
const { codeOf, phaseOf } = require('./causes.js');

// Fallback difensivo per un adapter builtin incompleto nei test. Il provider di
// prodotto dichiara sempre la propria capability list.
const DEFAULT_CAPS = ['status', 'up', 'down', 'engine', 'boot'];

function capList(fleet) {
  return typeof fleet.capabilities === 'function' ? fleet.capabilities() : DEFAULT_CAPS;
}

// A route for a method unsupported by the provider returns 501
// (never ambiguous 404/502). The builtin native errors (400/403/409) pass
// through the guard with their own status.
function requireCap(fleet, cap) {
  if (!capList(fleet).includes(cap)) {
    const e = new Error('not supported by this fleet provider');
    e.status = 501;
    throw e;
  }
}

// Router /api/fleet — fleetP è una Promise<Fleet> (createServer non diventa
// async): ogni handler attende la resolve; unavailable → 404 sui comandi.
function fleetRoutes(fleetP, cfg = {}) {
  const r = express.Router();
  const smallJson = express.json({ limit: '4kb' });
  const restoreJson = express.json({ limit: '256kb' });
  r.use((req, res, next) => (req.path === '/restore-cells' || req.path === '/restore-engines' ? restoreJson : smallJson)(req, res, next));

  const readonly = () => (cfg.readonlyDefault === true || process.env.NEXUSCREW_READONLY === '1');
  const diagnostics = cfg.diagnostics || null;
  const safeCell = (body) => {
    const value = body && typeof body.cell === 'string' ? body.cell : '';
    return /^[A-Za-z0-9._-]{1,32}$/.test(value) ? value : '';
  };
  const emit = (level, code, message, meta) => {
    if (diagnostics && typeof diagnostics.record === 'function') diagnostics.record(level, 'fleet', code, message, meta);
  };

  const guard = (fn, opts = {}) => async (req, res) => {
    try {
      // READONLY e' un gate di prodotto oltre che del provider built-in.
      if (opts.mutate && readonly()) return res.status(403).json({ error: 'READONLY: mutazione fleet bloccata' });
      const fleet = await fleetP;
      if (!fleet.available) return res.status(404).json({ error: 'fleet non disponibile' });
      const body = req.body || {};
      if (opts.action) emit('info', 'FLEET_ACTION_STARTED', 'Fleet action started', {
        action: opts.action, cell: safeCell(body), state: 'starting',
      });
      const result = await fn(fleet, body);
      if (opts.action) emit('info', 'FLEET_ACTION_COMPLETED', 'Fleet action completed', {
        action: opts.action, cell: safeCell(body), state: opts.action === 'down' ? 'stopped' : 'ready',
      });
      if (opts.action && typeof cfg.onFleetStateChange === 'function') {
        try {
          cfg.onFleetStateChange({ cell: safeCell(body), active: opts.action !== 'down', action: opts.action });
        } catch (_) { /* observability never breaks the action */ }
      }
      res.json(result);
    } catch (e) {
      if (opts.action) {
        const match = String(e && e.message || '').match(/cell spawn failed:\s+([A-Z][A-Z0-9_]{0,31})\s+([A-Za-z0-9._+-]{1,128})/);
        if (match) emit('error', 'CELL_SPAWN_FAILED', 'Cell client spawn failed', {
          action: opts.action, cell: safeCell(req.body), errno: match[1], client: match[2], status: e.status || 500,
        });
        else emit('warn', 'FLEET_ACTION_FAILED', 'Fleet action failed', {
          action: opts.action, cell: safeCell(req.body), state: 'failed',
          status: e.status || 500,
          // Cause-preserving: bounded enum only (lib/fleet/causes.js). The
          // cause triple is {status, code, phase}; no free message, cwd/path,
          // argv, env, prompt, token or credential ever enters here. Untagged /
          // legacy errors degrade to the bounded UNKNOWN fallback.
          code: codeOf(e.fleetCode), phase: phaseOf(e.fleetPhase),
        });
      }
      res.status(e.status || 500).json({
        error: String(e.message || e),
        ...(e.data || {}),
        // Structured, bounded cause on the HTTP error too: stable enum only,
        // never cwd/path, argv, env, prompt, token or credentials. Untagged
        // errors (no fleetCode) keep the historical response shape.
        ...(e.fleetCode ? { code: codeOf(e.fleetCode), phase: phaseOf(e.fleetPhase) } : {}),
      });
    }
  };

  // /status also exposes `provider` and `capabilities`, besides the
  // historical fields (available/cells/engines).
  r.get('/status', async (_req, res) => {
    try {
      const fleet = await fleetP;
      if (!fleet.available) {
        return res.json({
          available: false,
          provider: fleet.provider || 'disabled',
          bootOwner: 'none', // provider unavailable -> no boot owner
          capabilities: capList(fleet),
          ...(fleet.reason ? { reason: fleet.reason } : {}),
          // Il codice macchina del boot bloccato viaggia fino al client.
          // La policy di lettura decide su QUESTO, non sulla prosa del reason
          // (la prosa resta ripiego per i reason che un codice non ce l'hanno).
          // Senza campo dal provider, la risposta semplicemente non lo ha.
          ...(fleet.migrationCode ? { migrationCode: fleet.migrationCode } : {}),
        });
      }
      const st = await fleet.status();
      const provider = st.provider || fleet.provider || 'builtin';
      const bootOwner = st.bootOwner || (provider === 'disabled' ? 'none' : 'builtin');
      res.json({
        ...st,
        provider,
        bootOwner,
        capabilities: capList(fleet),
      });
    } catch (e) { res.status(500).json({ error: String(e.message || e) }); }
  });

  // /up: il built-in (capability 'edit') è definitions-driven — se il body porta
  // un engine lo persiste sulla cella (f.engine) PRIMA di up, perché l'engine
  // dichiarato vince sull'override runtime (ignored dal builtin.up).
  r.post('/up', guard(async (f, b) => {
    const cell = String(b.cell || '');
    if (b.engine && capList(f).includes('edit')) {
      const opts = { model: typeof b.model === 'string' ? b.model : '' };
      if (typeof b.permissionPolicy === 'string') opts.permissionPolicy = b.permissionPolicy;
      await f.engine(cell, String(b.engine), opts);
    }
    if (Object.prototype.hasOwnProperty.call(b, 'boot') && capList(f).includes('edit') && capList(f).includes('boot')) {
      await f.boot(cell, b.boot === true);
    }
    return f.up(cell, { engine: b.engine, boot: !!b.boot });
  }, { mutate: true, action: 'up' }));
  r.post('/down', guard(async (f, b) => {
    const cell = String(b.cell || '');
    if (b.boot === true && capList(f).includes('edit') && capList(f).includes('boot')) await f.boot(cell, false);
    return f.down(cell, { boot: !!b.boot });
  }, { mutate: true, action: 'down' }));
  r.post('/restart', guard((f, b) => { requireCap(f, 'restart'); return f.restart(String(b.cell || '')); }, { mutate: true, action: 'restart' }));
  r.post('/engine', guard((f, b) => {
    const opts = { model: typeof b.model === 'string' ? b.model : '' };
    if (typeof b.permissionPolicy === 'string') opts.permissionPolicy = b.permissionPolicy;
    return f.engine(String(b.cell || ''), String(b.engine || ''), opts);
  }, { mutate: true }));
  r.post('/boot', guard((f, b) => {
    requireCap(f, 'boot');
    return f.boot(String(b.cell || ''), b.enabled === true);
  }, { mutate: true }));

  // --- Extension: schema + define/edit/remove (engine and cell) ---
  // Every route negotiates the provider capability: missing -> 501.
  r.get('/schema', guard((f) => { requireCap(f, 'schema'); return f.schema(); }));
  r.get('/definitions', guard((f) => { requireCap(f, 'definitions'); return f.definitions(); }));
  r.get('/credentials/status', guard((f) => {
    requireCap(f, 'credentials'); return f.credentialStatus();
  }));
  r.post('/credentials/set', guard((f, b) => {
    requireCap(f, 'credentials');
    return f.setLocalCredential(String(b.envKey || ''), typeof b.value === 'string' ? b.value : '');
  }, { mutate: true }));
  r.post('/credentials/remove', guard((f, b) => {
    requireCap(f, 'credentials'); return f.removeLocalCredential(String(b.envKey || ''));
  }, { mutate: true }));
  // Prova di un modello sull'API del suo profilo. E' nell'allowlist federata:
  // chi e' accoppiato amministra il nodo remoto come farebbe in locale, e un
  // modello che non si puo' provare a distanza si finisce per provarlo facendo
  // partire la cella — cioe' nel modo peggiore. La credenziale si risolve dove
  // la prova gira e non attraversa il router.
  // Qui non e' gated da READONLY, per la stessa ragione di `nodes test`: e' una
  // diagnosi, non una mutazione. Sulla via FEDERATA invece READONLY la blocca,
  // perche' li' la richiesta autenticata partirebbe da un nodo dichiarato di
  // sola lettura su comando di qualcun altro.
  r.post('/model-test', guard((f, b) => {
    requireCap(f, 'model-test');
    if (typeof f.testModel !== 'function') {
      const e = new Error('not supported by this fleet provider'); e.status = 501; throw e;
    }
    return f.testModel(String(b.engine || ''), String(b.model || ''));
  }));
  r.post('/define-model', guard((f, b) => {
    requireCap(f, 'define');
    if (typeof f.defineModel !== 'function') { const e = new Error('not supported by this fleet provider'); e.status = 501; throw e; }
    return f.defineModel(b.def);
  }, { mutate: true }));
  r.post('/remove-model', guard((f, b) => {
    requireCap(f, 'remove');
    if (typeof f.removeModel !== 'function') { const e = new Error('not supported by this fleet provider'); e.status = 501; throw e; }
    return f.removeModel(String(b.id || ''), String(b.engine || ''));
  }, { mutate: true }));
  r.post('/edit-model', guard((f, b) => {
    requireCap(f, 'edit-model');
    if (typeof f.editModel !== 'function') { const e = new Error('not supported by this fleet provider'); e.status = 501; throw e; }
    return f.editModel(String(b.id || ''), String(b.engine || ''), b.patch);
  }, { mutate: true }));
  r.post('/define-engine', guard((f, b) => { requireCap(f, 'define'); return f.defineEngine(b.def); }, { mutate: true }));
  r.post('/edit-engine', guard((f, b) => { requireCap(f, 'edit'); return f.editEngine(b.id, b.patch, b.envChanges); }, { mutate: true }));
  r.post('/remove-engine', guard((f, b) => { requireCap(f, 'remove'); return f.removeEngine(b.id); }, { mutate: true }));
  r.post('/define-cell', guard((f, b) => { requireCap(f, 'define'); return f.defineCell(b.def); }, { mutate: true }));
  r.post('/edit-cell', guard((f, b) => { requireCap(f, 'edit'); return f.editCell(b.id, b.patch); }, { mutate: true }));
  r.post('/remove-cell', guard((f, b) => { requireCap(f, 'remove'); return f.removeCell(b.id, { stop: b.stop === true }); }, { mutate: true }));
  r.post('/restore-cells', guard((f, b) => { requireCap(f, 'restore'); return f.restoreCells(b.cells); }, { mutate: true }));
  r.post('/restore-engines', guard((f, b) => {
    requireCap(f, 'restore');
    if (typeof f.restoreEngines !== 'function') { const e = new Error('not supported by this fleet provider'); e.status = 501; throw e; }
    return f.restoreEngines(b.engines, { overwrite: b.overwrite === true, models: b.models });
  }, { mutate: true }));
  // Riconciliazione sessione tmux esistente (cella Fleet legacy orfana) in cella
  // gestita fleet.json.
  r.post('/import-cell', guard(async (f, b) => { requireCap(f, 'import'); return f.importCell(b || {}); }, { mutate: true }));

  r.use((err, _req, res, _next) => {
    if (err && (err.type === 'entity.too.large' || err.status === 413)) {
      return res.status(413).json({ error: 'body troppo grande', code: 'body-too-large' });
    }
    if (err instanceof SyntaxError) return res.status(400).json({ error: 'JSON non valido', code: 'invalid-json' });
    return res.status(err.status || 400).json({ error: String(err.message || err) });
  });

  return r;
}

module.exports = { fleetRoutes };
