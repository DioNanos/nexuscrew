...
async function createBuiltinFleet(cfg = {}) {
  ...
  // leaseManager/launchBroker e di leaseManager.boot(). Invariante strutturale:
  // leaseManager non esiste finche' la Fleet non e' confermata disponibile, quindi
  // NESSUN endpoint lease resta vivo su un early return (off/blocked). Un gate
  // availability futuro va qui dentro, non dopo: qui non c'e' ancora nulla da chiudere.
  // Regge per il prossimo early return PER COSTRUZIONE (leaseManager non esiste nei
  // gate), non per memoria di chi scrive il ramo — ed e' per questo che batte
  // «chiudi su ogni early return», che scaricherebbe l'obbligo sul futuro.
