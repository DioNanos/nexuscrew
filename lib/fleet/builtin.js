  ...
  const launchBroker = cfg.launchBroker || createLaunchBroker({
    ...
    ...(leaseManager ? {
      onLease: (socket, lease) => {
        // P1-3 (reaudit dd38c83): attachInitial restituisce false se la persistenza
        // del bind fallisce. Il payload lease e' gia' stato consegnato al supervisore
        // (il broker scrive PRIMA di chiamare onLease). Se consumiamo il false e
        // chiudiamo la socket, il supervisore vede EOF e non resta con un lease-client
        // connesso a un manager che non ha il lease (state=none). Senza questo, la
        // socket resta aperta su entrambi i lati ma il manager e' vuoto: orfano.
