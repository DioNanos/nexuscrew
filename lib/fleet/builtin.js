...
function resolveCellCwd(cell, home) {
  ...
  if (hasCwd && hasRel) {
    ...
    resolved = { realCwd, cwdRel: relNorm };
  } else if (hasCwd) {
    if (!realCwd) return { ok: false, fail: { reason: 'invalid-cwd', cwd: cell.cwd } };
    const rel = deriveCwdRel(realCwd, realHome);
    if (rel === null) return { ok: false, fail: { reason: 'not-under-home', cwd: cell.cwd } };
    resolved = { realCwd, cwdRel: rel };
  } else {
    // solo cwdRel
    if (!realFromRel) return { ok: false, fail: { reason: 'invalid-rel', cwdRel: cell.cwdRel } };
    resolved = { realCwd: realFromRel, cwdRel: relNorm };
  }
  // hostCwd (validated on the host filesystem) is resolved.realCwd. guestCwd,
  // when present, is the child's effective cwd and is NOT host-validated: an
  // engine running in a guest namespace (e.g. Proot) uses host-invisible paths,
  // so a valid guestCwd must never raise a spurious CWD_INVALID on the host.
  const hasGuest = typeof cell.guestCwd === 'string' && cell.guestCwd.length > 0;
  return {
    ok: true,
    cwd: hasGuest ? cell.guestCwd : resolved.realCwd,
    hostCwd: resolved.realCwd,
