...
async function createBuiltinFleet(cfg = {}) {
  ...
  // Lease opt-in: attivo solo quando cfg.cellLeaseEnabled === true.
  // server.js lo passa true in start(); i test che chiamano createServer()
  // direttamente non passano il flag, quindi la suite di test esistente non vede
  // endpoint lease aperti da up().
  ...
  const launchBroker = cfg.launchBroker || createLaunchBroker({
    ...cfg, home,
    // R3.1.1: la connessione broker one-shot resta APERTA dopo il payload e viene
    // consegnata qui per divenire il canale lease del supervisore.
    ...(leaseManager ? {
      onLease: (socket, lease) => {
        if (lease && lease.cellId) leaseManager.attachInitial(lease.cellId, socket, { generation: 0 });
      },
    } : {}),
  });

  // Bootstrap: fleet.json valido? garbage -> unavailable (fail-closed, design §7).
  // B4: il gate fail-closed PRECEDE la recovery lease. Si decide PRIMA se la Fleet
  // sara' disponibile, e solo allora si riaprono gli endpoint lease delle celle
  // note. Inversione dell'ordine storico: leaseManager.boot() veniva eseguito prima
  // del gate, e con fleet.json invalido createBuiltinFleet ritornava `off` lasciando
  // gli endpoint lease vivi e senza close() — un reconnect valido otteneva un lease
  // con Fleet non disponibile (la recovery non deve precedere il gate fail-closed).
  let boot = loadDefinitions(defsPath);
  if (!boot) return off;

  // Recovery di produzione (rilievo 3 / R3.3.1): al boot del server nessun lease
  // sopravvive, ma ripristiniamo le identity persistite e RIAPRIAMO gli endpoint
  // stabili delle celle note, cosicche' un supervisore vivo che reconnecta dopo il
  // restart trovi l'endpoint. Non fatale: un fallimento di singolo endpoint non
  // blocca il boot del fleet. Ora dopo il gate fail-closed.
  if (leaseManager) {
  ...
  async function removeModel(id, engine) {
    ...
    // cambiato. Meglio rifiutare adesso, dicendo chi lo usa.
    const usato = (defs.engines || [])
      .filter((e) => e.managed && e.managed.model === wantedId)
      .map((e) => e.id);
    if (usato.length) {
  ...
  function definitions() {
    ...
    return {
      ...
      engines: defs.engines.map((e) => {
        ...
        delete out.env;
        if (e.managed) out.managedInfo = describeManaged(e.managed, { ...cfg, home, extraModels: extraModelsFrom(defs) });
        return out;
      }),
      cells: defs.cells.map((c) => {
        const out = {
