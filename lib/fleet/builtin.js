...
async function createBuiltinFleet(cfg = {}) {
  ...
  async function removeModel(id, engine) {
    ...
    const usato = (defs.engines || [])
      ...
      .map((e) => e.id);
    if (usato.length) {
      throw httpError(409, `modello in uso: ${usato.join(', ')}`);
    }
    await mutate(defs, (d) => {
  ...
  function definitions() {
    ...
    return {
      ...
      engines: defs.engines.map((e) => {
        ...
        delete out.env;
        if (e.managed) out.managedInfo = describeManaged(e.managed, { ...cfg, home, extraModels: extraModelsFrom(defs), engineId: e.id });
        return out;
      }),
      cells: defs.cells.map((c) => {
        const out = {
  ...
  async function testModel(engineId, model, opts = {}) {
    const profile = CATALOG.find((p) => p.id === String(engineId || ''));
    if (!profile) {
