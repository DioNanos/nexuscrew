  ...
  try {
    ...
    if (d.models !== undefined) {
      ...
      for (const m of d.models) {
        ...
        seen.add(key);
        if (!extraModels.has(model.engine)) extraModels.set(model.engine, new Set());
        extraModels.get(model.engine).add(model.id);
        models.push(model);
      }
    }
...
//
// Rilievo dell'audit indipendente, provato con probe su due profili distinti.
