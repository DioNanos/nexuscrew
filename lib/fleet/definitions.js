'use strict';
// Editable fleet definitions (~/.nexuscrew/fleet.json).
...
// handled. The definitions carry arbitrary commands, so validation is
// STRICT (garbage -> error, no guessing). Same confinement as
// lib/fs/routes.js and lib/tmux/lifecycle.js.
...
// Capacita' dichiarate di una cella, per NOME (come `mcp`, che ne e' il caso
// particolare e l'alias legacy). Tre chiavi: `mcp` (server MCP), `skills`,
// `ondemand`. Profili di ruolo condivisi in `capabilityProfiles` (top-level del
// documento); la cella riferisce un profilo e puo' sostituire SINGOLE CHIAVI
// con il proprio set: l'override e' sostitutivo, non additivo, perche' unire
// non saprebbe togliere.
const MAX_CAPABILITIES = 64;       // nomi per singola chiave
const MAX_CAPABILITY_PROFILES = 32; // profili dichiarabili nel documento
const CAPABILITY_KEYS = ['mcp', 'skills', 'ondemand'];
const CAPABILITY_NAME_RE = /^[a-z0-9][a-z0-9._-]{0,63}$/;  // skill/ondemand: slug
const CAPABILITY_KEY_RES = { mcp: MCP_NAME_RE, skills: CAPABILITY_NAME_RE, ondemand: CAPABILITY_NAME_RE };
...
//  - Dotted IDs: dot-free v2 mapping:
...
// ---------------------------------------------------------------------------
function parseDefinitions(raw, { allowLegacyTmuxNames = true, onReject = null, onIssue = null } = {}) {
  ...
  try {
    ...
    if (d.models !== undefined) {
      ...
      if (d.models.length > MAX_MODELS) {
        // Come per gli engine: il rifiuto NOMINA il limite, altrimenti chi
        // dichiara si ritrova un 400 generico senza sapere che il cap era
        // quello. `onIssue` porta campo/valore/regola al messaggio del comando
        // (400 di una scrittura), `reject` alla lettura del file.
        if (typeof onIssue === 'function') onIssue({ field: 'models', value: d.models.length, rule: `troppi modelli dichiarati, cap ${MAX_MODELS}` });
        return reject(`fleet.json rifiutato: ${d.models.length} modelli dichiarati, cap ${MAX_MODELS}; riduci i modelli a ${MAX_MODELS} o meno`);
      }
      const seen = new Set();
    ...
    for (const e of d.engines) {
      const eng = parseEngine(e, { extraModels, onIssue });
      if (!eng) return null;
    ...
    // I profili di capability si leggono PRIMA delle celle: una cella riferisce
    // un profilo per NOME, e leggerli dopo la farebbe rifiutare per un dato che
    // in realta' c'e'. `capabilityProfiles` e' opzionale: senza, il documento
    // resta com'era (chiave assente, non vuota).
    let capabilityProfiles;
    if (d.capabilityProfiles !== undefined) {
      if (!d.capabilityProfiles || typeof d.capabilityProfiles !== 'object' || Array.isArray(d.capabilityProfiles)) return null;
      const voci = Object.entries(d.capabilityProfiles);
      if (voci.length > MAX_CAPABILITY_PROFILES) {
        if (typeof onIssue === 'function') onIssue({ field: 'capabilityProfiles', value: voci.length, rule: `troppi profili di capability dichiarati, cap ${MAX_CAPABILITY_PROFILES}` });
        return reject(`fleet.json rifiutato: ${voci.length} profili di capability dichiarati, cap ${MAX_CAPABILITY_PROFILES}; riduci i profili a ${MAX_CAPABILITY_PROFILES} o meno`);
      }
      capabilityProfiles = {};
      for (const [nome, profilo] of voci) {
        // Il nome e' un identificatore (finira' in riferimento per cella e, poi,
        // in un nome di file generato): stessa forma chiusa dell'id cella.
        if (typeof nome !== 'string' || !CELL_ID_RE.test(nome) || capabilityProfiles[nome] !== undefined) return null;
        const set = parseCapabilitySet(profilo, onIssue);
        if (!set) return null;
        capabilityProfiles[nome] = set;
      }
    }
    const capabilityProfileNames = capabilityProfiles ? new Set(Object.keys(capabilityProfiles)) : null;
    ...
    for (const c of d.cells) {
      const cell = parseCell(c, engineIds, engineMap, {
        allowLegacyTmuxNames, extraModels, onReject: reject, onIssue, capabilityProfileNames,
      });
      if (!cell) return null;
    ...
    // `models` e `capabilityProfiles` compaiono solo se dichiarati: una
    // configurazione senza resta byte-identica a prima, e l'aggiornamento non
    // riscrive nulla.
    const parsed = { schemaVersion: SCHEMA_VERSION, engines, cells };
    if (models.length) parsed.models = models;
    if (capabilityProfiles) parsed.capabilityProfiles = capabilityProfiles;
...
function parseEngine(e, { extraModels = null, onIssue = null } = {}) {
...
function cellModelAllowed(engineId, model, engineMap, extraModels, onIssue) {
  ...
  return !!normalizeManagedSpec({ ...engine.managed, model }, { extraModels, engineId, onIssue });
}

// Un set di capability (di profilo o di cella): oggetto le cui chiavi ammesse
// sono CAPABILITY_KEYS, ciascuna un array di nomi VALIDATI PER CHIAVE, senza
// duplicati, sotto cap. Chiave non ammessa -> rifiuto dell'intero set: come
// per panelUrl, due esiti opposti che si assomigliano non collassano in uno.
function parseCapabilitySet(v, onIssue = null) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return null;
  const out = {};
  for (const key of Object.keys(v)) {
    if (!CAPABILITY_KEYS.includes(key)) return null;
    const arr = v[key];
    if (!Array.isArray(arr) || arr.length > MAX_CAPABILITIES) {
      if (typeof onIssue === 'function') onIssue({ field: `capabilities.${key}`, value: Array.isArray(arr) ? arr.length : -1, rule: `troppi nomi, cap ${MAX_CAPABILITIES}` });
      return null;
    }
    const visti = new Set();
    for (const nome of arr) {
      if (typeof nome !== 'string' || !CAPABILITY_KEY_RES[key].test(nome) || visti.has(nome)) return null;
      visti.add(nome);
    }
    out[key] = [...visti];
  }
...
// La superficie effettiva di una cella: il profilo alimenta le chiavi che la
// cella non copre, e l'override della cella SOSTITUISCE per chiave (unire non
// saprebbe togliere). LEGACY = nessun campo: la cella di sempre, comportamento
// invariato. Pura: la vista e i materializzatori la usano senza toccare nulla.
function effectiveCapabilities(cell, defs) {
  const profili = (defs && defs.capabilityProfiles) || {};
  const profilo = cell && cell.capabilityProfile !== undefined ? profili[cell.capabilityProfile] : undefined;
  const declared = {};
  let any = false;
  for (const key of CAPABILITY_KEYS) {
    const source = (cell && cell.capabilities && cell.capabilities[key] !== undefined) ? cell.capabilities[key]
      : (profilo && profilo[key] !== undefined) ? profilo[key] : undefined;
    if (source !== undefined) { declared[key] = [...source]; any = true; }
  }
  return { legacy: cell?.capabilities === undefined && cell?.capabilityProfile === undefined, declared };
}

function parseCell(c, engineIds, engineMap = new Map(), {
  allowLegacyTmuxNames = true, extraModels = null, onReject = null, onIssue = null,
  capabilityProfileNames = null,
} = {}) {
  const reject = (message) => {
    if (typeof onReject === 'function') onReject(message);
    return null;
  ...
  // cwdRel (optional, canonical): portable home-relative form.
  ...
  if (c.model !== undefined) {
    ...
    if (!cellModelAllowed(c.engine, c.model, engineMap, extraModels, onIssue)) return null;
    model = c.model;
  }
  ...
  if (c.models !== undefined) {
    ...
    for (const [engineId, value] of entries) {
      ...
      if (!cellModelAllowed(engineId, value, engineMap, extraModels, onIssue)) return null;
  ...
  // capabilities / capabilityProfile: la superficie operativa DICHIARATA della
  // cella. `capabilities` e' un set per chiave (`mcp`/`skills`/`ondemand`,
  // parseCapabilitySet sopra); `capabilityProfile` riferisce un profilo
  // top-level del documento — DEVE esistere: un profilo dangling rifiuta la
  // cella, non collassa in "niente profilo" (stesso motivo di panelUrl).
  // Assenti entrambi = cella legacy: comportamento invariato (effectiveCapabilities).
  let capabilities;
  if (c.capabilities !== undefined) {
    capabilities = parseCapabilitySet(c.capabilities, onIssue);
    if (!capabilities) return null;
  }
  let capabilityProfile;
  if (c.capabilityProfile !== undefined) {
    if (typeof c.capabilityProfile !== 'string' || !CELL_ID_RE.test(c.capabilityProfile)) return null;
    if (!capabilityProfileNames || !capabilityProfileNames.has(c.capabilityProfile)) {
      return reject(`fleet.json rifiutato: la cella ${c.id} riferisce il profilo di capability "${c.capabilityProfile}" che non e' dichiarato in capabilityProfiles`);
    }
  ...
  if (capabilityProfile !== undefined) out.capabilityProfile = capabilityProfile;
...
function validateCommandTrust(command) {
  ...
  // Owner check: the command must belong to the service user or root —
  // a third-party owner could replace the executable while keeping the
  // path "trusted".
...
// ---------------------------------------------------------------------------
// cwdRel — portable home-relative cwd representation.
...
function atomicWrite(p, data) {
  ...
  // La causa viene raccolta DURANTE la validazione e riportata nel
  // messaggio finale (campo, valore, regola). Senza cause nominate il
  // messaggio resta quello storico, generico.
  const issues = [];
  const parsed = parseDefinitions(data, { allowLegacyTmuxNames: false, onIssue: (i) => issues.push(i) });
  if (!parsed) {
    backupPredecessor(p); // conserva il precedente per recovery/forensics
    const detail = issues.length
      ? ' — ' + issues.map((i) => `${i.field} ${JSON.stringify(i.value ?? '')}: ${i.rule}`).join('; ')
      : ' — validazione fallita';
    throw new Error(`definizioni fleet non valide${detail}`);
  }
...
const CAPS = {
  SCHEMA_VERSION, MAX_ENGINES, MAX_MODELS, MAX_CELLS, MAX_ARGS, MAX_ARG_LEN,
