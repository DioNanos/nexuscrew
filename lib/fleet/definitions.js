...
function parseCell(c, engineIds, engineMap = new Map(), { allowLegacyTmuxNames = true } = {}) {
  ...
  // guestCwd (opzionale, design WP1): cwd effettiva del figlio per engine che
  // girano in un namespace guest (es. Proot). NON e validata sul filesystem host
  // (a differenza di cwd/cwdRel): un path guest valido non deve generare un falso
  // CWD_INVALID sul host. Qui si valida solo il FORMATO (stringa non vuota, no
  // null byte, limite). La risoluzione host resta demandata a resolveCellCwd.
  let guestCwd;
  if (c.guestCwd !== undefined) {
    if (typeof c.guestCwd !== 'string' || !c.guestCwd || c.guestCwd.length > MAX_CWD_LEN) return null;
    if (c.guestCwd.includes('\0')) return null;
    guestCwd = c.guestCwd;
  }
  ...
  if (guestCwd !== undefined) out.guestCwd = guestCwd;
