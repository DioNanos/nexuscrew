...
function createLeaseManager(cfg = {}, seams = {}) {
  ...
  // 2b: se il bind committa, il server consegna SUBITO leaseId + proof sul
  // canale: il supervisore detiene un proof valido dal primo istante, anche se
  // l'EOF arrivesse prima del primo refresh.
  function attachInitial(cellId, socket, { generation = 0 } = {}) {
    ...
    if (!bindLiveSocket(cellId, entry, socket, { lease })) return false;
    writeSafe(socket, { type: 'lease', leaseId: lease.leaseId, proof: issueLeaseProof(cellId, entry, lease) });
    return true;
  }
