...
function createLeaseManager(cfg = {}, seams = {}) {
  ...
  function persistEntry(cellId, entry) {
    ...
    obj[cellId] = { launchEpoch: entry.launchEpoch, capability: entry.capability };
    writePersisted(obj);
  }
  ...
  function handleReconnect(cellId, socket, msg) {
    ...
    // R3.3.5: oltre la grace il reconnect e' rifiutato. reattach restituisce null
    // se il lease corrente e' in grace scaduta.
  ...
  async function track(cellId) {
    ...
    await new Promise((resolve, reject) => {
      ...
      server.listen(sp, () => {
        ...
        try { fsImpl.chmodSync(sp, 0o600); } catch (_) {}
        resolve();
      });
    });
    const entry = existing
      || { launchEpoch, capability, stablePath: sp, stableServer: null, lease: null, socket: null, graceTimer: null };
    ...
    return { stablePath: sp, launchEpoch, capability };
  }
  ...
  // {launchEpoch, capability} persistiti, per riconoscere i supervisori che
  // reconnectano dopo il restart. Nessun lease/eligibilita' sopravvive.
  function loadPersisted() {
    const obj = readPersisted();
    for (const [cellId, info] of Object.entries(obj)) {
      if (cells.has(cellId)) continue;
      // L'endpoint stabile viene (ri)aperto da track() in up(); qui registriamo
      // solo l'identita' nota, se assente, cosicche' handleReconnect possa validarla
      // anche quando la cella non sia ancora passata di nuovo per track().
      cells.set(cellId, { launchEpoch: info.launchEpoch, capability: info.capability, stablePath: stablePathFor(cellId), stableServer: null, lease: null, socket: null, graceTimer: null });
    }
  }
  ...
  return { track, attachInitial, loadPersisted, status, close, _cells: cells };
}
