...
// Usato sia dal path iniziale che dal ramo EEXIST (race symlink-safe).
...
function loadOrCreateToken(tokenPath) {
  ...
  try {
    ...
    if (e.code === 'EEXIST') {
      // Un altro processo ha creato nel frattempo: re-validate no-follow (race symlink-safe)
...
// Atomic replace: same-directory tmp file -> chmod 0600 -> rename, no-follow symlinks.
