...
/// Stesso tetto dichiarato dal device (`MAX_PROMPT_TEXT` in protocol.rs): due
/// numeri diversi qui e la' vorrebbero dire consegnare comandi che il nodo
/// rifiuta.
const MAX_PROMPT_TEXT = 4 * 1024;
...
function commandOf(body) {
  ...
  // L'unico verbo che porta testo dell'utente fino alla sessione del device.
  // Il tetto e' lo stesso che il nodo dichiara (4 KiB): rifiutare QUI cio' che
  // il device rifiuterebbe comunque evita un comando in volo destinato a
  // fallire, e un `submitted` che l'operatore leggerebbe come «e' partito».
  if (body.kind === 'prompt') {
    const args = body.args;
    if (!args || typeof args !== 'object' || Array.isArray(args)) return null;
    if (Object.keys(args).join(',') !== 'text') return null;
    if (typeof args.text !== 'string') return null;
    if (args.text.length < 1 || Buffer.byteLength(args.text, 'utf8') > MAX_PROMPT_TEXT) return null;
    return { kind: 'prompt', args: { text: args.text } };
  }
  if (body.kind === 'logs') {
...
function publicRoutes({ storePath, broker, ownerId }) {
  ...
  r.post('/poll', body, async (req, res) => {
    ...
    // Il cursore degli eventi viaggia in un HEADER, non nel corpo: la risposta
    // a un poll scaduto e' un 204 senza corpo, e senza header il device non
    // saprebbe mai fin dove sono stati accettati i suoi eventi — continuerebbe
    // a rimandarli. L'header sta su OGNI risposta, anche su quelle vuote.
    if (Number.isSafeInteger(result.eventsCursor)) {
      res.set('x-vl-events-cursor', String(result.eventsCursor));
    }
...
function apiRoutes({ storePath, broker, ownerId, readonly = () => false }) {
  ...
  // Lettura pura del ring in memoria: nessun gate READONLY, perche' non muta
  // nulla. `after` e' il cursore della UI, cosi' non riscarica tutto a ogni
  // giro. Cio' che non c'e' piu' nel ring non si recupera da qui: la copia
  // durevole e' il journal sul device (contratto passo 2, punto 5).
  r.get('/:nodeId/events', (req, res) => {
    ...
    const out = broker.events(req.params.nodeId, { after });
    return res.json({ protocol: PROTOCOL, nodeId: req.params.nodeId, ...out });
  });
