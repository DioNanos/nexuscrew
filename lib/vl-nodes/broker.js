...
// e non sparisce dalla lista.
function sanitizeSession(raw) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;
  if (raw.attached !== true && raw.attached !== false) return null;
  const profile = boundedString(raw.profile, 64);
  if (!profile) return null;
  return { attached: raw.attached, profile };
}
...
function createBroker(options = {}) {
  ...
  function poll(node, rawHeartbeat, { waitMs = MAX_WAIT_MS, signal = null } = {}) {
    ...
    return new Promise((resolve) => {
      ...
      // Nessun unref: il timer e' REF perche' la promise del poll si risolva
      // sempre (test e shutdown); il flush alla chiusura evita attese residue.
      if (signal) {
        const onAbort = () => {
          if (state.waiter === waiter) state.waiter = null;
          clearTimer(waiter.timer);
  ...
  // Chiusura/shutdown: i waiter pendenti si risolvono SUBITO (idle) cosi' il
  // processo puo' terminare senza aspettare i boundedWait residui.
  function flushPendingWaiters() {
    let flushed = 0;
    for (const state of states.values()) {
      if (state.waiter) { finishWaiter(state, { type: 'idle', generation: state.generation }); flushed += 1; }
    }
    return flushed;
  }
  ...
  function list(nodes) {
    ...
    return nodes.map((node) => {
      ...
      return {
        ...
        session: state?.heartbeat?.session ?? null,
  ...
  return { poll, dispatch, forget, list, observe, events, flushPendingWaiters, _states: states };
}
