...
function topologyOwners(payload) {
  ...
  for (const node of (payload && Array.isArray(payload.nodes) ? payload.nodes : [])) {
    ...
    out.push({
      ...
      // Serve alla directory per dire QUANDO un nodo stale e' stato visto
      // l'ultima volta: senza, la voce in unavailable parlerebbe dello stato
      // senza dire da quanto.
      lastSeen: Number.isFinite(node.lastSeen) ? node.lastSeen : null,
    });
  }
  return out;
}
...
async function readCellDirectory(ctx, callerSession = null) {
  ...
  const candidati = topologyOwners(topology).filter((owner) => owner.instanceId !== localId);
  const owners = [{ instanceId: localId, route: [], label: 'Local', stale: false },
    ...candidati.filter((owner) => !owner.stale)];
  const cells = [];
  const unavailable = [];
  // Un owner STALE resta nel gate (non si interroga un nodo che la topologia
  // dichiara stantio) ma non SPARISCE dalla directory: chi legge `nc_cells`
  // deve distinguere «questo nodo non risponde da X» da «questo nodo non
  // esiste». Stesso shape di unavailableOwner, failure 'stale' + lastSeen.
  for (const owner of candidati) {
    if (!owner.stale) continue;
    unavailable.push({
      instanceId: owner.instanceId,
      owner: owner.label,
      route: owner.route.length ? owner.route.join('/') : 'local',
      ...(owner.route.length === 0 ? { local: true } : {}),
      failure: 'stale',
      lastSeen: Number.isFinite(owner.lastSeen) ? owner.lastSeen : null,
    });
  }
