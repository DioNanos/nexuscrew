...
const TOOLS = [
  ...
  },
  {
    ...
    async handler(args, ctx) {
      const target = argString(args, 'target', { required: true });
      const text = argString(args, 'text', { required: true, max: 4096 });
      const lang = argString(args, 'lang');
      const voice = argString(args, 'voice');
      const urgency = args.urgency === 'high' ? 'high' : 'normal';
      const utteranceId = argString(args, 'utteranceId');
      const identity = await ctx.identity();
      const session = requireSession(identity.session, 'nc_speak', identity.code);
      const payload = { target, text, urgency, ...(lang ? { lang } : {}), ...(voice ? { voice } : {}), ...(utteranceId ? { utteranceId } : {}), session };
      const receipt = await ctx.api('POST', '/api/audio/speak', payload);
      return { receipt };
    },
  },
  {
    ...
    annotations: { readOnlyHint: true },
    async handler(args, ctx) {
      const utteranceId = argString(args, 'utteranceId', { required: true });
      const identity = await ctx.identity();
      requireSession(identity.session, 'nc_speak_status', identity.code);
      const receipt = await ctx.api('GET', `/api/audio/speak/status/${encodeURIComponent(utteranceId)}`);
      return { receipt };
    },
  },
];
