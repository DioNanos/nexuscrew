...
const IDENTITY_CODE = Object.freeze({
  ...
  // coerente (versione, owner, cella, canale, introspezione) e' fail-closed:
  // MAI fallback alle variabili legacy quando la fonte verified esiste.
  VERIFIED_ENV_INVALID: 'NEXUSCREW_MCP_IDENTITY_VERIFIED_ENV_INVALID',
});
...
// la remediation NON consiglia piu' di allowlistare TMUX/TMUX_PANE/
// NEXUSCREW_MCP_SESSION in env_vars: sul percorso shared verified quel permesso
// fa fallire lo spawn (rmcp-client utils: nomi riservati rifiutati). Le strade
// sono due: sessione verified (codex-vl >= 0.154.0-vl.3 + NexusCrew >= 0.9.25
// con authority mode provisionato) oppure sessione legacy con le tre variabili
// ereditate dal client MCP.
const IDENTITY_REMEDIATION =
  'Due percorsi validi: (1) sessione verified — serve codex-vl >= 0.154.0-vl.3 e NexusCrew >= 0.9.25 '
  + 'con authority mode provisionato (nexuscrew identity provision): il launcher consegna i metadati '
  + 'NEXUSCREW_VERIFIED_* al figlio MCP, non serve allowlistare nulla; (2) sessione legacy — il client '
  + 'MCP deve ereditare TMUX/TMUX_PANE/NEXUSCREW_MCP_SESSION (niente allowlist in env_vars del server '
  + 'MCP: con codex-vl >= 0.154.0-vl.3 i nomi sono riservati e lo spawn fallisce).';
...
async function readVlDirectory(ctx) {
  ...
  const owners = [{ instanceId: localId, route: [], label: 'Local', stale: false },
    ...topologyOwners(topology).filter((owner) => owner.instanceId !== localId)];
  const nodes = [];
...
const TOOLS = [
  {
    ...
    async handler(args, ctx) {
      ...
      // (rilievo di una revisione indipendente). Se un giorno servira' il dettaglio,
  ...
  },
  {
    ...
    async handler(_args, ctx) {
      ...
      // Gli owner stale restano interrogabili con la loro route (best-effort):
      // un ramo lento degrada a stale, non sparisce dalla directory dei deck.
      const remotes = topologyOwners(topology).filter((owner) => owner.instanceId !== localNodeId);
