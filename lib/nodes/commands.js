...
function nodesAdd(opts) {
  ...
  const entry = { name, ssh, remotePort, localPort, keyPath, roles: { client: true, node: false } };
  ...
  log(`nodes add: nodo "${name}" aggiunto (ssh ${ssh}, remote ${remotePort} -> local ${localPort})`);
...
function nodesList(opts) {
  ...
  for (const n of nodes) {
    log(`${n.name}\t${n.ssh}\tlocal:${n.localPort} -> remote:${n.remotePort}\ttunnel:${n.tunnel.status}\ttoken:${n.hasToken ? 'set' : 'unset'}`);
  }
  return { code: 0, nodes };
}
...
function nodesUp(opts) {
  ...
  const r = tunnel.startForward({ home: ctx.home, node: ctx.node, spawnImpl: opts.spawnImpl, spawnSyncImpl: opts.spawnSyncImpl, sshBin: opts.sshBin, logFd: opts.logFd });
  if (r.started) {
