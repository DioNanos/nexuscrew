...
const accessPresets = require('./access-presets.js');
...
function nodesAdd(opts) {
  ...
  // da revisione: permitopen con le destinazioni EXPLICITE dei -L. Qui si conosce solo
...
// --- Event feed observability -------------------------------------------------
// Best-effort LOCAL read of the running server's feed diagnostics. The CLI
// never reads runtime files for this: if the local server is unreachable it
// says so explicitly instead of reporting zero deliveries.
// One line for the OS alert relay of that peer: same local diagnostics payload,
// counters and last reason only. A peer nobody tried to alert stays honest with
// zeros instead of disappearing.
function relayLineFor(body, peerNodeId) {
  const section = (body && body.pushRelay) || {};
  const own = (section.byOwner && section.byOwner[peerNodeId]) || {};
  const last = own.lastReason ? ` · ultimo motivo ${own.lastReason}` : '';
  return `relay:     avvisi OS in coda ${own.queued || 0} · inviati ${own.pushed || 0} · scartati ${own.dropped || 0} · errori ${own.sendFailed || 0}${last}`;
}

function feedLinesFor(diag, peerNodeId) {
  if (!diag || diag.ok !== true) {
    return [`feed:      diagnostica non disponibile (${(diag && diag.reason) || 'sconosciuto'}) — il server locale non risponde`];
  }
  const entry = (diag.body && Array.isArray(diag.body.peers) ? diag.body.peers : [])
    .find((p) => p.nodeId === peerNodeId);
  if (!entry) return ['feed:      nessuna vista per questo peer'];
  const enabled = diag.body.enabled === true;
  const counters = (entry.feed && entry.feed.counters) || {};
  const lastProblem = (entry.feed && (entry.feed.lastDropReason || entry.feed.lastRejectReason)) || 'none';
  const grants = entry.grants || {};
  return [
    // The kill-switch state is its OWN line: it is the operator's first
    // question when the feed is silent.
    `feed:      kill-switch ${enabled ? 'attivo (feed abilitato)' : 'OFF: feed disabilitato'} · revision ${entry.accessRevision} · label ${entry.accessLabel}`,
    `feed:      grant events=${grants.eventsAccess === true ? 'on' : 'off'} node-events=${grants.nodeEventsAccess === true ? 'on' : 'off'} · scope celle ${grants.cellVisibility || 'none'}${Array.isArray(grants.cells) && grants.cells.length ? ` (${grants.cells.join(', ')})` : ''}`,
    `feed:      cursore ${(entry.feed && entry.feed.lastCursor) || '-'} · stream ${(entry.feed && entry.feed.streams) || 0} · ultimo mancato: ${lastProblem}`,
    `feed:      contatori delivered ${(counters.delivered || 0)} · dropped ${(counters.dropped || 0)} · replayed ${(counters.replayed || 0)} · rejected ${(counters.rejected || 0)}`,
    relayLineFor(diag.body, peerNodeId),
  ];
}

async function fetchFeedDiagnostics(opts = {}) {
  let port = Number(opts.localAppPort) || null;
  if (!port && opts.configPath) {
    try { port = Number(JSON.parse(fs.readFileSync(opts.configPath, 'utf8')).port) || null; } catch (_) { port = null; }
  }
  let token = '';
  try { token = fs.readFileSync(opts.tokenPath, 'utf8').trim(); } catch (_) { return { ok: false, reason: 'no-token' }; }
  if (!port) return { ok: false, reason: 'no-port' };
  try {
    // The local diagnostics live under the authenticated API mount: without
    // the /api prefix the CLI would read the static handler and report the
    // server as unreachable while it is answering fine.
    const r = await (opts.fetchImpl || fetch)(`http://127.0.0.1:${port}/api/diagnostics/events`, {
...
function nodesInspect(opts) {
  ...
  if (opts.json) log(JSON.stringify(found.peer, null, 2));
    ...
    // Effective federated access: the label is DERIVED from the grants, and a
    // record whose grants were never set says so instead of showing a preset
    // inferred from defaults.
    if (peer.kind === 'direct') {
      const view = accessPresets.grantsOf(peer);
      log(`accesso:   ${view.label}`);
      log(`grant:     ${ACCESS_GRANT_KEYS.map((key) => `${key}=${view.grants[key] ? 'on' : 'off'}`).join(' ')}`);
      log(`scope:     celle ${view.grants.cellVisibility}`);
      for (const line of feedLinesFor(opts.feedDiagnostics, peer.nodeId)) log(line);
    }
...
// --- Federated access grants -------------------------------------------------
// The grant vector of a peer is set on the node that OWNS the resources, in one
// validated write. The label (admin/user/nexushost/custom) is derived from the
// grants; nothing here stores or reads a role.
const ACCESS_GRANT_KEYS = ['eventsAccess', 'nodeEventsAccess', 'askReplyAccess', 'filesReadAccess', 'peerOperatorAccess'];
const ACCESS_EDIT_KEYS = new Set(['accessRole', ...ACCESS_GRANT_KEYS]);

const ACCESS_VECTOR_FIELDS = ['cellVisibility', 'cells', 'panelAccess', 'liveHostAccess'];

// The prospective vector of a peer after applying `overrides`. It covers every
// field the invariants talk about, so a granular edit cannot leave the record
// in a state the preset validator would refuse (live hosting without the
// complete admin set being the case that matters).
function accessVectorFrom(peer, overrides) {
  const visibility = Object.hasOwn(overrides, 'cellVisibility') ? overrides.cellVisibility : peer.cellVisibility;
  const vector = { cellVisibility: visibility };
  const cells = Object.hasOwn(overrides, 'cells') ? overrides.cells : peer.cells;
  if (visibility === 'selected') vector.cells = Array.isArray(cells) ? cells : [];
  const all = [...ACCESS_GRANT_KEYS, 'panelAccess', 'liveHostAccess'];
  for (const key of all) {
    vector[key] = Object.hasOwn(overrides, key) ? overrides[key] === true : peer[key] === true;
  }
  return vector;
}
...
function nodesEdit(opts) {
  ...
  // eventsReceive is the node's OWN choice to import a peer's feed: it is a
  // local client decision in BOTH directions and never part of the owner's
  // grant presets.
  const allowed = node.direction === 'inbound'
    ? new Set(['label', 'visibility', 'selected', 'cellVisibility', 'cells', 'panelAccess', 'liveHostAccess', 'accessRole', 'eventsReceive', ...ACCESS_GRANT_KEYS])
    : new Set(['label', 'ssh', 'sshPort', 'autostart', 'cellVisibility', 'cells', 'panelAccess', 'liveHostAccess', 'accessRole', 'eventsReceive', ...ACCESS_GRANT_KEYS]);
  ...
  if (Object.hasOwn(supplied, 'eventsReceive') && typeof supplied.eventsReceive !== 'boolean') {
    log('nodes edit: eventsReceive vuole un booleano');
    return { code: 1, reason: 'invalid-events-receive' };
  }
  ...
  const accessRequested = keys.some((key) => ACCESS_EDIT_KEYS.has(key));
  const touchesVector = keys.some((key) => ACCESS_VECTOR_FIELDS.includes(key));
  // A peer whose grants were never configured keeps its historical behaviour
  // for panel/live flags; once a vector IS configured, every edit of that
  // vector passes through the same validator and the same single write.
  const enforceVector = accessRequested || (touchesVector && node.accessConfigured === true);
  let accessVector = null;
  if (enforceVector) {
    const hasRole = Object.hasOwn(supplied, 'accessRole');
    const granular = ACCESS_GRANT_KEYS.filter((key) => Object.hasOwn(supplied, key));
    if (hasRole && granular.length > 0) {
      log('nodes edit: --access-role e i grant granulari non si combinano nella stessa richiesta');
      return { code: 1, reason: 'preset-and-grants' };
    }
    for (const key of granular) {
      if (typeof supplied[key] !== 'boolean') {
        log(`nodes edit: ${key} vuole un booleano`);
        return { code: 1, reason: 'invalid-grant' };
      }
    }
    if (hasRole) {
      if (typeof supplied.accessRole !== 'string' || !accessPresets.PRESET_NAMES.includes(supplied.accessRole)) {
        log(`nodes edit: access-role non valido (atteso: ${accessPresets.PRESET_NAMES.join('|')})`);
        return { code: 1, reason: 'invalid-access-role' };
      }
      accessVector = { ...accessPresets.PRESETS[supplied.accessRole] };
    } else if (!accessRequested && touchesVector) {
      // Panel/live/scope edit on a configured peer: same vector, same rules.
      accessVector = accessVectorFrom(node, supplied);
    } else {
      // A granular edit only refines a vector that was set explicitly: a peer
      // whose grants were never configured stays denied, and a partial edit
      // must not promote it.
      if (node.accessConfigured !== true) {
        log('nodes edit: peer non configurato — applica prima un preset (--access-role)');
        return { code: 1, reason: 'access-not-configured' };
      }
      accessVector = accessVectorFrom(node, supplied);
    }
    const check = accessPresets.validateGrants(accessVector);
    if (!check.ok) {
      log(`nodes edit: concessioni rifiutate — ${check.errors.join('; ')}`);
      return { code: 1, reason: 'invalid-grants' };
    }
    for (const key of [...ACCESS_EDIT_KEYS, ...ACCESS_VECTOR_FIELDS]) delete patch[key];
  }

  let next;
  if (enforceVector) {
    // One validated read-modify-write against the policy revision: a concurrent
    // edit loses the race and rereads instead of overwriting it.
    const expected = Number.isInteger(opts.expectedRevision) ? opts.expectedRevision : store.accessRevisionOf(st);
    let res;
    try {
      res = store.setPeerAccessGrantsCas({
        filePath: nodesPath, name: node.name, grants: accessVector, expectedRevision: expected, extraPatch: patch,
      });
    } catch (e) { log(`nodes edit: ${e.message}`); return { code: 1, reason: 'invalid-patch' }; }
    if (!res.ok) {
      log(`nodes edit: revision delle concessioni superata (attuale ${res.revision}) — rileggi e riprova`);
      return { code: 1, reason: 'revision-conflict', revision: res.revision };
    }
    next = store.loadStoreStrict(nodesPath);
  } else {
...
// --- nodes access: read-only inventory --------------------------------------
// Shows what every peer may currently do on this node, with the revision of
// the policy. Peers whose grants were never configured are listed as such:
// they are the ones an explicit migration has to set, and nothing promotes
// them by name or by age of pairing.
function nodesAccess(opts = {}) {
  const log = opts.log || console.log;
  const { nodesPath } = resolveNodePaths(opts);
  let st;
  try { st = store.loadStoreStrict(nodesPath); }
  catch (e) { log(`nodes access: ${e.message}`); return { code: 1, reason: 'store invalido', status: e.status || 500 }; }
  const view = store.accessInventory(st);
  const legacy = view.peers.filter((peer) => !peer.configured);
  if (opts.json) {
    log(JSON.stringify({ revision: view.revision, peers: view.peers, unconfigured: legacy.map((p) => p.name) }, null, 2));
    return { code: 0, ...view, unconfigured: legacy.map((p) => p.name) };
  }
  log(`revision concessioni: ${view.revision}`);
  if (view.peers.length === 0) log('nessun peer diretto');
  for (const peer of view.peers) {
    log(`${peer.name}${peer.nodeId ? ` (${peer.nodeId})` : ''}: ${peer.label}`);
    log(`  ${ACCESS_GRANT_KEYS.map((key) => `${key}=${peer.grants[key] ? 'on' : 'off'}`).join(' ')} · celle ${peer.grants.cellVisibility}`);
  }
  if (legacy.length > 0) log(`da configurare esplicitamente (${legacy.length}): ${legacy.map((p) => p.name).join(', ')}`);
  return { code: 0, ...view, unconfigured: legacy.map((p) => p.name) };
}
...
function nodesRemove(opts) {
  ...
  // Ferma un eventuale forward tunnel ATTIVO prima di togliere la config (da revisione):
...
module.exports = {
  fetchFeedDiagnostics, feedLinesFor,

  nodesAccess, ACCESS_GRANT_KEYS,
