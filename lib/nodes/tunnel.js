'use strict';
// lib/nodes/tunnel.js — SSH tunnel manager.
//
...
// `status --json` e (in) dal proxy. Porta STABILE da nodes.json; una
...
// SSH options shared by both templates (exact order and values).
...
function assertForwardSpec(node) {
  ...
  // Coppia pannello (stessa-origin remota): o entrambe o nessuna, e mai
...
// Forward client->node (client role):
...
// Exponential backoff + jitter. Deterministic with an injected rng.
...
// ssh -V esiste su ogni platform supportata; ENOENT = binario mancante (da revisione).
...
function startTunnel(opts) {
  ...
  // crash (da revisione: prima restituiva started:true e poi l'event 'error' killava Node).
  ...
  closeOwnedFd(); // copia del padre: il figlio ha la sua dup; nessun leak (da revisione)
...
// da revisione: la riga authorized_keys con le destinazioni EXPLICITE dei -L. Una sola
...
//
// Validare il nome dell'algoritmo, la struttura del blob o il risultato di
// `ssh-keygen -l` su `A.pub` prova soltanto che il file contiene UNA chiave
// valida — non che sia LA chiave derivata dalla privata di `A`. Se `A.pub` e'
// stale, ripristinato
...
// registrato in revisione: `null | stringa` comprimeva stati DIVERSI, e il
...
// Compongono al confine UI/log, mai qui. Si aggancia a: una chiave cifrata
