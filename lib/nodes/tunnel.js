...
function assertForwardSpec(node) {
  ...
  if (!store.isAbsPath(node.keyPath)) throw new Error('tunnel: keyPath non valido');
  if (!store.parseSsh(node.ssh)) throw new Error('tunnel: ssh (user@host) non valido');
}
...
function startForward(opts) {
  ...
  const args = buildForwardArgs(node);
  return startTunnel({ ...opts, name: node.name, args });
}
