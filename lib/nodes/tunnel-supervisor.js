...
// die: it stays "degraded" and keeps retrying on a fixed cadence (default 60s),
// so a transient reverse listener on the hub (e.g. a mobile reconnect) heals on
// its own without an OFF/ON. Tunable for determinism in tests.
const steadyRetryMsRaw = Number(process.env.NEXUSCREW_TUNNEL_STEADY_RETRY_MS || 60000);
const steadyRetryMs = Number.isFinite(steadyRetryMsRaw) && steadyRetryMsRaw >= 1
...
// the reverse failure class plus the negotiated reversePort and whether it still
// owns the generation, then retries on a fixed cadence. It never exits on a
// reverse-forward failure: the tunnel self-heals when the channel frees up.
function enterDegraded(diagnosis) {
  ...
  const ownership = ownsGeneration() ? 'self' : 'unknown';
