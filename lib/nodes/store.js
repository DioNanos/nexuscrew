...
const NODE_KEYS = new Set([
  ...
  'direction', 'reversePort', 'shared', 'label', 'audio',
]);
function parseNode(n, schemaVersion = SCHEMA_VERSION) {
  ...
  // audio: WP2 node audio capability/consent. consent default false; schema
  // chiuso (solo `consent` qui — la capability metadata e' runtime, esposta via
  // route, mai persistita nel node store). Mai mutabile via federation.
  let audio;
  if (n.audio !== undefined) {
    if (!n.audio || typeof n.audio !== 'object' || Array.isArray(n.audio)) return null;
    if (Object.keys(n.audio).some((k) => k !== 'consent')) return null;
    const consent = n.audio.consent === undefined ? false : n.audio.consent;
...
function redactNode(n) {
  const out = {
    ...
    visibility: n.visibility || 'network',
    audio: { consent: !!(n.audio && n.audio.consent === true) },
