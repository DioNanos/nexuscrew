'use strict';
// lib/nodes/store.js — nodes.json secret store.
//
// nodes.json e' un SECRET STORE, non config qualunque: contiene i token dei nodi
// Remoti (iniettati dal proxy in). Percio' riusa lo stesso hardening di
...
// name: strict key also used as a path/route segment -> no '.', no '/',
// no uppercase. Aligned with the strict name contract (^[a-z0-9-]{1,32}$).
...
// Va usato via Host alias in ~/.ssh/config, fuori scope).
...
// lanciare setHeader in modo sincrono (ERR_INVALID_CHAR). (hardening).
...
const NODE_KEYS = new Set([
  ...
  // eventsReceive: the CLIENT's own local choice to import an owner's feed.
  // It is NOT a grant the owner sets, it never travels in a preset, and it
  // only ever decides what THIS node pulls — never what a peer may read here.
  'eventsReceive',
  ...
  // Stessa-origin, meta' remota: la porta pannello del peer inoltrata sul
  ...
  // Federated access grants: what this peer may do with the resources this node
  // OWNS (events, asks, files) beyond cell visibility. Same shape and same rule
  // as panelAccess/liveHostAccess: default denied, one node at a time. A peer
  // does not gain them by being paired, and the receiving client's own choice
  // to receive events is NOT one of them (it is local to that client).
  'eventsAccess', 'nodeEventsAccess', 'askReplyAccess', 'filesReadAccess', 'peerOperatorAccess',
  // Whether the grant vector was set EXPLICITLY. Grants alone cannot tell a
  // legacy record (all defaults denied) from a deliberate vector that happens
  // to deny everything: without this marker a legacy peer would read as a
  // coherent "custom" preset instead of "not configured". It is provenance,
  // not a permission — it grants nothing and handlers never read it as a role.
  'accessConfigured',
]);
...
function parseNode(n, schemaVersion = SCHEMA_VERSION) {
  ...
  if (n.eventsReceive !== undefined && typeof n.eventsReceive !== 'boolean') return null;
  ...
  const out = {
    ...
    // Le cinque concessioni federate seguono la stessa regola: assenti valgono
    // NEGATE. Nessuna migrazione silenziosa al load: il vettore va impostato da
    // un atto esplicito (preset o edit granulare), altrimenti resta negato.
    eventsReceive: n.eventsReceive === undefined ? false : n.eventsReceive === true,
    eventsAccess: n.eventsAccess === undefined ? false : n.eventsAccess,
    nodeEventsAccess: n.nodeEventsAccess === undefined ? false : n.nodeEventsAccess,
    askReplyAccess: n.askReplyAccess === undefined ? false : n.askReplyAccess,
    filesReadAccess: n.filesReadAccess === undefined ? false : n.filesReadAccess,
    peerOperatorAccess: n.peerOperatorAccess === undefined ? false : n.peerOperatorAccess,
    accessConfigured: n.accessConfigured === true,
  ...
  if (typeof out.eventsAccess !== 'boolean') return null;
  if (typeof out.nodeEventsAccess !== 'boolean') return null;
  if (typeof out.askReplyAccess !== 'boolean') return null;
  if (typeof out.filesReadAccess !== 'boolean') return null;
  if (typeof out.peerOperatorAccess !== 'boolean') return null;
  if (typeof out.accessConfigured !== 'boolean') return null;
  ...
  // Coppia pannello (stessa-origin remota): o entrambe le porte o nessuna.
...
function parseStore(raw) {
  try {
    ...
    const rootKeys = d.schemaVersion === SCHEMA_VERSION
      ? new Set(['schemaVersion', 'nodeId', 'nodes', 'rendezvous', 'reversePoolAnchor', 'reversePoolLedgerInitialized', 'accessRevision'])
    ...
    // Revision of the peer access policy. Absent means 0: a store written
    // before this field existed is at revision 0 and the first explicit write
    // advances it to 1. It is kept out of the parsed object when absent so a
    // plain load never rewrites the file with a field nobody set.
    if (d.accessRevision !== undefined) {
      if (d.schemaVersion !== SCHEMA_VERSION) return null;
      if (!Number.isInteger(d.accessRevision) || d.accessRevision < 0) return null;
      out.accessRevision = d.accessRevision;
    }
...
// Tenerlo strict impedisce che un call-site dimenticato reintroduca quel difetto.
...
// --- Concessioni federate: preset, CAS, inventario -------------------------

function accessRevisionOf(store) {
  return store && Number.isInteger(store.accessRevision) && store.accessRevision >= 0
    ? store.accessRevision : 0;
}

// Pure: returns a new store with the peer's grant vector replaced by a
// validated one. The vector is validated as a whole, so an edit that keeps
// live hosting while dropping another admin grant is refused here too.
function setPeerAccessGrants(store, name, grants) {
  const idx = store.nodes.findIndex((n) => n.name === name);
  if (idx < 0) throw new Error(`nodo sconosciuto: "${name}"`);
  const check = accessPresets.validateGrants(grants);
  if (!check.ok) throw new Error(`concessioni non valide: ${check.errors.join('; ')}`);
  const next = { ...store.nodes[idx] };
  next.cellVisibility = grants.cellVisibility;
  if (grants.cellVisibility === 'selected') next.cells = [...(grants.cells || [])];
  else delete next.cells;
  for (const key of accessPresets.BOOLEAN_GRANTS) next[key] = grants[key] === true;
  next.accessConfigured = true;
  const nodes = store.nodes.slice();
  nodes[idx] = next;
  const out = { ...store, schemaVersion: SCHEMA_VERSION, nodes };
  out.accessRevision = accessRevisionOf(store) + 1;
  return out;
}

// Pure: applies a named preset to one peer.
function setPeerAccessPreset(store, name, presetName) {
  const idx = store.nodes.findIndex((n) => n.name === name);
  if (idx < 0) throw new Error(`nodo sconosciuto: "${name}"`);
  const applied = accessPresets.applyPreset(store.nodes[idx], presetName);
  const vector = { cellVisibility: applied.cellVisibility };
  if (applied.cells !== undefined) vector.cells = applied.cells;
  for (const key of accessPresets.BOOLEAN_GRANTS) vector[key] = applied[key];
  return setPeerAccessGrants(store, name, vector);
}

// Read-only inventory: what every peer is allowed to do on THIS node, with the
// label derived from the grants. Used by `nodes show`, by the settings view and
// by the explicit migration: a legacy peer shows up as not configured, never as
// a preset inferred from defaults.
function accessInventory(store) {
  const peers = (store && store.nodes ? store.nodes : []).map((peer) => {
  ...
  return { revision: accessRevisionOf(store), peers };
}

// Compare-and-set write for the peer access policy: one validated read-modify-
// write over the store file. `expectedRevision` must be an integer that matches
// the current revision, otherwise the write is refused and the caller rereads
// and retries — never a permissive default. The whole path is synchronous, so
// two callers in this process cannot interleave between the read and the write.
function setPeerAccessGrantsCas({ filePath, name, grants, expectedRevision, extraPatch }) {
  const current = loadStoreStrict(filePath);
  if (!Number.isInteger(expectedRevision) || accessRevisionOf(current) !== expectedRevision) {
    return { ok: false, conflict: true, revision: accessRevisionOf(current) };
  }
  let next = current;
  if (extraPatch && Object.keys(extraPatch).length > 0) next = updateNode(next, name, extraPatch);
  next = setPeerAccessGrants(next, name, grants);
  const written = atomicWriteStore(filePath, next);
  return {
    ok: true,
    name,
    revision: accessRevisionOf(written),
    grants: accessPresets.grantsOf(written.nodes.find((n) => n.name === name)).grants,
    label: accessPresets.grantsOf(written.nodes.find((n) => n.name === name)).label,
  };
}

// Same, by preset name.
function setPeerAccessPresetCas({ filePath, name, presetName, expectedRevision, extraPatch }) {
  const current = loadStoreStrict(filePath);
  if (!Number.isInteger(expectedRevision) || accessRevisionOf(current) !== expectedRevision) {
    return { ok: false, conflict: true, revision: accessRevisionOf(current) };
  }
  let next = current;
  if (extraPatch && Object.keys(extraPatch).length > 0) next = updateNode(next, name, extraPatch);
  next = setPeerAccessPreset(next, name, presetName);
  const written = atomicWriteStore(filePath, next);
  const view = accessPresets.grantsOf(written.nodes.find((n) => n.name === name));
  return { ok: true, name, revision: accessRevisionOf(written), grants: view.grants, label: view.label };
}
...
function redactNode(n) {
  const out = {
    ...
    eventsReceive: n.eventsReceive === true,
...
// Se config.json contiene un array `nodes` legacy (vecchio formato pre-),
...
module.exports = {
  ...
  // concessioni federate (preset, CAS, inventario)
  accessRevisionOf, setPeerAccessGrants, setPeerAccessPreset, accessInventory,
  setPeerAccessGrantsCas, setPeerAccessPresetCas,
  // redazione
