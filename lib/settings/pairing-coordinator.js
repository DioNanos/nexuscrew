...
function createPairHandler(deps) {
  ...
  return async (req, res) => {
    ...
    try {
      ...
      try {
        jr = await pairFetch(`http://127.0.0.1:${localPort}/pair/join`, {
          ...
          body: JSON.stringify({
            ...
            shared: false,
            // La nostra pubblica, passo 1 del modello di autorita'. Se non si
            // riesce a leggerla si accoppia senza: il passo 1 non ha effetti, e
            // un guasto sulla chiave non deve togliere la capacita' di
            // accoppiare un dispositivo.
            ...(() => {
              try {
                return { publicKey: identity.ensureNodeKey({ keyPath: identity.keyPathNextTo(nodesPath) }).publicKey };
              } catch (_) { return {}; }
            })(),
      ...
      //
      // Porta pannello del peer (stessa-origin, meta' remota): se il peer
      ...
      st = nodesStore.updateNode(st, b.name, {
        ...
        shared: false,
        // L'altra meta' dello scambio. Un hub piu' vecchio non manda la sua
        // pubblica e il pairing riesce lo stesso — nessun campo, nessun
        // cambiamento di comportamento.
        ...(identity.isPublicKey(joined.publicKey)
          ? { publicKey: joined.publicKey, keySource: 'pairing', keyBoundAt: new Date().toISOString() }
          : {}),
      ...
      // rifiuterà il canale pannello). La riga NON è un segreto (pubkey con
      // restrict/permitopen, stessa prassi della route /nodes) e va SOSTITUITA,
      // non aggiunta: due righe per la stessa chiave lascerebbero vivere anche
      // quella vecchia.
      let authorizedKeysPayload = null;
      if (peerPanelPort) {
        try {
          const nodo = nodesStore.getNode(st, b.name);
          // `ensureNodeKey` legge l'identita' FEDERATION (node-key.json, un
          // JSON con la chiave privata): passargli il path di una chiave SSH
          // lancia sempre, e l'eccezione inghiottita qui sotto trasformava il
          // difetto in "nessuna riga da mostrare". La chiave che serve e'
          // quella DICHIARATA per il nodo, derivata dalla sua privata.
          authorizedKeysPayload = nodesTunnel.authorizedKeysForNode(nodo, peerPanelPort);
        } catch (_) { authorizedKeysPayload = null; } // la riga è un'estensione: il pairing non fallisce per lei
      }
      send(res, 200, {
