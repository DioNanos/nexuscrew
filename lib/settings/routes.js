...
function settingsRoutes(deps = {}) {
  ...
  // Audio consent is a LOCAL-only mutation (UI is WP3). It is NOT in the
  // federation whitelist, so a federated consent mutation is unreachable/refused.
  r.patch('/nodes/:name/audio', mutGate, (req, res) => {
    const name = String(req.params.name || '');
    const consent = req.body && req.body.consent;
    if (!validName(name) || typeof consent !== 'boolean') {
      return send(res, 400, { error: 'body non valido: atteso {consent:boolean}' });
    }
    return applyNodeEdit(res, name, { audio: { consent } });
  });
