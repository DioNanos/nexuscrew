'use strict';
// lib/settings/routes.js — Settings API for the wizard/settings UI.
//
// Contratto duro:
//   - read-only: GET /api/settings (roles, firstRun, port, platform, service,
//     version).
//   - mutators (CLOSED LIST), all behind requireToken (mounted under the
//     already-authenticated /api router) + route-level READONLY gate
//     (lib/fleet/routes.js pattern, explicit 403):
//       POST   /config             scrittura config.json ATOMICA (whitelist chiavi)
//       POST   /token/rotate       reuses rotateToken(); token NEVER in the response
...
// Documentata in lib/nodes/commands.js: READONLY blocca le mutazioni di
...
// NIENTE reimplementazione della logica: i mutanti nodes incapsulano le
...
// Commento "mai spawn dalla superficie web" e' superato per da questo
// explicit contract ("restart tunnel processes" is an allowed mutation).
...
const aiDesktop = require('../fleet/ai-desktop.js');

// Whitelist chiavi scrivibili di config.json via API (lista chiusa dal task).
const CONFIG_KEYS = new Set(['roles', 'port', 'wizardDone', 'autoUpdate', 'alternateScreen', 'aiDesktop']);
...
const EDIT_KEYS = new Set([
  'label', 'ssh', 'sshPort', 'autostart', 'visibility', 'selected', 'cellVisibility', 'cells',
  // Federated access: `accessRole` applies a preset, the granular keys refine an
  // explicitly set vector, and `accessRevision` makes the write a compare-and-set.
  // A `role` key is NOT here: a declared role is refused instead of being stored.
  'accessRole', 'accessRevision', 'eventsAccess', 'nodeEventsAccess', 'askReplyAccess', 'filesReadAccess', 'peerOperatorAccess',
]);
...
// Belt: recursive removal of every `token` key from ANY response
...
function settingsRoutes(deps = {}) {
  ...
  const applyNodeEdit = (res, name, patch, extra = {}) => {
    const cap = capture();
    const out = nodesCmds.nodesEdit({ ...cliOpts(cap), ref: name, patch, ...extra });
    ...
    if (out.reason === 'revision-conflict') {
      return send(res, 409, { error: message, revision: out.revision });
    }
  ...
  r.post('/config', mutGate, (req, res) => {
    try {
      ...
      for (const k of Object.keys(b)) {
        if (!CONFIG_KEYS.has(k)) return send(res, 400, { error: `chiave non ammessa: "${k}" (whitelist: roles, port, wizardDone, autoUpdate, alternateScreen, aiDesktop)` });
      }
      if (Object.keys(b).length === 0) {
        return send(res, 400, { error: 'nessuna chiave da scrivere (whitelist: roles, port, wizardDone, autoUpdate, alternateScreen, aiDesktop)' });
      }
      ...
      if (b.aiDesktop !== undefined && typeof b.aiDesktop !== 'boolean') {
        return send(res, 400, { error: 'aiDesktop deve essere boolean' });
      }
      ...
      if (b.aiDesktop !== undefined) next.aiDesktop = b.aiDesktop;
  ...
  // GET: la chiave esplicita comanda; se l'utente non l'ha mai toccata, il
  // default si deriva dal container (in esecuzione → true). docker viene
  // interrogato SOLO qui, su chiamata esplicita — mai nel percorso caldo del
  // fleetStatus. Il binario passa dal seam per i test (finto binario).
  r.get('/ai-desktop', async (req, res) => {
    const { cfg: cfgNow } = readConfigFile(configPath);
    const esplicita = aiDesktop.chiaveEsplicita(cfgNow);
    const stato = await aiDesktop.containerRunning({ dockerBin: seams.aiDesktopDockerBin });
    const desired = esplicita !== null ? esplicita : stato.running;
    return send(res, 200, {
      desired, running: stato.running, exists: stato.exists, explicit: esplicita !== null,
      ...(stato.error ? { error: stato.error } : {}),
    });
  });

  // POST {enabled}: salva la chiave ED esegue start o stop, restituendo
  // l'esito VERO del comando — nessun successo ottimistico. Se docker
  // risponde male la risposta è 500 con la causa: la chiave resta salvata
  // (la spunta dice COSA si vuole, l'esito dice SE il mondo ci è arrivato)
  // e il badge della UI mostra il disallineamento invece di fingere.
  r.post('/ai-desktop', mutGate, async (req, res) => {
    const b = req.body;
    if (!b || typeof b !== 'object' || Array.isArray(b) || typeof b.enabled !== 'boolean') {
      return send(res, 400, { error: 'body deve essere {enabled: boolean}' });
    }
    const { cfg: cfgNow } = readConfigFile(configPath);
    try {
      atomicWriteConfig(configPath, { ...cfgNow, aiDesktop: b.enabled });
    } catch (e) {
      return send(res, 500, { error: `config non salvata: ${e.message}` });
    }
    const dockerOpts = { dockerBin: seams.aiDesktopDockerBin };
    const esito = b.enabled
      ? await aiDesktop.avvia(dockerOpts)
      : await aiDesktop.ferma(dockerOpts);
    if (!esito.ok) {
      return send(res, 500, {
        error: esito.error || 'comando docker fallito',
        desired: b.enabled,
        running: esito.running ?? null,
        timeout: !!esito.timeout,
      });
    }
    return send(res, 200, { ok: true, desired: b.enabled, running: esito.running });
  });
  ...
  // Contract: "atomic write of the token file + closing of the active local
  // WS/API sessions + proxy credential reload". From review: before, this route
  // only wrote the file, and the server kept the OLD token in memory -> it stayed
  // accepted until a manual restart. Now: (1) atomic write, (2) live reload
  ...
  r.patch('/nodes/:name', mutGate, (req, res) => {
    ...
    if (Object.hasOwn(body, 'accessRevision') && !Number.isInteger(body.accessRevision)) {
      return send(res, 400, { error: 'accessRevision vuole un intero' });
    }
    const { accessRevision, ...patch } = body;
    return applyNodeEdit(res, name, patch, { expectedRevision: accessRevision });
  });
  ...
  // READONLY gate (tunnel process restart is among the mutators of the
  // closed list and NEXUSCREW_READONLY blocks ALL of them). Before, these
  // Endpoints were explicitly NOT gated (" decision") — but that contradicted
  // the hard contract: up/down/restart mutate processes and must be blocked
  // in READONLY. /nodes/:name/test stays NOT gated (diagnostic probe).
  ...
  r.patch('/nodes/:name/share', mutGate, async (req, res) => {
    ...
    try {
      ...
      // OFF is fail-safe and crash-durable: persist desired
      // false -> authenticated revoke on the still-live -L channel -> local restart
      // without -R. NEVER restart-before-revoke: the -L is the very channel of
      // the revocation, and a crash before the ACK can never republish by intent.
  ...
  r.put('/audio/groups/:name', mutGate, (req, res) => {
    ...
    // : il catch nudo raccontava QUALUNQUE errore come «gruppo non
  ...
  r.delete('/audio/groups/:name', mutGate, (req, res) => {
    try {
      ...
      // : come PUT sopra — vedi il commento lì.
  ...
  // SENZA eseguirlo: il contratto vieta il restart automatico dall'API
  ...
  r.post('/service/regenerate', mutGate, (_req, res) => {
    try {
      ...
      // 1-bis: come in init — il path del boot si risolve alla
