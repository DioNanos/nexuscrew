...
const { setConsent: setAudioConsent } = require('../audio/consent.js');
...
function settingsRoutes(deps = {}) {
  ...
  // Audio consent is SELF-OWNED by the local target node: stored in a dedicated
  // local file (NOT in peer nodes.json records). Local-only mutation; the
  // federation whitelist never exposes /audio/consent, so a federated mutation
  // is unreachable/refused. UI is WP3.
  r.patch('/audio/consent', mutGate, (req, res) => {
    ...
    try {
      const result = setAudioConsent({ home }, consent, home);
