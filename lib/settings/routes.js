...
function settingsRoutes(deps = {}) {
  ...
  r.patch('/nodes/:name/share', mutGate, async (req, res) => {
    ...
    const applyLocal = async (shared) => {
      ...
      // del supervisor: in quel caso il canale resta in quarantena, vivo. Lo
      // stato persistito direbbe "privato" mentre il reverse pool e' ancora su:
      // non si puo' ignorare quel booleano, va riportato a chi chiama.
      if (!shared && node.reversePool && typeof reverseSlots?.close === 'function') {
    ...
    const reconcileOff = async ({ restart = false, unchanged = false } = {}) => {
      ...
      if (node.reversePool && typeof reverseSlots?.close === 'function') {
        reverseChannelClosed = (await reverseSlots.close(name)) !== false;
      }
      try {
