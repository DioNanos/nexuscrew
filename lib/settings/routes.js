...
function settingsRoutes(deps = {}) {
  ...
  const audioCfg = { home, tokenPath: cfg && cfg.tokenPath };
  r.get('/audio', (_req, res) => {
    try {
      const consent = isAudioConsent(audioCfg, home) === true;
      const adapter = audioAdapters.createAdapter(audioAdapters.detectAdapter({}));
      ...
      return send(res, 200, describeCapability({ adapter, consent }));
    } catch (_) {
